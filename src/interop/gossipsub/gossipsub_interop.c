...
static void log_received_message_event(const context_t *ctx, const char *topic, const char *from_peer,
...
static int derive_node_identity(int node_id, buffer_t *priv_key_pb, char peer_id_str[MAX_PEER_ID_STR])
{
	...
	buffer_t pb = {0};
	if (build_private_key_protobuf(PEER_ID_KEY_ED25519, priv_concat, sizeof(priv_concat), &pb) != 0)
		return -1;
	if (priv_key_pb)
		*priv_key_pb = pb;
	if (peer_id_str)
	{
		peer_id_t *pid = NULL;
		if (peer_id_new_from_private_key_pb(pb.bytes, pb.len, &pid) != PEER_ID_OK)
		{
			buffer_free(&pb);
			return -1;
		}
		size_t peer_id_len = 0U;
		if (peer_id_text_write_default(pid, peer_id_str, MAX_PEER_ID_STR, &peer_id_len) != PEER_ID_OK)
		{
			peer_id_free(pid);
			buffer_free(&pb);
			return -1;
...
static libp2p_err_t message_id_fn(const libp2p_gossipsub_message_t *msg, uint8_t **out_id, size_t *out_len,
				  void *user_data)
{
	(void)user_data;
...
static libp2p_gossipsub_validation_result_t validator_cb(const libp2p_gossipsub_message_t *msg, void *user_data)
{
	...
	{
		size_t from_len = 0U;
		(void)peer_id_text_write_default((const peer_id_t *)msg->from, from_buf, sizeof(from_buf), &from_len);
	}
	log_received_message_event(state->ctx, msg->topic.topic ? msg->topic.topic : NULL,
				   (msg->from && from_buf[0]) ? from_buf : NULL, id_buf);
	return LIBP2P_GOSSIPSUB_VALIDATION_ACCEPT;
}
...
static int connect_to_node(context_t *ctx, int target_node)
{
	...
	peer_id_t *pid = NULL;
	if (peer_id_new_from_text(peer_id_str, &pid) != PEER_ID_OK)
	{
...
static int apply_gossipsub_params(const gossipsub_params_t *params, libp2p_gossipsub_config_t *cfg)
{
	...
	{
		double hb_ms =
			cfg->heartbeat_interval_ms > 0 ? cfg->heartbeat_interval_ms : ns_to_ms(HEARTBEAT_DEFAULT_NS);
...
static int run(const char *params_path)
{
	...
	(void)libp2p_host_builder_flags(builder,
					LIBP2P_HOST_F_AUTO_IDENTIFY_INBOUND | LIBP2P_HOST_F_AUTO_IDENTIFY_OUTBOUND);
