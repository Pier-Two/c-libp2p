...
static char *redis_blpop(int fd, const char *key, int timeout_sec)
{
	char cmd[256];
	snprintf(cmd, sizeof(cmd), "*3\r\n$5\r\nBLPOP\r\n$%zu\r\n%s\r\n$%d\r\n%d\r\n", strlen(key), key,
		 (int)snprintf(NULL, 0, "%d", timeout_sec), timeout_sec);
	if (redis_send(fd, cmd) != 0)
		return NULL;
	char line[256];
...
static int build_loopback_variant(const char *addr, char *out, size_t out_len)
{
	...
	if (multiaddr_get_address_bytes(ma, 1, port_bytes, &port_len) != MULTIADDR_SUCCESS ||
	    port_len != sizeof(port_bytes))
	{
		multiaddr_free(ma);
		return 0;
	}
...
static void interop_evt_cb(const libp2p_event_t *evt, void *ud)
{
	...
	if (evt->kind == LIBP2P_EVT_LISTEN_ADDR_ADDED && evt->u.listen_addr_added.addr)
	{
			...
			if (proto_start)
			{
				...
				if (test_sock >= 0)
				{
					...
					if (connect(test_sock, (struct sockaddr *)&test_addr, sizeof(test_addr)) == 0)
					{
						...
						{
							char ip_str[INET_ADDRSTRLEN];
							inet_ntop(AF_INET, &local_addr.sin_addr, ip_str,
								  INET_ADDRSTRLEN);
							snprintf(actual_addr, sizeof(actual_addr), "/ip4/%s%s", ip_str,
								 proto_start);
		...
		if (strstr(publish_buf, "/ip4/0.0.0.0/") || strstr(publish_buf, "/ip4/127.0.0.1/"))
		{
			...
			if (first_non_loopback_ipv4(ip_buf, sizeof(ip_buf)))
			{
				...
				if (prefix)
				{
					...
					if (after)
					{
						...
						if (snprintf(rewritten, sizeof(rewritten), "/ip4/%s%s", ip_buf, after) <
						    (int)sizeof(rewritten))
						{
							snprintf(publish_buf, sizeof(publish_buf), "%s", rewritten);
							LP_LOGI("INTEROP",
								"listener replacing loopback address with %s",
								publish_buf);
						}
					}
				}
			...
			{
				LP_LOGW("INTEROP", "listener failed to find non-loopback IPv4, publishing %s",
					publish_buf);
			}
		}
			...
			if (libp2p_host_get_peer_id(sync->host, &pid) == 0 && pid)
			{
				    ...
				    pid_len > 0U)
				{
					...
					if (base_len < sizeof(publish_buf))
					{
						...
						int appended =
							snprintf(publish_buf + base_len, remaining, "/p2p/%s", pid_buf);
	...
	else if (evt->kind == LIBP2P_EVT_STREAM_OPENED)
	{
		pthread_mutex_lock(&sync->mtx);
		sync->open_streams++;
		if (evt->u.stream_opened.protocol_id &&
		    strcmp(evt->u.stream_opened.protocol_id, LIBP2P_PING_PROTO_ID) == 0)
			sync->ping_seen = 1;
		pthread_cond_broadcast(&sync->cv);
		pthread_mutex_unlock(&sync->mtx);
	}
...
static int run_listener(const char *ip, const char *redis_host, const char *redis_port, int timeout,
			const char *transport_name, const char *security_name, const char *muxer_name)
{
	...
	LP_LOGI("INTEROP",
		"run_listener: ip=%s redis_host=%s redis_port=%s timeout=%d transport=%s security=%s muxer=%s", ip,
	...
	const int is_quic =
		(transport_name && ((strcmp(transport_name, "quic") == 0) || (strcmp(transport_name, "quic-v1") == 0)));
	...
	const char *security_to_use =
		(!is_quic && security_name && security_name[0]) ? security_name : (!is_quic ? "noise" : NULL);
...
static int run_dialer(const char *redis_host, const char *redis_port, int timeout, const char *transport_name,
		      const char *security_name, const char *muxer_name)
{
	const char *sec_log = (security_name && security_name[0]) ? security_name : "(builtin)";
	const char *mux_log = (muxer_name && muxer_name[0]) ? muxer_name : "(builtin)";
	...
	if ((rc != 0 || !ping_stream) && loopback_addr[0] != '\0')
	{
		...
		rc = libp2p_host_dial_protocol_blocking(host, loopback_addr, LIBP2P_PING_PROTO_ID, timeout * 1000,
							&ping_stream);
	}
	if (rc != 0 || !ping_stream)
	{
