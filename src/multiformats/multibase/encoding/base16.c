...
static int base16_decode_nibble(uint8_t ch, uint8_t *value)
{
	int valid;
	uint8_t nibble;
	...
	else
	{
		...
		else
		{
			while (index < data_len)
			{
				...
				out[index * 2U] = hex_digits[high];
				out[(index * 2U) + 1U] = hex_digits[low];
				index++;
			}
	...
	else
	{
		...
		else
		{
			...
			while ((index < decoded_len) && (result == (int)MULTIBASE_SUCCESS))
			{
				...
				high = 0U;
				low = 0U;
				high_valid = base16_decode_nibble((uint8_t)in[index * 2U], &high);
				low_valid = base16_decode_nibble((uint8_t)in[(index * 2U) + 1U], &low);
