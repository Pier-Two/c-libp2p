...
static int base32_upper_compute_encoded_len(size_t data_len, size_t *encoded_len)
{
	int status;
	size_t local_len;
...
static int base32_upper_value(uint8_t ch, uint8_t *value)
{
	int valid;
	uint8_t local;
...
int multibase_base32_upper_encode(const uint8_t *data, size_t data_len, char *out, size_t out_len)
{
	...
	static const char base32_upper_alphabet[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";
	...
	{
		result = base32_upper_compute_encoded_len(data_len, &encoded_len);
		...
		else
		{
			while (in_index < data_len)
			{
				...
				while (bits_in_buffer >= 5U)
				{
					...
					out[out_index] = base32_upper_alphabet[value];
					out_index++;
				}
			}
		...
		else
		{
			...
			else
			{
				...
				while ((in_index < data_len) && (result == (int)MULTIBASE_SUCCESS))
				{
					...
					value = 0U;
					valid = base32_upper_value((uint8_t)in[in_index], &value);
					if (valid == 0)
					{
