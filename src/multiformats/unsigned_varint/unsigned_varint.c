		...
		*written = (size_t)0U;
		if (value > UNSIGNED_VARINT_MAX_VALUE) {
			...
			if (required_size == (size_t)0U) {
				...
				remaining_value = value;
				for (index = 0; index < required_size;
				     ++index) {
					uint8_t byte;

					byte = (uint8_t)(remaining_value &
							 UINT64_C(0x7F));
					remaining_value >>= 7U;
				...
				if (index < UNSIGNED_VARINT_MAX_ENCODED_SIZE) {
					...
					if ((byte & UINT8_C(0x80)) ==
						...
						encoded_size =
							index + (size_t)1U;
						if (decoded_value >
						    UNSIGNED_VARINT_MAX_VALUE) {
							status =
								UNSIGNED_VARINT_ERR_VALUE_OVERFLOW;
						} else if (
							unsigned_varint_size_internal(
								decoded_value) !=
							encoded_size) {
							status =
					 ...
					 */
					if (index >
					    UNSIGNED_VARINT_MAX_ENCODED_SIZE) {
						status =
