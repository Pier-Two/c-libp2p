	...
	*read = (size_t)0U;
	...
	for (index = 0; index < in_size; ++index) {
		...
		if (index < UNSIGNED_VARINT_MAX_ENCODED_SIZE) {
			decoded_value |= ((uint64_t)(byte & UINT8_C(0x7F)))
					 << shift;
			if ((byte & UINT8_C(0x80)) == UINT8_C(0)) {
				...
				if (unsigned_varint_size_internal(
					    decoded_value) != encoded_size) {
		...
		/*
		 * Tenth byte handling: permit explicit overflow detection for
		 * 2^63, reject overlong/minimal-invalid 10-byte forms, and
		 * reject any continuation beyond this point.
		 */
		if ((byte & UINT8_C(0x80)) != UINT8_C(0)) {
			return UNSIGNED_VARINT_ERR_TOO_LONG;
		}
