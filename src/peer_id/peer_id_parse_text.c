...
static peer_id_error_t peer_id_decode_multibase(multibase_t base, const char *text, uint8_t **out_buf, size_t *out_len)
{
...
peer_id_error_t peer_id_internal_text_write(const peer_id_t *pid, peer_id_text_format_t fmt, char *out, size_t out_cap,
					    size_t *out_len)
{
	peer_id_error_t status;
	ptrdiff_t mb_rc;
	...
	*out_len = (size_t)0U;
	...
	if (fmt == PEER_ID_TEXT_CIDV1_BASE32)
	{
		...
		if (unsigned_varint_encode((uint64_t)MULTICODEC_LIBP2P_KEY, codec_buf, sizeof(codec_buf),
					   &codec_size) != UNSIGNED_VARINT_OK)
		{
			return PEER_ID_ERR_ENCODING;
		}
