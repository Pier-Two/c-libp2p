...
static libp2p_upgrader_err_t upgrader_upgrade_outbound(libp2p_upgrader_t *self, libp2p_conn_t *raw,
						       const peer_id_t *remote_hint, libp2p_uconn_t **out)
{
	if (!self || !raw || !out)
	...
	libp2p_security_err_t rc =
		libp2p_noise_negotiate_outbound((libp2p_security_t *)ctx->security[0], raw, remote_hint,
						ctx->handshake_timeout_ms, &secured, &remote_peer);
	...
	if (ctx->muxers && ctx->n_muxers > 0)
	{
		...
		{
			libp2p_muxer_err_t mrc = libp2p_muxer_negotiate_outbound((libp2p_muxer_t *)ctx->muxers[i],
										 secured, ctx->handshake_timeout_ms);
...
static libp2p_upgrader_err_t upgrader_upgrade_inbound(libp2p_upgrader_t *self, libp2p_conn_t *raw, libp2p_uconn_t **out)
{
	...
	libp2p_conn_t *secured = NULL;
	peer_id_t *remote_peer = NULL;
	libp2p_security_err_t rc = libp2p_noise_negotiate_inbound((libp2p_security_t *)ctx->security[0], raw,
								  ctx->handshake_timeout_ms, &secured, &remote_peer);
	if (rc != LIBP2P_SECURITY_OK)
		return LIBP2P_UPGRADER_ERR_HANDSHAKE;
	...
	if (ctx->muxers && ctx->n_muxers > 0)
	{
		for (size_t i = 0; i < ctx->n_muxers; i++)
		{
			libp2p_muxer_err_t mrc = libp2p_muxer_negotiate_inbound((libp2p_muxer_t *)ctx->muxers[i],
										secured, ctx->handshake_timeout_ms);
			if (mrc == LIBP2P_MUXER_OK)
			{
				selected = ctx->muxers[i];
