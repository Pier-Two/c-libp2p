...
int libp2p_peerstore_add_addr(libp2p_peerstore_t *ps, const peer_id_t *peer, const multiaddr_t *addr, int ttl_ms)
{
	...
	if (!e)
	{
		...
		if (peer_id_clone(peer, &e->pid) != PEER_ID_OK)
		{
			free(e);
			pthread_mutex_unlock(&ps->mtx);
			return -1;
...
int libp2p_peerstore_get_addrs(const libp2p_peerstore_t *ps_const, const peer_id_t *peer,
			       const multiaddr_t ***out_addrs, size_t *out_len)
{
...
int libp2p_peerstore_set_public_key(libp2p_peerstore_t *ps, const peer_id_t *peer, const uint8_t *pubkey_pb,
				    size_t pubkey_pb_len)
{
...
int libp2p_peerstore_get_public_key(const libp2p_peerstore_t *ps_const, const peer_id_t *peer, uint8_t **out_pb,
				    size_t *out_len)
{
...
int libp2p_peerstore_set_protocols(libp2p_peerstore_t *ps, const peer_id_t *peer, const char *const *protocols,
				   size_t n_protocols)
{
	if (!ps || !peer)
		return -1;
...
int libp2p_peerstore_get_protocols(const libp2p_peerstore_t *ps_const, const peer_id_t *peer,
				   const char ***out_protocols, size_t *out_len)
{
	...
	if (!e || e->n_protocols == 0)
	{
		...
		*out_len = 0;
		char pid_buf[128];
		size_t pid_len = 0U;
		if (peer_id_text_write(peer, PEER_ID_TEXT_LEGACY_BASE58, pid_buf, sizeof(pid_buf), &pid_len) !=
		    PEER_ID_OK)
			snprintf(pid_buf, sizeof(pid_buf), "<unknown>");
