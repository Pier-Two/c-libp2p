...
void libp2p_host_free(libp2p_host_t *host)
{
	...
	if (host->have_identity && host->local_peer)
	{
		peer_id_free(host->local_peer);
		host->local_peer = NULL;
	}
...
int libp2p_host_set_private_key(libp2p_host_t *host, const uint8_t *privkey_pb, size_t privkey_len)
{
	...
	peer_id_error_t perr = peer_id_new_from_private_key_pb(privkey_pb, privkey_len, &host->local_peer);
	if (perr != PEER_ID_OK)
	{
		memset(host->identity_key, 0, host->identity_key_len);
		free(host->identity_key);
...
int libp2p_host_get_peer_id(const libp2p_host_t *host, peer_id_t **out)
{
	...
	if (!host->have_identity || !host->local_peer)
		return LIBP2P_ERR_INTERNAL;
	peer_id_t *dup = NULL;
	if (peer_id_clone(host->local_peer, &dup) != PEER_ID_OK)
		return LIBP2P_ERR_INTERNAL;
	char pid_buf[128];
	size_t pid_len = 0U;
	if (peer_id_text_write(dup, PEER_ID_TEXT_LEGACY_BASE58, pid_buf, sizeof(pid_buf), &pid_len) != PEER_ID_OK)
...
{
	libp2p_peerstore_free_protocols(protocols, len);
}
/* One-shot timer thread: waits briefly, then runs async publisher inline */
static void *__libp2p__publisher_timer(void *arg)
{
	(void)arg;
	return NULL;
}
