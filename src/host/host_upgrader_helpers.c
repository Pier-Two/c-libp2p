...
static libp2p_muxer_t *(*quic_muxer_factory)(libp2p_host_t *host, libp2p_quic_session_t *session,
					     const multiaddr_t *local, const multiaddr_t *remote,
					     libp2p_conn_t *conn) = libp2p_quic_muxer_new;

void libp2p__host_set_quic_muxer_factory(libp2p_muxer_t *(*factory)(libp2p_host_t *host, libp2p_quic_session_t *session,
								    const multiaddr_t *local, const multiaddr_t *remote,
								    libp2p_conn_t *conn))
{
...
int libp2p__host_upgrade_outbound(libp2p_host_t *host, libp2p_conn_t *raw, const peer_id_t *remote_hint,
				  int allow_mplex, libp2p_uconn_t **out_uc)
{
	if (!host || !raw || !out_uc)
	...
	if (urc != LIBP2P_UPGRADER_OK || !uc || !uc->conn || !uc->muxer)
	{
		...
		evt.u.outgoing_conn_error.msg =
			(urc == LIBP2P_UPGRADER_ERR_MUXER) ? "muxer negotiation failed" : "security negotiation failed";
		libp2p_event_publish(host, &evt);
...
int libp2p__host_upgrade_inbound(libp2p_host_t *host, libp2p_conn_t *raw, int allow_mplex, libp2p_uconn_t **out_uc)
{
	if (!host || !raw || !out_uc)
		return LIBP2P_ERR_NULL_PTR;
	...
	if (urc != LIBP2P_UPGRADER_OK || !uc || !uc->conn || !uc->muxer)
	{
		...
		evt.u.incoming_conn_error.msg =
			(urc == LIBP2P_UPGRADER_ERR_MUXER) ? "muxer negotiation failed" : "noise negotiation failed";
		libp2p_event_publish(host, &evt);
		return libp2p_error_from_upgrader(urc);
	}
...
int libp2p__host_upgrade_outbound_quic(libp2p_host_t *host, libp2p_conn_t *session_conn, libp2p_uconn_t **out_uc)
{
	...
	libp2p_muxer_t *mx = quic_muxer_factory(host, session, libp2p_conn_local_addr(session_conn),
						libp2p_conn_remote_addr(session_conn), session_conn);
	if (!mx)
	{
...
int libp2p__host_upgrade_inbound_quic(libp2p_host_t *host, libp2p_conn_t *session_conn, libp2p_uconn_t **out_uc)
{
	...
	if (!uc)
	{
		libp2p_muxer_free(mx);
		libp2p_conn_free(session_conn);
		libp2p__emit_incoming_error_with_peer(host, remote_peer, LIBP2P_ERR_INTERNAL,
						      "quic inbound upgrade allocation failed");
		peer_id_free(remote_peer);
		return LIBP2P_ERR_INTERNAL;
	}
