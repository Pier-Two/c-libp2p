...
static int addr_supports_identify_push(libp2p_host_t *host, const char *addr)
{
	...
	peer_id_t *tmp = NULL;
	peer_id_error_t perr = peer_id_new_from_text(peer_part, &tmp);
	if (perr != PEER_ID_OK)
		return 0;
...
{
	if (!peers || !len || !cap || !candidate)
		return LIBP2P_ERR_NULL_PTR;

	peer_id_t **arr = *peers;
	for (size_t i = 0; i < *len; i++)
	{
		if (peer_id_equal(arr[i], candidate) == 1)
			return 0;
	}
	...
	if (peer_id_clone(candidate, &arr[*len]) != PEER_ID_OK)
		return LIBP2P_ERR_INTERNAL;
	(*len)++;
	return 0;
}
...
static void *__libp2p__publisher_async(void *arg)
{
	...
	LP_LOGD("IDENTIFY_PUB", "[async] start (host=%p attempt=%d running=%d)", (void *)host, attempt,
		running_initial ? 1 : 0);
	LIBP2P_TRACE("idpush", "async start host=%p attempt=%d running=%d", (void *)host, attempt,
		     running_initial ? 1 : 0);
	free(pc);
	...
	for (size_t i = 0; i < peers_len && atomic_load_explicit(&host->running, memory_order_acquire); i++)
	{
		...
		(void)peer_id_multihash_view(peers[i], &peer_bytes, &peer_len);
		LP_LOGD("IDENTIFY_PUB", "[async] open_stream_async by peer id (size=%zu)", peer_len);
		LIBP2P_TRACE("idpush", "open_stream_async peer_size=%zu", peer_len);
		(void)libp2p_host_open_stream_async(host, peers[i], LIBP2P_IDENTIFY_PUSH_PROTO_ID,
						    __libp2p__idpush_on_open, ctx);
	}
...
static void push_publish_cb(const libp2p_event_t *evt, void *ud)
{
	...
	LP_LOGD("IDENTIFY_PUB", "[cb] schedule async publisher attempt=%d workers=%d", next_attempt,
		(int)atomic_load(&host->worker_count));
