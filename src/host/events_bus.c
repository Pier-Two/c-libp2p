...
static void event_deep_copy(libp2p_event_t *dst, const libp2p_event_t *src)
{
	...
	*dst = *src; /* start with shallow copy, then duplicate dynamic fields */
	...
	case LIBP2P_EVT_PEER_PROTOCOLS_UPDATED:
		...
		{
			const char **arr =
				(const char **)calloc(src->u.peer_protocols_updated.num_protocols, sizeof(*arr));
			if (!arr)
			{
...
static void enqueue_dispatch_event(struct libp2p_host *host, const libp2p_event_t *evt)
{
	...
	if (host->metrics)
	{
		libp2p_metrics_observe_histogram(host->metrics, "libp2p_dispatch_queue_depth", "{}",
						 (double)host->disp_len);
		if (reported_backpressure)
		{
			uint64_t now_ms = monotonic_ms();
			if (now_ms >= wait_start_ms && wait_start_ms != 0)
			{
				libp2p_metrics_observe_histogram(host->metrics, "libp2p_dispatch_backpressure_wait_ms",
								 "{}", (double)(now_ms - wait_start_ms));
			}
		}
	}
...
static peer_id_t *peer_id__clone(const peer_id_t *in)
{
	...
	if (!in)
		return NULL;
	if (peer_id_clone(in, &copy) != PEER_ID_OK)
		return NULL;
	return copy;
}
...
void libp2p__enqueue_event(struct libp2p_host *host, const libp2p_event_t *evt)
{
	...
	if (host->metrics)
	{
		...
		case LIBP2P_EVT_STREAM_OPENED: {
			const char *pid =
				evt->u.stream_opened.protocol_id ? evt->u.stream_opened.protocol_id : "unknown";
		...
		case LIBP2P_EVT_PROTOCOL_NEGOTIATED: {
			const char *pid = evt->u.protocol_negotiated.protocol_id
						  ? evt->u.protocol_negotiated.protocol_id
						  : "unknown";
			char labels[256];
		...
		case LIBP2P_EVT_OUTGOING_CONNECTION_ERROR: {
			libp2p_metrics_inc_counter(host->metrics, "libp2p_conn_error", "{\"direction\":\"outbound\"}",
						   1.0);
			break;
		}
		case LIBP2P_EVT_INCOMING_CONNECTION_ERROR: {
			libp2p_metrics_inc_counter(host->metrics, "libp2p_conn_error", "{\"direction\":\"inbound\"}",
						   1.0);
			break;
		}
		default:
...
void libp2p_event_free(libp2p_event_t *evt)
{
	...
	case LIBP2P_EVT_DIALING:
		...
		{
			peer_id_free((peer_id_t *)evt->u.dialing.peer);
	...
	case LIBP2P_EVT_INCOMING_CONNECTION_ERROR:
		...
		{
			peer_id_free((peer_id_t *)evt->u.outgoing_conn_error.peer);
	...
	case LIBP2P_EVT_NEW_EXTERNAL_ADDR_OF_PEER:
		...
		{
			peer_id_free((peer_id_t *)evt->u.new_external_addr_of_peer.peer);
	...
	case LIBP2P_EVT_PEER_PROTOCOLS_UPDATED:
		...
		{
			peer_id_free((peer_id_t *)evt->u.peer_protocols_updated.peer);
	...
	case LIBP2P_EVT_CONN_OPENED:
		...
		{
			peer_id_free((peer_id_t *)evt->u.conn_opened.peer);
	...
	case LIBP2P_EVT_CONN_CLOSED:
		...
		{
			peer_id_free((peer_id_t *)evt->u.conn_closed.peer);
	...
	case LIBP2P_EVT_STREAM_OPENED:
		...
		{
			peer_id_free((peer_id_t *)evt->u.stream_opened.peer);
