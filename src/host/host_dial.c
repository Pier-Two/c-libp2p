...
static uint64_t host_dial_quic_trace_begin(const char *remote)
{
	...
	{
		LP_LOGI("HOST_DIAL", "quic_handshake[%llu] begin remote=%s pending=%u", (unsigned long long)id,
			remote ? remote : "(unknown)", pending);
	}
	return id;
}

static void host_dial_quic_trace_end(uint64_t id, const char *remote, const char *phase, int rc, uint64_t elapsed_ms)
{
	uint32_t before = __atomic_fetch_sub(&g_quic_handshake_pending, 1u, __ATOMIC_RELAXED);
	uint32_t pending = before > 0u ? (before - 1u) : 0u;
	if (libp2p_log_is_enabled(LIBP2P_LOG_INFO))
	{
		LP_LOGI("HOST_DIAL", "quic_handshake[%llu] %s remote=%s rc=%d elapsed_ms=%llu pending=%u",
			(unsigned long long)id, phase ? phase : "done", remote ? remote : "(unknown)", rc,
			(unsigned long long)elapsed_ms, pending);
	}
}
...
static void schedule_dial_on_open(libp2p_host_t *host, libp2p_on_stream_open_fn cb, libp2p_stream_t *s, void *ud,
				  int err)
{
	if (!host || !cb)
		return;
...
static void auto_identify_run(libp2p_host_t *host, libp2p_yamux_ctx_t *yctx, const peer_id_t *peer)
{
	...
	libp2p_multiselect_err_t ms =
		libp2p_multiselect_dial_io(io, prop, host->opts.multiselect_handshake_timeout_ms, &accepted);
	...
	ssize_t n = libp2p_lp_recv_io_timeout(
		io, buf, 64 * 1024,
		...
		if (libp2p_identify_message_decode(buf, (size_t)n, &id) == 0 && id)
		{
			if (host->peerstore)
			{
				if (id->public_key && id->public_key_len)
					(void)libp2p_peerstore_set_public_key(host->peerstore, peer, id->public_key,
									      id->public_key_len);
				if (id->num_protocols && id->protocols)
				{
					if (libp2p_peerstore_set_protocols(host->peerstore, peer,
									   (const char *const *)id->protocols,
									   id->num_protocols) == 0)
						libp2p__notify_peer_protocols_updated(
							host, peer, (const char *const *)id->protocols,
							id->num_protocols);
				}
				for (size_t i = 0; i < id->num_listen_addrs; i++)
				{
					...
					if (ma)
					{
						(void)libp2p_peerstore_add_addr(host->peerstore, peer, ma,
										10 * 60 * 1000);
						multiaddr_free(ma);
					}
				}
			...
			if (id->observed_addr && id->observed_addr_len)
			{
				int ma_err = 0;
				multiaddr_t *oma =
					multiaddr_new_from_bytes(id->observed_addr, id->observed_addr_len, &ma_err);
				if (oma)
				{
					int serr = 0;
...
static int do_dial_and_select(libp2p_host_t *host, const char *remote_multiaddr, const char *const proposals[],
			      int dial_timeout_ms, const char **accepted_out, libp2p_stream_t **out_stream,
			      const libp2p_cancel_token_t *cancel)
{
	...
	/* Fast path: reuse existing session to this peer if available. */
	{
		...
		if (p)
		{
			...
			if (len > 0 && len < 128)
			{
				...
				peer_str[len] = '\0';
				if (peer_id_new_from_text(peer_str, &peer) == PEER_ID_OK)
					have_peer = 1;
			}
		}
		...
		if (have_peer)
		{
			...
			for (session_node_t *sess = host->sessions; sess; sess = sess->next)
			{
				if (!sess->mx || !sess->mx->vt || !sess->mx->vt->open_stream)
					continue;
				if (!sess->remote_peer || !peer)
					continue;
				if (peer_id_equal(sess->remote_peer, peer) != 1)
					continue;
				mx = sess->mx;
				...
				if (mx->vt->open_stream(mx, NULL, 0, &s) == LIBP2P_MUXER_OK && s)
				{
					...
					if (io)
					{
						libp2p_multiselect_err_t ms = libp2p_multiselect_dial_io(
							io, proposals, host->opts.multiselect_handshake_timeout_ms,
							&accepted);
						libp2p_io_free(io);
						if (ms == LIBP2P_MULTISELECT_OK && accepted)
						{
							...
							if (peer_copy)
							{
								...
								{
									peer_id_free(peer_copy);
								}
							}
							*out_stream = s;
							peer_id_free(peer);
							LP_LOGD("HOST_DIAL", "reused existing session for %s",
								remote_multiaddr);
							return 0;
						}
					}
				...
				if (mx->vt->open_stream(mx, NULL, 0, &s) == LIBP2P_MUXER_OK && s)
				{
					...
					if (io)
					{
						...
						if (ms == LIBP2P_MULTISELECT_OK && accepted)
						{
							...
							*out_stream = s;
							LP_LOGD("HOST_DIAL",
								"reused existing session for %s (addr match)",
								remote_multiaddr);
							return 0;
						}
					}
	...
	if (is_quic)
	{
		...
		if (libp2p_log_is_enabled(LIBP2P_LOG_TRACE))
		{
			...
			LP_LOGT("HOST_DIAL", "do_dial_and_select: remote=%s proposals_first=%s timeout_ms=%d",
				remote_multiaddr ? remote_multiaddr : "(unknown)", first,
				host->opts.multiselect_handshake_timeout_ms);
		}
		libp2p_multiselect_err_t ms = libp2p_multiselect_dial_io(
			subio, proposals, host->opts.multiselect_handshake_timeout_ms, &accepted);
		int emit_ms_error = 1;
		...
		if (ms != LIBP2P_MULTISELECT_OK)
		{
			...
			if (proposals_only_idpush)
			{
				...
				if (libp2p_log_is_enabled(LIBP2P_LOG_DEBUG))
					LP_LOGD("HOST_DIAL",
						"do_dial_and_select: remote=%s lacks identify-push support",
		 ...
		 * can find it when trying to open new streams to the same peer */
		{
			...
			if (snode)
			{
				snode->is_quic = 1;
				if (stream_peer)
					(void)peer_id_clone(stream_peer, &snode->remote_peer);
		...
		*out_stream = dial_stream;
		...
		libp2p__emit_stream_opened(host, libp2p_stream_protocol_id(dial_stream),
					   libp2p_stream_remote_peer(dial_stream), true);
		libp2p__emit_conn_opened(host, false, libp2p_stream_remote_peer(dial_stream),
					 libp2p_stream_remote_addr(dial_stream));
	...
	quic_fail:
		...
		{
			uint64_t elapsed_ms = (quic_trace_start_ms > 0) ? (now_mono_ms() - quic_trace_start_ms) : 0;
			host_dial_quic_trace_end(quic_trace_id, remote_multiaddr, "fail", rc ? rc : LIBP2P_ERR_INTERNAL,
						 elapsed_ms);
			quic_trace_active = 0;
		}
		if (subio)
	...
	else
	{
		...
		size_t name_len = 0;
		if (libp2p_mplex_stream_open((libp2p_mplex_ctx_t *)mx->ctx, name, name_len, &mps) != LIBP2P_MPLEX_OK ||
		    !mps)
		{
			libp2p_conn_free(secured);
			if (remote_peer)
	...
	libp2p_multiselect_err_t ms =
		libp2p_multiselect_dial_io(subio, proposals, host->opts.multiselect_handshake_timeout_ms, &accepted);
	int emit_ms_error = 1;
	int proposals_only_idpush = 0;
	if (ms == LIBP2P_MULTISELECT_ERR_UNAVAIL && proposals)
	{
	...
	if (ms != LIBP2P_MULTISELECT_OK)
	{
		if (proposals_only_idpush)
		{
			if (libp2p_log_is_enabled(LIBP2P_LOG_DEBUG))
				LP_LOGD("HOST_DIAL",
					"do_dial_and_select: remote=%s lacks identify-push support (yamux/mplex)",
					remote_multiaddr ? remote_multiaddr : "(unknown)");
			libp2p_io_close_free(subio);
			libp2p_conn_free(secured);
			if (remote_peer)
	...
	libp2p_stream_t *ss =
		yctx ? libp2p_stream_from_yamux(host, yctx, sid, accepted, 1, remote_peer)
...
static int do_dial_upgrade_only(libp2p_host_t *host, const char *remote_multiaddr, int dial_timeout_ms,
				libp2p_stream_t **out_stream, const libp2p_cancel_token_t *cancel)
{
	...
	extern libp2p_stream_t *libp2p_stream_from_conn(libp2p_host_t * host, libp2p_conn_t * c,
							const char *protocol_id, int initiator, peer_id_t *remote_peer);
	if (cancel && libp2p_cancel_token_is_canceled(cancel))
	{
		libp2p_conn_free(secured);
		if (remote_peer)
			      ...
			      libp2p_on_stream_open_fn on_open, void *user_data)
{
	libp2p_stream_t *s = NULL;
	int rc =
		libp2p_host_dial_protocol_blocking(host, remote_multiaddr, protocol_id, host->opts.dial_timeout_ms, &s);
	if (on_open)
...
static void *open_stream_async_thread(void *arg)
{
	...
	if (ctx->peer)
	{
		...
		(void)peer_id_text_write(ctx->peer, PEER_ID_TEXT_LEGACY_BASE58, peer_id_str, sizeof(peer_id_str),
					 &peer_id_len);
	}
	...
	if (!host || !host->peerstore || !protocol_id || !on_open)
	{
		LP_LOGW("STREAM_ASYNC", "[open_async] early fail: host=%p peerstore=%p proto=%s on_open=%p",
			(void *)host, host ? (void *)host->peerstore : NULL, protocol_id ? protocol_id : "(null)",
			(void *)on_open);
		if (on_open)
	...
	char peer_str[128] = {0};
	{
		size_t peer_str_len = 0U;
		(void)peer_id_text_write(ctx->peer, PEER_ID_TEXT_LEGACY_BASE58, peer_str, sizeof(peer_str),
					 &peer_str_len);
	}
	...
	{
		LP_LOGI("STREAM_ASYNC", "[open_async] no peerstore addrs for peer=%s (n=%zu), will try session lookup",
			peer_str[0] ? peer_str : "(unknown)", n);
		/* Don't return early - try session lookup below */
	}
	else
	{
		LP_LOGI("STREAM_ASYNC", "[open_async] found %zu peerstore addrs for peer=%s", n,
	...
	for (size_t i = 0; i < n; i++)
	{
		...
		{
			if (ent->remote_addr && ent->protocol_id && strcmp(ent->protocol_id, protocol_id) == 0 &&
			    strcmp(ent->remote_addr, maddr) == 0)
			{
				...
				if (ent->s && libp2p_stream_is_initiator(ent->s))
				{
					s = ent->s;
					rc = 0;
					LP_LOGD("STREAM_ASYNC", "[open_async] reusing initiator stream %p for %s",
						(void *)s, maddr);
					break;
				}
				else
				{
					LP_LOGD("STREAM_ASYNC",
						"[open_async] found stream but we're not initiator, will dial new");
				}
			}
		}
		pthread_mutex_unlock(&host->mtx);
	...
	for (session_node_t *it = host->sessions; it; it = it->next)
		session_count++;
	LP_LOGD("STREAM_ASYNC", "[open_async] searching %zu sessions for existing connection to peer=%s", session_count,
		peer_str);

	for (session_node_t *sess = host->sessions; sess; sess = sess->next)
	{
		LP_LOGD("STREAM_ASYNC",
			"[open_async] checking session=%p mx=%p has_vt=%d has_open_stream=%d remote_peer=%p",
			(void *)sess, (void *)sess->mx, (sess->mx && sess->mx->vt) ? 1 : 0,
			(sess->mx && sess->mx->vt && sess->mx->vt->open_stream) ? 1 : 0, (void *)sess->remote_peer);
		...
		if (!sess->remote_peer || !ctx->peer)
		{
			LP_LOGD("STREAM_ASYNC",
				"[open_async] session skip: no remote peer data sess_peer=%p ctx_peer=%p",
				(void *)sess->remote_peer, (void *)ctx->peer);
			continue;
		}
		if (peer_id_equal(sess->remote_peer, ctx->peer) != 1)
		{
			LP_LOGD("STREAM_ASYNC", "[open_async] session skip: peer mismatch");
			continue;
		}
			...
			if (io)
			{
				...
				if (ms == LIBP2P_MULTISELECT_OK && accepted)
				{
					if (libp2p_stream_set_protocol_id(new_stream, accepted) == 0)
					{
						...
						if (sess->remote_peer)
							(void)peer_id_clone(sess->remote_peer, &remote_copy);
						if (remote_copy)
						{
						...
						s = new_stream;
						rc = 0;
						LP_LOGI("STREAM_ASYNC",
							"[open_async] protocol negotiation succeeded on existing conn "
							"proto=%s",
							accepted);
						free((void *)accepted);
						schedule_dial_on_open(host, on_open, s, ud, rc);
				...
				/* If peer explicitly doesn't support the protocol (NA), don't retry with a new
				 * connection */
				  ...
				  libp2p_on_stream_open_fn on_open, void *user_data)
{
	...
	if (!ctx)
		return LIBP2P_ERR_INTERNAL;
	ctx->host = host;
	if (peer_id_clone(peer, &ctx->peer) != PEER_ID_OK)
	{
		free(ctx);
		return LIBP2P_ERR_INTERNAL;
	}
...
int libp2p_host_dial_selected_blocking(libp2p_host_t *host, const char *remote_multiaddr,
				       const libp2p_proto_selector_t *selector, int timeout_ms, libp2p_stream_t **out)
{
	...
	const char *accepted = NULL;
	libp2p_stream_t *s = NULL;
	rc = do_dial_and_select(host, remote_multiaddr, (const char *const *)proposals, timeout_ms, &accepted, &s,
				NULL);
...
int libp2p_host_dial_selected(libp2p_host_t *host, const char *remote_multiaddr,
			      const libp2p_proto_selector_t *selector, const libp2p_proto_dial_opts_t *opts,
			      libp2p_on_stream_open_fn on_open, void *user_data)
{
		...
		for (stream_entry_t *it = host->active_streams; it; it = it->next)
		{
			...
			case LIBP2P_PROTO_SELECT_LIST:
				if (selector->id_list && selector->id_list_len > 0)
				{
					...
					{
						if (selector->id_list[i] &&
						    strcmp(selector->id_list[i], it->protocol_id) == 0)
						{
							match = 1;
							break;
						}
			...
			case LIBP2P_PROTO_SELECT_PREFIX:
				if (selector->prefix)
					match = (strncmp(it->protocol_id, selector->prefix, strlen(selector->prefix)) ==
						 0);
				break;
			case LIBP2P_PROTO_SELECT_SEMVER:
				if (selector->base_path && selector->semver_range)
				{
					...
					if (strncmp(it->protocol_id, selector->base_path,
						    strlen(selector->base_path)) == 0 &&
	    ...
	    (selector->kind == LIBP2P_PROTO_SELECT_PREFIX || selector->kind == LIBP2P_PROTO_SELECT_SEMVER))
	{
		...
		if (ctx.done && ctx.err == 0 && ctx.ids && ctx.n > 0)
		{
			...
			for (size_t i = 0; i < ctx.n; i++)
			{
				...
				{
					if (!selector->prefix ||
					    strncmp(id, selector->prefix, strlen(selector->prefix)) != 0)
						continue;
					version_triplet_t vtmp = {0};
					...
					{
						if (vtmp.major > accepted_v.major ||
						    (vtmp.major == accepted_v.major && vtmp.minor > accepted_v.minor) ||
						    (vtmp.major == accepted_v.major && vtmp.minor == accepted_v.minor &&
						     vtmp.patch > accepted_v.patch))
						{
							accepted = id;
							accepted_v = vtmp;
						}
...
int libp2p_host_dial_opts(libp2p_host_t *host, const libp2p_dial_opts_t *opts, libp2p_on_stream_open_fn on_open,
			  void *user_data)
{
...
int libp2p_host_dial_opts_cancellable(libp2p_host_t *host, const libp2p_dial_opts_t *opts,
				      struct libp2p_cancel_token *cancel, libp2p_on_stream_open_fn on_open,
				      void *user_data)
{
	if (!host || !opts || opts->struct_size == 0)
		return LIBP2P_ERR_NULL_PTR;
	...
	if (opts->protocol_id && opts->protocol_id[0] != '\0')
	{
		...
		const char *accepted = NULL;
		rc = do_dial_and_select(host, opts->remote_multiaddr, proposals, timeout, &accepted, &s,
					(const libp2p_cancel_token_t *)cancel);
		if (accepted)
			free((void *)accepted);
	}
	...
	{
		rc = do_dial_upgrade_only(host, opts->remote_multiaddr, timeout, &s,
					  (const libp2p_cancel_token_t *)cancel);
	}
	if (on_open)
		schedule_dial_on_open(host, on_open, s, user_data, rc);
