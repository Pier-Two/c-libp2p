...
struct libp2p_host
{
	...
	peer_id_t *local_peer;	 /* derived local Peer ID when have_identity=1 */
...
int libp2p__host_upgrade_outbound(struct libp2p_host *host, struct libp2p_connection *raw, const peer_id_t *remote_hint,
 ...
 * returns a newly allocated uconn carrying secured conn, selected muxer and
 * remote peer (caller assumes ownership and must free the uconn struct). */
int libp2p__host_upgrade_inbound(struct libp2p_host *host, struct libp2p_connection *raw, int allow_mplex,
...
int libp2p__host_upgrade_outbound_quic(struct libp2p_host *host, struct libp2p_connection *session_conn,
				       libp2p_uconn_t **out_uc);

int libp2p__host_upgrade_inbound_quic(struct libp2p_host *host, struct libp2p_connection *session_conn,
				      libp2p_uconn_t **out_uc);
...
void libp2p__emit_incoming_error_with_peer(struct libp2p_host *host, const peer_id_t *peer, libp2p_err_t code,
					   const char *msg);
...
void libp2p__emit_stream_opened(struct libp2p_host *host, const char *protocol_id, const peer_id_t *peer,
				bool initiator);
void libp2p__notify_peer_protocols_updated(struct libp2p_host *host, const peer_id_t *peer,
