...
typedef struct yamux_cb_ctx
{
	...
	peer_id_t *peer_template; /* cloned for subsequent streams */
...
static void detach_listeners(libp2p_host_t *host, listener_node_t ***out_arr, size_t *out_len,
			     listener_node_t **out_head)
{
...
static void yamux_cb_ctx_free(yamux_cb_ctx_t *c)
{
	...
	if (c->first_peer)
		peer_id_free(c->first_peer);
	if (c->peer_template)
		peer_id_free(c->peer_template);
...
static void *aid_thread(void *a)
{
	...
	libp2p_multiselect_err_t ms =
		libp2p_multiselect_dial_io(io, prop, c->host->opts.multiselect_handshake_timeout_ms, &accepted);
	if (ms == LIBP2P_MULTISELECT_OK)
	{
		...
		if (buf)
		{
			ssize_t n = libp2p_lp_recv_io_timeout(io, buf, 64 * 1024,
							      (c->host->opts.handshake_timeout_ms > 0)
								      ? (uint64_t)c->host->opts.handshake_timeout_ms
								      : 0);
			if (n > 0)
			{
				...
				if (libp2p_identify_message_decode(buf, (size_t)n, &id) == 0 && id)
				{
					if (c->host->peerstore)
					{
						if (id->public_key && id->public_key_len)
							(void)libp2p_peerstore_set_public_key(c->host->peerstore,
											      c->peer, id->public_key,
											      id->public_key_len);
						if (id->num_protocols && id->protocols)
						{
							if (libp2p_peerstore_set_protocols(
								    c->host->peerstore, c->peer,
						...
						for (size_t i = 0; i < id->num_listen_addrs; i++)
						{
							...
							if (ma)
							{
								(void)libp2p_peerstore_add_addr(c->host->peerstore,
												c->peer, ma,
												10 * 60 * 1000);
								multiaddr_free(ma);
							}
...
static void *inbound_session_thread(void *arg)
{
	...
	if (remote_peer)
	{
		...
		(void)peer_id_clone(remote_peer, &cbctx->peer_template);
	}
	if (fd >= 0)
...
static void mplex_stream_event_cb(libp2p_mplex_stream_t *stream, libp2p_mplex_event_t ev, void *ud)
{
	...
	if (ev == LIBP2P_MPLEX_STREAM_OPENED)
	{
		...
		pthread_mutex_unlock(&c->mtx);
		if (!peer_for_stream && c->peer_template)
			(void)peer_id_clone(c->peer_template, &peer_for_stream);

		/* Resource manager removed: no admission gating */

		libp2p_stream_t *stream_pub =
			libp2p_stream_from_mplex(c->host, c->mctx, stream, accepted_heap, 0, peer_for_stream);
		free((void *)accepted_heap);
		libp2p_io_free(subio);
		if (!stream_pub)
		{
			if (peer_for_stream)
				peer_id_free(peer_for_stream);
			return;
		}
		...
		libp2p__emit_stream_opened(c->host, libp2p_stream_protocol_id(stream_pub),
					   libp2p_stream_remote_peer(stream_pub), false);
			...
			if (require_ident)
			{
				...
				if (rp && c->host->peerstore)
				{
					...
					if (!identified)
					{
						...
						size_t n = 0;
						if (libp2p_peerstore_get_protocols(c->host->peerstore, rp, &protos,
										   &n) == 0)
						{
							if (n > 0)
								identified = 1;
...
static void *inbound_mplex_session_thread(void *arg)
{
	...
	/* Keep a template copy for subsequent streams */
	cb.peer_template = NULL;
	if (remote_peer)
		(void)peer_id_clone(remote_peer, &cb.peer_template);
	...
	/* Cleanup */
	if (cb.first_peer)
		peer_id_free(cb.first_peer);
	if (cb.peer_template)
		peer_id_free(cb.peer_template);
...
static void *inbound_substream_worker(void *arg)
{
	...
	libp2p__emit_stream_opened(c->host, libp2p_stream_protocol_id(stream), libp2p_stream_remote_peer(stream),
				   false);
	...
	if (!found)
	{
		LP_LOGD("HOST", "dispatch miss for %s; showing registered protocols",
		...
		for (size_t i = 0; i < pending_stream_count; i++)
			if (pending_streams[i])
			{
				...
				if (session_conn_count > 0)
				{
					...
					int owns = 0;
					if (libp2p__stream_get_parent(pending_streams[i], &pconn, NULL, &owns) &&
					    owns && pconn &&
...
static void *listener_accept_thread(void *arg)
{
	...
	for (;;)
	{
		...
		if (rc == LIBP2P_LISTENER_ERR_AGAIN || rc == LIBP2P_LISTENER_ERR_TIMEOUT ||
		...
		if (rc != LIBP2P_LISTENER_OK)
		{
			...
			if (!node->emitted_error && atomic_load_explicit(&host->running, memory_order_acquire) &&
			    node->thread_running)
			{
				libp2p_event_t evt = (libp2p_event_t){0};
		...
		if (host->gater_fn)
		{
			...
			libp2p_gater_decision_t gd =
				host->gater_fn(addr_str ? addr_str : "", remote_peer, host->gater_ud);
			if (addr_str)
				free(addr_str);
		...
		LP_LOGI("HOST", "inbound conn established is_quic=%d has_remote_peer=%d mx=%p", is_quic,
			remote_peer ? 1 : 0, (void *)mx);
		...
		if (is_quic)
		{
			...
			if (snode)
			{
				...
				if (remote_peer)
					(void)peer_id_clone(remote_peer, &snode->remote_peer);
				...
				LP_LOGI("HOST",
					"registered inbound QUIC session node=%p peer=%p mx=%p has_open_stream=%d",
