                                                      ...
                                                      gossipsub_rpc_parsed_t *out)
{
    ...
    for (size_t i = 0; i < raw_count; ++i)
    {
        ...
        if (!sub || !libp2p_gossipsub_RPC_SubOpts_has_topic(sub))
            continue;

        size_t topic_len = libp2p_gossipsub_RPC_SubOpts_get_size_topic(sub);
        const char *topic_raw = libp2p_gossipsub_RPC_SubOpts_get_topic(sub);
        ...
        memcpy(topic, topic_raw, topic_len);
        topic[topic_len] = '\0';

        subs[used].topic = topic;
        subs[used].subscribe = libp2p_gossipsub_RPC_SubOpts_get_subscribe(sub) ? 1 : 0;
        used++;
    }
                                                      ...
                                                      gossipsub_rpc_parsed_t *out)
{
    ...
    for (size_t i = 0; i < raw_count; ++i)
    {
        ...
        if (!ihave || !libp2p_gossipsub_ControlIHave_has_topic(ihave) || !libp2p_gossipsub_ControlIHave_has_message_ids(ihave))
            continue;

        size_t topic_len = libp2p_gossipsub_ControlIHave_get_size_topic(ihave);
        const char *topic_raw = libp2p_gossipsub_ControlIHave_get_topic(ihave);
        if (!topic_raw || topic_len == 0)
            continue;
                                                      ...
                                                      gossipsub_rpc_parsed_t *out)
{
    ...
    for (size_t i = 0; i < raw_count; ++i)
    {
        ...
        size_t topic_len = libp2p_gossipsub_ControlGraft_get_size_topic(graft);
        const char *topic_raw = libp2p_gossipsub_ControlGraft_get_topic(graft);
        if (!topic_raw || topic_len == 0)
            continue;
                                                      ...
                                                      gossipsub_rpc_parsed_t *out)
{
    ...
    for (size_t i = 0; i < raw_count; ++i)
    {
        ...
        const char *topic_raw = libp2p_gossipsub_ControlPrune_get_topic(prune);
        if (!topic_raw || topic_len == 0)
            continue;
