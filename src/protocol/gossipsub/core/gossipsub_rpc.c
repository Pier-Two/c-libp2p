...
libp2p_err_t gossipsub_rpc_encode_control_iwant(uint8_t **ids, const size_t *id_lengths, size_t count,
...
libp2p_err_t gossipsub_rpc_encode_control_prune(const gossipsub_prune_target_t *const *targets, size_t count,
						uint64_t backoff_seconds, gossipsub_rpc_out_t *out)
{
	if (!out)
		return LIBP2P_ERR_NULL_PTR;
	gossipsub_rpc_out_init(out);
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		if (target->px_peers && target->px_len > 0)
		{
			for (size_t j = 0; j < target->px_len; ++j)
			{
				...
				const uint8_t *pid_bytes = NULL;
				size_t pid_len = 0;
				if (!pid || peer_id_multihash_view(pid, &pid_bytes, &pid_len) != PEER_ID_OK)
					continue;
				...
				noise_rc = libp2p_gossipsub_PeerInfo_set_peer_id(info, pid_bytes, pid_len);
				if (noise_rc != NOISE_ERROR_NONE)
					goto cleanup;
			}
		}
...
libp2p_err_t gossipsub_rpc_encode_control_graft(const char *const *topics, size_t count, gossipsub_rpc_out_t *out)
{
	if (!out)
		return LIBP2P_ERR_NULL_PTR;
	gossipsub_rpc_out_init(out);
...
libp2p_err_t gossipsub_rpc_encode_subscription(const char *topic, int subscribe, gossipsub_rpc_out_t *out)
{
	if (!out)
		return LIBP2P_ERR_NULL_PTR;
	gossipsub_rpc_out_init(out);
	...
	if (result == LIBP2P_ERR_OK && out->frame && out->frame_len > 0)
	{
		...
		LP_LOGD(GOSSIPSUB_MODULE, "encode_subscription topic=%s subscribe=%d frame_len=%zu hex=%s", topic,
			subscribe ? 1 : 0, out->frame_len, preview);
	}
...
libp2p_err_t gossipsub_rpc_encode_control_ihave(const char *topic, uint8_t **ids, const size_t *id_lengths,
						size_t count, gossipsub_rpc_out_t *out)
{
	if (!out)
		return LIBP2P_ERR_NULL_PTR;
	gossipsub_rpc_out_init(out);
...
static libp2p_err_t gossipsub_rpc_parse_subscriptions(const libp2p_gossipsub_RPC *rpc, gossipsub_rpc_parsed_t *out)
{
						      ...
						      gossipsub_rpc_parsed_t *out)
{
	...
	gossipsub_rpc_control_iwant_t *shrunk =
		(gossipsub_rpc_control_iwant_t *)realloc(iwants, used * sizeof(*iwants));
	if (shrunk)
		iwants = shrunk;
						      ...
						      gossipsub_rpc_parsed_t *out)
{
	...
	gossipsub_rpc_control_ihave_t *shrunk =
		(gossipsub_rpc_control_ihave_t *)realloc(ihaves, used * sizeof(*ihaves));
	if (shrunk)
		ihaves = shrunk;
						      ...
						      gossipsub_rpc_parsed_t *out)
{
	...
	gossipsub_rpc_control_graft_t *shrunk =
		(gossipsub_rpc_control_graft_t *)realloc(grafts, used * sizeof(*grafts));
	if (shrunk)
		grafts = shrunk;
						      ...
						      gossipsub_rpc_parsed_t *out)
{
	...
	for (size_t i = 0; i < raw_count; ++i)
	{
		...
		prunes[used].backoff = libp2p_gossipsub_ControlPrune_has_backoff(prune)
					       ? libp2p_gossipsub_ControlPrune_get_backoff(prune)
					       : 0;
		...
		if (libp2p_gossipsub_ControlPrune_has_peers(prune))
		{
			...
			if (px_count > 0)
			{
				gossipsub_rpc_px_record_t *records =
					(gossipsub_rpc_px_record_t *)calloc(px_count, sizeof(*records));
				if (!records)
				{
				...
				{
					libp2p_gossipsub_PeerInfo *info =
						libp2p_gossipsub_ControlPrune_get_at_peers(prune, j);
					...
					peer_id_t *dup = NULL;
					if (peer_id_new_from_multihash((const uint8_t *)peer_bytes, peer_len, &dup) !=
					    PEER_ID_OK)
					{
					...
					if (libp2p_gossipsub_PeerInfo_has_signed_peer_record(info))
					{
						const void *record_bytes =
							libp2p_gossipsub_PeerInfo_get_signed_peer_record(info);
						size_t record_len =
							libp2p_gossipsub_PeerInfo_get_size_signed_peer_record(info);
				...
				{
					gossipsub_rpc_px_record_t *shrunk = (gossipsub_rpc_px_record_t *)realloc(
						records, collected * sizeof(*records));
					if (shrunk)
						records = shrunk;
	...
	gossipsub_rpc_control_prune_t *shrunk =
		(gossipsub_rpc_control_prune_t *)realloc(prunes, used * sizeof(*prunes));
	if (shrunk)
		prunes = shrunk;
