					...
					size_t frame_len)
{
		...
		LP_LOGW(GOSSIPSUB_MODULE, "rpc decode failed entry=%p len=%zu rc=%d", (void *)entry, frame_len, rc);
		{
			...
			for (size_t i = 0; i < dbg_n && dbg_off + 3 < sizeof(dbg_hex); ++i)
				dbg_off += (size_t)snprintf(&dbg_hex[dbg_off], sizeof(dbg_hex) - dbg_off,
							    "%02x ", frame ? frame[i] : 0);
			if (dbg_off > 0 && dbg_hex[dbg_off - 1] == ' ')
				dbg_hex[dbg_off - 1] = '\0';
			else
				dbg_hex[dbg_off] = '\0';
			fprintf(stderr,
	...
	if (libp2p_log_is_enabled(LIBP2P_LOG_DEBUG))
	{
		...
		for (size_t i = 0; i < parsed.subscriptions_len; ++i)
		{
			...
			if (sub && sub->topic)
			{
				LP_LOGD(GOSSIPSUB_MODULE, "rpc trace peer=%s subscribe=%d topic=%s topic_id=%s",
					peer_repr, sub->subscribe ? 1 : 0, sub->topic,
		...
		for (size_t i = 0; i < parsed.prune_len; ++i)
		{
			...
			if (p && p->topic)
			{
				LP_LOGD(GOSSIPSUB_MODULE,
					...
					peer_repr, p->topic, p->topic_id ? p->topic_id : "(null)", (uint64_t)p->backoff,
					p->px_count);
			}
		}
	}
	...
	{
		LP_LOGD(GOSSIPSUB_MODULE, "heartbeat trigger after %zu inbound subscriptions",
			parsed.subscriptions_len);
		LP_LOGD(GOSSIPSUB_MODULE, "heartbeat call started=%d", gs ? gs->started : -1);
		...
		if (gs)
		{
			...
			if (started)
			{
				...
				LP_LOGD(GOSSIPSUB_MODULE, "heartbeat inline run now_ms=%" PRIu64, now_ms);
				...
				for (gossipsub_topic_state_t *topic = gs->topics; topic; topic = topic->next)
				{
					...
					LP_LOGD(GOSSIPSUB_MODULE,
						"mesh snapshot topic=%s mesh=%zu subscribers=%zu backoff=%zu "
						"fanout=%zu",
						topic && topic->name ? topic->name : "(null)", mesh, subscribers,
						backoff, fanout);
				}
				pthread_mutex_unlock(&gs->lock);
			}
	...
	if (libp2p_gossipsub_RPC_has_publish(rpc))
	{
		...
		LP_LOGD(GOSSIPSUB_MODULE, "RPC frame contains %zu publish message(s) from peer=%s", publish_count,
			peer_repr);
		for (size_t i = 0; i < publish_count; ++i)
		{
			...
			if (!proto_msg)
				continue;
			libp2p_err_t msg_rc =
				gossipsub_propagation_handle_inbound_publish(gs, entry, proto_msg, frame, frame_len);
					    ...
					    size_t frame_len)
{
	...
	{
		LP_LOGD(GOSSIPSUB_MODULE, "graylisting peer rpc (score=%f threshold=%f)", peer_score,
...
static void gossipsub_init_config(libp2p_gossipsub_config_t *dst, const libp2p_gossipsub_config_t *src)
{
	...
	*dst = *src;
	...
	if (dst->message_cache_gossip == 0 || dst->message_cache_gossip > dst->message_cache_length)
		dst->message_cache_gossip = (dst->message_cache_length > 0) ? dst->message_cache_length
									    : GOSSIPSUB_DEFAULT_MESSAGE_CACHE_GOSSIP;
	if (dst->px_peer_target == 0)
		dst->px_peer_target = 32;
	size_t publish_threshold_size =
		offsetof(libp2p_gossipsub_config_t, publish_threshold) + sizeof(dst->publish_threshold);
	if (!src || src->struct_size < publish_threshold_size)
		dst->publish_threshold = 0.0;
	size_t gossip_threshold_size =
		offsetof(libp2p_gossipsub_config_t, gossip_threshold) + sizeof(dst->gossip_threshold);
	if (!src || src->struct_size < gossip_threshold_size)
		dst->gossip_threshold = 0.0;
	size_t graylist_threshold_size =
		offsetof(libp2p_gossipsub_config_t, graylist_threshold) + sizeof(dst->graylist_threshold);
	...
	size_t opportunistic_peers_size =
		offsetof(libp2p_gossipsub_config_t, opportunistic_graft_peers) + sizeof(dst->opportunistic_graft_peers);
	if (!src || src->struct_size < opportunistic_peers_size || dst->opportunistic_graft_peers <= 0)
		dst->opportunistic_graft_peers = 2;
	size_t score_decay_size =
		offsetof(libp2p_gossipsub_config_t, score_decay_interval_ms) + sizeof(dst->score_decay_interval_ms);
	if (!src || src->struct_size < score_decay_size || dst->score_decay_interval_ms <= 0)
		dst->score_decay_interval_ms = 1000;
	size_t score_cap_size =
		offsetof(libp2p_gossipsub_config_t, score_time_in_mesh_cap) + sizeof(dst->score_time_in_mesh_cap);
	if (!src || src->struct_size < score_cap_size || dst->score_time_in_mesh_cap <= 0.0)
		dst->score_time_in_mesh_cap = 3600.0;
	size_t followup_size =
		offsetof(libp2p_gossipsub_config_t, iwant_followup_time_ms) + sizeof(dst->iwant_followup_time_ms);
	if (!src || src->struct_size < followup_size || dst->iwant_followup_time_ms <= 0)
		dst->iwant_followup_time_ms = 3000;
	size_t ihave_msg_size =
		offsetof(libp2p_gossipsub_config_t, max_ihave_messages) + sizeof(dst->max_ihave_messages);
	if (!src || src->struct_size < ihave_msg_size)
		dst->max_ihave_messages = 10;
	size_t ihave_len_size = offsetof(libp2p_gossipsub_config_t, max_ihave_length) + sizeof(dst->max_ihave_length);
	if (!src || src->struct_size < ihave_len_size)
		dst->max_ihave_length = 5000;
	size_t ihave_penalty_size =
		offsetof(libp2p_gossipsub_config_t, ihave_spam_penalty) + sizeof(dst->ihave_spam_penalty);
	if (!src || src->struct_size < ihave_penalty_size)
		dst->ihave_spam_penalty = 0.1;
	size_t score_cb_size = offsetof(libp2p_gossipsub_config_t, on_score_update) + sizeof(dst->on_score_update);
	if (!src || src->struct_size < score_cb_size)
		dst->on_score_update = NULL;
	size_t score_cb_ud_size =
		offsetof(libp2p_gossipsub_config_t, score_update_user_data) + sizeof(dst->score_update_user_data);
	if (!src || src->struct_size < score_cb_ud_size)
		dst->score_update_user_data = NULL;
	size_t app_weight_size =
		offsetof(libp2p_gossipsub_config_t, app_specific_weight) + sizeof(dst->app_specific_weight);
	if (!src || src->struct_size < app_weight_size)
		dst->app_specific_weight = 1.0;
	size_t ip_weight_size =
		offsetof(libp2p_gossipsub_config_t, ip_colocation_weight) + sizeof(dst->ip_colocation_weight);
	if (!src || src->struct_size < ip_weight_size)
		dst->ip_colocation_weight = 0.0;
	size_t ip_threshold_size =
		offsetof(libp2p_gossipsub_config_t, ip_colocation_threshold) + sizeof(dst->ip_colocation_threshold);
	if (!src || src->struct_size < ip_threshold_size || dst->ip_colocation_threshold <= 0)
		dst->ip_colocation_threshold = 1;
	size_t behaviour_weight_size =
		offsetof(libp2p_gossipsub_config_t, behaviour_penalty_weight) + sizeof(dst->behaviour_penalty_weight);
	if (!src || src->struct_size < behaviour_weight_size)
		dst->behaviour_penalty_weight = -1.0;
	size_t behaviour_decay_size =
		offsetof(libp2p_gossipsub_config_t, behaviour_penalty_decay) + sizeof(dst->behaviour_penalty_decay);
	if (!src || src->struct_size < behaviour_decay_size || dst->behaviour_penalty_decay < 0.0 ||
	    dst->behaviour_penalty_decay > 1.0)
		dst->behaviour_penalty_decay = 0.999;

	size_t fanout_ttl_size = offsetof(libp2p_gossipsub_config_t, fanout_ttl_ms) + sizeof(dst->fanout_ttl_ms);
	if (!src || src->struct_size < fanout_ttl_size || dst->fanout_ttl_ms == 0)
		dst->fanout_ttl_ms = GOSSIPSUB_DEFAULT_FANOUT_TTL_MS;

	size_t protocol_ids_field_size = offsetof(libp2p_gossipsub_config_t, protocol_ids) + sizeof(dst->protocol_ids);
	if (!src || src->struct_size < protocol_ids_field_size)
		dst->protocol_ids = NULL;

	size_t protocol_count_field_size =
		offsetof(libp2p_gossipsub_config_t, protocol_id_count) + sizeof(dst->protocol_id_count);
	if (!src || src->struct_size < protocol_count_field_size)
		dst->protocol_id_count = 0;

	size_t anonymous_mode_field_size =
		offsetof(libp2p_gossipsub_config_t, anonymous_mode) + sizeof(dst->anonymous_mode);
...
static libp2p_err_t gossipsub_validate_config(const libp2p_gossipsub_config_t *cfg)
{
	...
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid mesh degrees (d=%d d_lo=%d d_hi=%d)", cfg->d, cfg->d_lo, cfg->d_hi);
		return LIBP2P_ERR_UNSUPPORTED;
	}

	if (cfg->d_lo > cfg->d || cfg->d > cfg->d_hi)
	{
		LP_LOGE(GOSSIPSUB_MODULE, "mesh degree out of bounds (d_lo=%d d=%d d_hi=%d)", cfg->d_lo, cfg->d,
			cfg->d_hi);
		return LIBP2P_ERR_UNSUPPORTED;
	}
	...
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid score retention (d_score=%d, d_hi=%d)", cfg->d_score, cfg->d_hi);
		return LIBP2P_ERR_UNSUPPORTED;
	}

	if (cfg->d_out < 0 || cfg->d_out >= cfg->d_lo || (cfg->d > 0 && (cfg->d_out * 2) >= cfg->d))
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid outbound quota (d_out=%d, d_lo=%d, d=%d)", cfg->d_out, cfg->d_lo,
			cfg->d);
		return LIBP2P_ERR_UNSUPPORTED;
	}
	...
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid publish threshold (publish=%f gossip=%f)", cfg->publish_threshold,
	...
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid graylist threshold (graylist=%f publish=%f)",
			cfg->graylist_threshold, cfg->publish_threshold);
		return LIBP2P_ERR_UNSUPPORTED;
	}
	...
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid IWANT followup time (followup=%d)", cfg->iwant_followup_time_ms);
		return LIBP2P_ERR_UNSUPPORTED;
	}

	if (cfg->app_specific_weight < 0.0)
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid application weight (weight=%f)", cfg->app_specific_weight);
		return LIBP2P_ERR_UNSUPPORTED;
	}

	if (cfg->ip_colocation_weight > 0.0)
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid IP colocation weight (weight=%f)", cfg->ip_colocation_weight);
		return LIBP2P_ERR_UNSUPPORTED;
	}

	if (cfg->ip_colocation_threshold <= 0)
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid IP colocation threshold (threshold=%d)",
	...
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid behaviour penalty weight (weight=%f)",
	...
	{
		LP_LOGE(GOSSIPSUB_MODULE, "invalid behaviour penalty decay (decay=%f)", cfg->behaviour_penalty_decay);
		return LIBP2P_ERR_UNSUPPORTED;
	}
...
libp2p_err_t libp2p_gossipsub_new(libp2p_host_t *host, const libp2p_gossipsub_config_t *cfg, libp2p_gossipsub_t **out)
{
	...
	*out = NULL;
	...
	size_t seen_capacity =
		gs->cfg.seen_cache_capacity ? gs->cfg.seen_cache_capacity : GOSSIPSUB_DEFAULT_SEEN_CACHE_CAPACITY;
	uint64_t seen_ttl_ms = (gs->cfg.seen_cache_ttl_ms > 0) ? (uint64_t)gs->cfg.seen_cache_ttl_ms
							       : GOSSIPSUB_DEFAULT_SEEN_CACHE_TTL_MS;
	...
	size_t mcache_len =
		gs->cfg.message_cache_length ? gs->cfg.message_cache_length : GOSSIPSUB_DEFAULT_MESSAGE_CACHE_LEN;
	size_t mcache_gossip =
		gs->cfg.message_cache_gossip ? gs->cfg.message_cache_gossip : GOSSIPSUB_DEFAULT_MESSAGE_CACHE_GOSSIP;
	...
	if (cfg && cfg->explicit_peers && cfg->num_explicit_peers)
	{
		for (size_t i = 0; i < cfg->num_explicit_peers; i++)
		{
			...
			size_t addr_count = 0;
			if (ep_struct_size >=
			    offsetof(libp2p_gossipsub_explicit_peer_t, addresses) + sizeof(ep->addresses))
				addr_list = ep->addresses;
			if (ep_struct_size >=
			    offsetof(libp2p_gossipsub_explicit_peer_t, address_count) + sizeof(ep->address_count))
			...
			if (gs->host && addr_list && addr_count)
			{
				for (size_t j = 0; j < addr_count; j++)
				{
					...
					if (!addr_str)
						continue;
					int rc = libp2p_host_add_peer_addr_str(gs->host, dup, addr_str,
									       GOSSIPSUB_EXPLICIT_ADDR_TTL_MS);
					if (rc != LIBP2P_ERR_OK)
						LP_LOGW(GOSSIPSUB_MODULE, "failed to seed explicit peer addr (rc=%d)",
							rc);
				}
			}
		}
...
libp2p_err_t libp2p_gossipsub_start(libp2p_gossipsub_t *gs)
{
	...
	if (gs->cfg.enable_opportunistic_graft && gs->opportunistic_timer_id <= 0 &&
	    gs->cfg.opportunistic_graft_interval_ms > 0)
	...
	for (size_t i = 0; i < gs->num_protocol_defs; i++)
	{
		...
		if (rc != 0)
		{
			LP_LOGE(GOSSIPSUB_MODULE, "failed to register protocol %s (rc=%d)",
				gs->protocol_defs[i].protocol_id, rc);
			libp2p_gossipsub_stop(gs);
			return LIBP2P_ERR_INTERNAL;
		}
	}
	...
	if (need_timer && gs->runtime)
	{
		uint64_t interval_ms =
			(gs->cfg.heartbeat_interval_ms > 0) ? (uint64_t)gs->cfg.heartbeat_interval_ms : 1000ULL;
	...
	{
		uint64_t interval_ms = (gs->cfg.opportunistic_graft_interval_ms > 0)
					       ? (uint64_t)gs->cfg.opportunistic_graft_interval_ms
					       : 60000ULL;
...
libp2p_err_t libp2p_gossipsub_update_topic(libp2p_gossipsub_t *gs, const libp2p_gossipsub_topic_config_t *topic_cfg)
{
	if (!gs || !topic_cfg)
		return LIBP2P_ERR_NULL_PTR;
	...
	size_t score_field_size =
		offsetof(libp2p_gossipsub_topic_config_t, score_params) + sizeof(topic_cfg->score_params);
	...
	size_t publish_field_size =
		offsetof(libp2p_gossipsub_topic_config_t, publish_threshold) + sizeof(topic_cfg->publish_threshold);
...
libp2p_err_t libp2p_gossipsub_peering_remove(libp2p_gossipsub_t *gs, const peer_id_t *peer)
{
	...
	while (*pp)
	{
		if (peer_id_equal((*pp)->peer, peer) == 1)
		{
...
libp2p_err_t libp2p_gossipsub_set_peer_application_score(libp2p_gossipsub_t *gs, const peer_id_t *peer, double score)
{
	if (!gs || !peer)
		return LIBP2P_ERR_NULL_PTR;
...
libp2p_err_t libp2p_gossipsub_set_peer_behaviour_penalty(libp2p_gossipsub_t *gs, const peer_id_t *peer, double penalty)
{
	if (!gs || !peer)
		return LIBP2P_ERR_NULL_PTR;
...
libp2p_err_t libp2p_gossipsub_add_peer_behaviour_penalty(libp2p_gossipsub_t *gs, const peer_id_t *peer, double delta)
{
	if (!gs || !peer)
		return LIBP2P_ERR_NULL_PTR;
	if (delta == 0.0)
...
int libp2p_gossipsub__peer_has_subscription(libp2p_gossipsub_t *gs, const peer_id_t *peer, const char *topic_name)
{
...
int libp2p_gossipsub__topic_mesh_contains(libp2p_gossipsub_t *gs, const char *topic_name, const peer_id_t *peer,
					  int *out_outbound, uint64_t *out_last_heartbeat_ms)
{
...
libp2p_err_t libp2p_gossipsub__topic_mesh_add_peer(libp2p_gossipsub_t *gs, const char *topic_name,
						   const peer_id_t *peer, int outbound_hint)
{
	if (!gs || !topic_name || !peer)
		return LIBP2P_ERR_NULL_PTR;
	...
	if (!topic)
	{
		libp2p_gossipsub_topic_config_t cfg = {
			...
			.descriptor = {.struct_size = sizeof(cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
...
libp2p_err_t libp2p_gossipsub__topic_mesh_remove_peer(libp2p_gossipsub_t *gs, const char *topic_name,
...
int libp2p_gossipsub__topic_fanout_contains(libp2p_gossipsub_t *gs, const char *topic_name, const peer_id_t *peer,
					    int *out_outbound, uint64_t *out_last_publish_ms)
{
	if (!gs || !topic_name || !peer)
		return 0;
	pthread_mutex_lock(&gs->lock);
...
libp2p_err_t libp2p_gossipsub__topic_fanout_add_peer(libp2p_gossipsub_t *gs, const char *topic_name,
						     const peer_id_t *peer, int outbound_hint, uint64_t ttl_ms)
{
	if (!gs || !topic_name || !peer)
		return LIBP2P_ERR_NULL_PTR;
	...
	{
		uint64_t cfg_ttl =
			(gs->cfg.fanout_ttl_ms > 0) ? gs->cfg.fanout_ttl_ms : GOSSIPSUB_DEFAULT_FANOUT_TTL_MS;
...
libp2p_err_t libp2p_gossipsub__topic_fanout_remove_peer(libp2p_gossipsub_t *gs, const char *topic_name,
							const peer_id_t *peer)
{
	if (!gs || !topic_name || !peer)
		return LIBP2P_ERR_NULL_PTR;
...
int libp2p_gossipsub__message_in_cache(libp2p_gossipsub_t *gs, const uint8_t *message_id, size_t message_id_len)
{
...
libp2p_err_t libp2p_gossipsub__peer_set_score(libp2p_gossipsub_t *gs, const peer_id_t *peer, double score)
{
	if (!gs || !peer)
		return LIBP2P_ERR_NULL_PTR;
...
libp2p_err_t libp2p_gossipsub__peer_clear_score_override(libp2p_gossipsub_t *gs, const peer_id_t *peer)
{
	if (!gs || !peer)
		return LIBP2P_ERR_NULL_PTR;
...
double libp2p_gossipsub__peer_get_score(libp2p_gossipsub_t *gs, const peer_id_t *peer, int *out_override)
{
	if (out_override)
		*out_override = 0;
	if (!gs || !peer)
...
libp2p_err_t libp2p_gossipsub__peer_set_remote_ip(libp2p_gossipsub_t *gs, const peer_id_t *peer, const char *ip)
{
	if (!gs || !peer)
		return LIBP2P_ERR_NULL_PTR;
...
libp2p_err_t libp2p_gossipsub__topic_set_publish_threshold(libp2p_gossipsub_t *gs, const char *topic_name,
							   double threshold)
{
	if (!gs || !topic_name)
		return LIBP2P_ERR_NULL_PTR;
...
int libp2p_gossipsub__topic_backoff_contains(libp2p_gossipsub_t *gs, const char *topic_name, const peer_id_t *peer)
{
	if (!gs || !topic_name || !peer)
		return 0;
