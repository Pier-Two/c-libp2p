...
void gossipsub_on_stream_open(struct libp2p_stream *s, void *user_data)
{
	...
	if (remote)
	{
		gossipsub_write_peer_string(remote, peer_buf, sizeof(peer_buf));
		...
		if (entry)
		{
			...
			{
				LP_LOGD(GOSSIPSUB_MODULE, "stream_open ignoring stale callback peer=%s stream=%p",
					peer_buf, (void *)s);
				pthread_mutex_unlock(&gs->lock);
				return;
			}
	...
	pthread_mutex_unlock(&gs->lock);
	LP_LOGD(GOSSIPSUB_MODULE, "gossipsub stream opened peer=%s protocol=%s initiator=%d",
		peer_buf[0] ? peer_buf : "(unknown)", protocol_id ? protocol_id : "(null)", is_initiator);
}
...
void gossipsub_on_stream_data(struct libp2p_stream *s, const uint8_t *data, size_t len, void *user_data)
{
	...
	gossipsub_decoder_cb_ctx_t ctx = {.gs = gs, .peer = entry};
	...
	if (libp2p_log_is_enabled(LIBP2P_LOG_DEBUG))
	{
		...
		char peer_buf[128] = {0};
		if (entry->peer)
			gossipsub_write_peer_string(entry->peer, peer_buf, sizeof(peer_buf));

		LP_LOGD(GOSSIPSUB_MODULE, "stream_data peer=%s len=%zu hex=%s%s", peer_buf[0] ? peer_buf : "(unknown)",
			len, preview, len > 64 ? "..." : "");
	}
...
static void gossipsub_outbound_dial_cb(libp2p_stream_t *s, void *user_data, int err)
{
	...
	{
		LP_LOGD(GOSSIPSUB_MODULE, "outbound gossipsub stream opened peer=%s",
	...
	LP_LOGD(GOSSIPSUB_MODULE, "outbound gossipsub stream failed peer=%s err=%d protocol_index=%zu",
		peer_buf[0] ? peer_buf : "(unknown)", err, ctx->protocol_index);
	...
	if (next_protocol)
	{
		int rc = libp2p_host_open_stream_async(gs->host, ctx->peer, next_protocol, gossipsub_outbound_dial_cb,
						       ctx);
		if (rc == LIBP2P_ERR_OK)
			return;
	...
	LP_LOGD(GOSSIPSUB_MODULE, "outbound gossipsub stream exhausted all protocols peer=%s",
		peer_buf[0] ? peer_buf : "(unknown)");
	if (ctx->peer)
		peer_id_free(ctx->peer);
	free(ctx);
...
static void gossipsub_try_open_outbound_stream(libp2p_gossipsub_t *gs, const peer_id_t *peer)
{
	char peer_buf[128] = {0};
	if (peer)
		gossipsub_write_peer_string(peer, peer_buf, sizeof(peer_buf));

	if (!gs || !peer || !gs->host || !gs->started)
	{
		LP_LOGW(GOSSIPSUB_MODULE, "try_open_outbound_stream early return: gs=%p peer=%p host=%p", (void *)gs,
			(void *)peer, gs ? (void *)gs->host : NULL);
		return;
	}
	...
	LP_LOGI(GOSSIPSUB_MODULE, "opening outbound gossipsub stream peer=%s protocol=%s",
		peer_buf[0] ? peer_buf : "(unknown)", protocol);
...
void gossipsub_host_events_on_host_event(const libp2p_event_t *evt, void *user_data)
{
	...
	case LIBP2P_EVT_CONN_OPENED: {
		...
		if (evt->u.conn_opened.peer)
			gossipsub_write_peer_string(evt->u.conn_opened.peer, peer_buf, sizeof(peer_buf));
