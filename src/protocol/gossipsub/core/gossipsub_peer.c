...
static gossipsub_sendq_item_t *gossipsub_sendq_item_new(const uint8_t *payload, size_t payload_len)
{
	...
	size_t header_len = 0;
	if (unsigned_varint_encode((uint64_t)payload_len, item->header, sizeof(item->header), &header_len) !=
	    UNSIGNED_VARINT_OK)
	{
		free(item->payload);
		free(item);
...
void gossipsub_peer_explicit_cancel_timer_locked(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry)
{
...
void gossipsub_peer_explicit_schedule_dial_locked(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry,
						  uint64_t delay_ms)
{
...
gossipsub_peer_entry_t *gossipsub_peer_find_or_add_locked(libp2p_gossipsub_t *gs, const peer_id_t *peer)
{
	...
	LP_LOGT(GOSSIPSUB_MODULE, "peer entry initialized entry=%p decoder_max=%zu", (void *)entry,
	...
	if (entry->stream && (!s || entry->stream == s))
	{
		...
		if (entry->sendq_head)
		{
			...
			for (gossipsub_sendq_item_t *it = entry->sendq_head; it; it = it->next)
				dropped++;
			LP_LOGW(GOSSIPSUB_MODULE, "peer_detach_stream dropping %zu queued messages peer=%s", dropped,
				peer_repr);
...
void gossipsub_peer_attach_stream_locked(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry, libp2p_stream_t *s)
{
	...
	if (old_stream && old_stream != s)
	{
		...
		LP_LOGI(GOSSIPSUB_MODULE,
			...
			peer_repr, (void *)old_stream, old_initiator, (void *)s, new_initiator);
		...
		{
			LP_LOGI(GOSSIPSUB_MODULE,
				"peer_attach_stream peer=%s KEEPING outbound stream for writing, setting user_data on "
				"inbound for receiving",
				peer_repr);
	...
	LP_LOGD(GOSSIPSUB_MODULE,
		"peer_attach_stream peer=%s stream=%p protocol=%s initiator=%d entry=%p decoder_max=%zu", peer_repr,
		(void *)s, proto ? proto : "(null)", is_initiator, (void *)entry, entry->decoder.max_frame_len);
	if (entry->explicit_peering)
	...
	if (s)
	{
		...
		LP_LOGD(GOSSIPSUB_MODULE, "peer_attach_stream peer=%s will_send_subscriptions=%zu", peer_repr,
			topic_count);
		...
		for (gossipsub_sendq_item_t *it = entry->sendq_head; it; it = it->next)
			queued++;
		LP_LOGD(GOSSIPSUB_MODULE,
			"peer_attach_stream peer=%s after_send_subscriptions sendq_size=%zu stream=%p", peer_repr,
			queued, (void *)entry->stream);
		...
		{
			LP_LOGD(GOSSIPSUB_MODULE, "peer_attach_stream peer=%s scheduling_flush_for_pending queued=%zu",
				peer_repr, queued);
...
libp2p_err_t gossipsub_peer_topic_subscribe(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry, char **topic_ptr)
{
...
void gossipsub_peer_topic_unsubscribe(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry, const char *topic_name)
{
...
libp2p_err_t gossipsub_peer_sendq_pop_locked(gossipsub_peer_entry_t *entry, uint8_t **out_buf, size_t *out_len)
{
...
static libp2p_err_t gossipsub_peer_flush_locked(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry)
{
	...
	if (!entry->stream)
	{
		...
		{
			LP_LOGD(GOSSIPSUB_MODULE,
				"flush NO_STREAM peer=%s entry=%p (explicit, will retry after reconnect)", peer_repr,
				(void *)entry);
		...
		if (dropped > 0)
		{
			LP_LOGW(GOSSIPSUB_MODULE, "flush NO_STREAM peer=%s entry=%p dropping %zu queued messages",
				peer_repr, (void *)entry, dropped);
			gossipsub_peer_sendq_clear(entry);
		}
		return LIBP2P_ERR_OK;
	}
	...
	{
		LP_LOGW(GOSSIPSUB_MODULE, "flush RETAIN_FAIL peer=%s stream=%p", peer_repr, (void *)stream);
	...
	for (gossipsub_sendq_item_t *it = entry->sendq_head; it; it = it->next)
		queued++;
	LP_LOGT(GOSSIPSUB_MODULE, "flush START peer=%s stream=%p queued=%zu", peer_repr, (void *)stream, queued);
	...
	while (entry->stream == stream && entry->sendq_head)
	{
		...
		if (item && item->payload && item->payload_len && item->payload_sent == 0 && item->header_sent == 0)
		{
			...
			if (log_level != (libp2p_log_level_t)-1 && log_level != LIBP2P_LOG_ERROR)
			{
				...
				if (combined_len < preview_cap)
				{
					...
					size_t payload_copy =
						item->payload_len < remaining ? item->payload_len : remaining;
					if (payload_copy)
					{
				...
				LP_LOGF(log_level, GOSSIPSUB_MODULE,
					"flush FRAME peer=%s header_len=%zu payload_len=%zu preview=%s", peer_repr,
					item->header_len, item->payload_len, preview);
			}
		}

		while (item && item->header_sent < item->header_len)
		{
			ssize_t n = libp2p_stream_write(stream, item->header + item->header_sent,
							item->header_len - item->header_sent);
			LP_LOGD(GOSSIPSUB_MODULE, "flush WRITE_HEADER peer=%s wrote=%zd remaining=%zu", peer_repr, n,
				item->header_len - item->header_sent);
			if (n > 0)
			{
			...
			{
				LP_LOGD(GOSSIPSUB_MODULE,
					"flush BACKPRESSURE peer=%s frames_written=%zu bytes_written=%zu", peer_repr,
					frames_written, total_bytes_written);
			...
			LP_LOGW(GOSSIPSUB_MODULE, "flush WRITE_HEADER_FAIL peer=%s rc=%zd header_remaining=%zu",
				peer_repr, n, item ? (item->header_len - item->header_sent) : 0);
		...
		{
			ssize_t n = libp2p_stream_write(stream, item->payload + item->payload_sent,
							item->payload_len - item->payload_sent);
			LP_LOGD(GOSSIPSUB_MODULE, "flush WRITE_PAYLOAD peer=%s wrote=%zd remaining=%zu", peer_repr, n,
				item->payload_len - item->payload_sent);
			if (n > 0)
			{
			...
			LP_LOGW(GOSSIPSUB_MODULE, "flush WRITE_PAYLOAD_FAIL peer=%s rc=%zd", peer_repr, n);
	...
	LP_LOGT(GOSSIPSUB_MODULE, "flush COMPLETE peer=%s frames_written=%zu bytes_written=%zu stream=%p", peer_repr,
		frames_written, total_bytes_written, (void *)stream);
...
libp2p_err_t gossipsub_peer_enqueue_frame_locked(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry,
						 const uint8_t *frame, size_t frame_len)
{
	...
	libp2p_log_level_t log_level =
		(libp2p_log_is_enabled(LIBP2P_LOG_TRACE)
			 ? LIBP2P_LOG_TRACE
			 : (libp2p_log_is_enabled(LIBP2P_LOG_DEBUG) ? LIBP2P_LOG_DEBUG : (libp2p_log_level_t)-1));
	if (frame_len && log_level != (libp2p_log_level_t)-1 && log_level != LIBP2P_LOG_ERROR)
	{
		...
		LP_LOGF(log_level, GOSSIPSUB_MODULE,
			"enqueue frame peer=%s frame_len=%zu topics=%zu first_topic=%s first_data_len=%zu preview=%s "
			"decode_rc=%d",
			peer_repr, frame_len, publish_count, topic_name ? topic_name : "(null)", data_len, preview,
			decode_rc);
...
libp2p_err_t gossipsub_peer_send_subscription_locked(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry,
						     const char *topic, int subscribe)
{
	if (!gs || !entry || !topic)
		return LIBP2P_ERR_NULL_PTR;
	...
	{
		LP_LOGW(GOSSIPSUB_MODULE, "send_subscription ENCODE_FAILED peer=%s topic=%s rc=%d", peer_repr, topic,
			enc_rc);
		return enc_rc;
	}
	...
	LP_LOGD(GOSSIPSUB_MODULE,
		"send_subscription peer=%s topic=%s subscribe=%d frame_len=%zu enqueue_rc=%d stream=%p", peer_repr,
		topic, subscribe ? 1 : 0, frame.frame_len, send_rc, (void *)entry->stream);
...
static void gossipsub_peer_send_current_subscriptions_locked(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry)
{
	...
	LP_LOGD(GOSSIPSUB_MODULE, "send_current_subscriptions START peer=%s has_stream=%d", peer_repr, has_stream);
	...
	for (gossipsub_topic_state_t *topic = gs->topics; topic; topic = topic->next)
	{
		if (!topic->subscribed || !topic->name)
			continue;
		LP_LOGD(GOSSIPSUB_MODULE, "send_current_subscriptions peer=%s topic=%s", peer_repr, topic->name);
		libp2p_err_t rc = gossipsub_peer_send_subscription_locked(gs, entry, topic->name, 1);
		if (rc == LIBP2P_ERR_OK)
			sent++;
	}

	LP_LOGD(GOSSIPSUB_MODULE, "send_current_subscriptions END peer=%s sent=%zu", peer_repr, sent);
}
