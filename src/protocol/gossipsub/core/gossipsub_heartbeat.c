...
static gossipsub_peer_entry_t **gossipsub_heartbeat_collect_candidates(libp2p_gossipsub_t *gs,
								       gossipsub_topic_state_t *topic, uint64_t now_ms,
								       int require_outbound, int ignore_backoff,
								       size_t *out_count)
{
	if (out_count)
		*out_count = 0;
	...
	for (gossipsub_peer_entry_t *entry = gs->peers; entry; entry = entry->next)
	{
		...
		if (count == capacity)
		{
			...
			gossipsub_peer_entry_t **new_list =
				(gossipsub_peer_entry_t **)realloc(list, new_cap * sizeof(*new_list));
			if (!new_list)
			{
				free(list);
				return NULL;
		...
		LP_LOGD(GOSSIPSUB_MODULE, "heartbeat candidate peer=%s topic=%s outbound=%d",
			peer_repr ? peer_repr : "-", topic->name ? topic->name : "(null)", outbound);
	}

	if (count == 0)
	{
		LP_LOGD(GOSSIPSUB_MODULE, "heartbeat candidates none topic=%s require_outbound=%d",
			topic && topic->name ? topic->name : "(null)", require_outbound ? 1 : 0);
		free(list);
		return NULL;
	}
...
static void gossipsub_heartbeat_try_graft_from_candidates(libp2p_gossipsub_t *gs, gossipsub_topic_state_t *topic,
							  gossipsub_peer_entry_t *const *candidates,
							  size_t candidate_count, size_t *needed,
							  size_t *outbound_needed, size_t *current_outbound,
							  uint64_t now_ms, int allow_inbound_fallback,
							  int ignore_backoff)
{
	...
	for (size_t i = 0; i < candidate_count && *needed > 0; ++i)
	{
		...
		if (send_rc == LIBP2P_ERR_OK)
		{
			char peer_buf[128];
			const char *peer_repr = gossipsub_peer_to_string(entry->peer, peer_buf, sizeof(peer_buf));
			LP_LOGD(GOSSIPSUB_MODULE, "send_graft peer=%s topic=%s", peer_repr,
				topic && topic->name ? topic->name : "(null)");
			if (libp2p_log_is_enabled(LIBP2P_LOG_DEBUG))
			{
				LP_LOGD(GOSSIPSUB_MODULE,
					"heartbeat graft peer=%s topic=%s outbound=%d score=%.3f remaining=%zu "
					"outbound_needed=%zu",
					peer_repr, topic && topic->name ? topic->name : "(null)", outbound,
					entry ? entry->score : 0.0, *needed, outbound_needed ? *outbound_needed : 0);
			}
		}
		gossipsub_rpc_out_clear(&frame);
		if (send_rc != LIBP2P_ERR_OK)
...
static void gossipsub_heartbeat_graft_locked(libp2p_gossipsub_t *gs, gossipsub_topic_state_t *topic, size_t needed,
					     uint64_t now_ms)
{
	...
	LP_LOGD(GOSSIPSUB_MODULE, "heartbeat graft topic=%s mesh_size=%zu needed=%zu outbound_needed=%zu d_out=%zu",
		topic->name ? topic->name : "(null)", topic->mesh_size, remaining, outbound_needed, d_out);

	if (outbound_needed > 0)
	{
		size_t candidate_count = 0;
		gossipsub_peer_entry_t **candidates =
			gossipsub_heartbeat_collect_candidates(gs, topic, now_ms, 1, 0, &candidate_count);
		if (candidates)
		{
			LP_LOGD(GOSSIPSUB_MODULE,
				"heartbeat graft outbound pass topic=%s candidates=%zu needed=%zu outbound_needed=%zu "
				"current_outbound=%zu",
				topic->name ? topic->name : "(null)", candidate_count, remaining, outbound_needed,
				current_outbound);
			gossipsub_heartbeat_try_graft_from_candidates(gs, topic, candidates, candidate_count,
								      &remaining, &outbound_needed, &current_outbound,
								      now_ms, 0, 0);
			free(candidates);
		}
	}
	...
	if (remaining > 0)
	{
		size_t candidate_count = 0;
		gossipsub_peer_entry_t **candidates =
			gossipsub_heartbeat_collect_candidates(gs, topic, now_ms, 0, 0, &candidate_count);
		if (candidates)
		{
			LP_LOGD(GOSSIPSUB_MODULE,
				"heartbeat graft any-pass topic=%s candidates=%zu needed=%zu outbound_needed=%zu "
				"current_outbound=%zu allow_inbound_fallback=%d",
				topic->name ? topic->name : "(null)", candidate_count, remaining, outbound_needed,
				current_outbound, outbound_needed > 0 ? 1 : 0);
			gossipsub_heartbeat_try_graft_from_candidates(gs, topic, candidates, candidate_count,
								      &remaining, &outbound_needed, &current_outbound,
								      now_ms, (outbound_needed > 0) ? 1 : 0, 0);
			free(candidates);
		}
	}
}

static void gossipsub_heartbeat_prune_locked(libp2p_gossipsub_t *gs, gossipsub_topic_state_t *topic, size_t target_mesh,
					     uint64_t now_ms)
{
	...
	if (candidate_capacity > 0)
	{
		...
		candidate_ptrs =
			(gossipsub_mesh_prune_candidate_t **)calloc(candidate_capacity, sizeof(*candidate_ptrs));
	}
	...
	for (size_t i = 0; i < used && prune_needed > 0; ++i)
	{
		...
		gossipsub_rpc_out_t frame;
		gossipsub_rpc_out_init(&frame);
		libp2p_err_t enc_rc = gossipsub_rpc_encode_control_prune(targets, 1, backoff_seconds, &frame);

		char peer_buf[128];
		const char *peer_repr = gossipsub_peer_to_string(cand->peer, peer_buf, sizeof(peer_buf));
		LP_LOGD(GOSSIPSUB_MODULE,
			"heartbeat prune peer=%s topic=%s outbound=%d score=%.3f backoff_ms=%" PRIu64 " px_len=%zu",
			peer_repr, topic->name ? topic->name : "(null)", cand->outbound, entry ? entry->score : 0.0,
			backoff_ms, px_len);
...
void gossipsub_opportunistic_tick(libp2p_gossipsub_t *gs, uint64_t now_ms)
{
	...
	for (gossipsub_topic_state_t *topic = gs->topics; topic; topic = topic->next)
	{
		...
		for (gossipsub_mesh_member_t *member = topic->mesh; member && score_count < mesh_capacity;
		     member = member->next)
		{
		...
		size_t candidate_count = 0;
		gossipsub_peer_entry_t **candidates =
			gossipsub_heartbeat_collect_candidates(gs, topic, now_ms, 0, 1, &candidate_count);
		if (!candidates)
			continue;
		...
		gossipsub_heartbeat_try_graft_from_candidates(gs, topic, candidates, filtered, &needed,
							      &outbound_needed, &current_outbound, now_ms, 1, 1);
		...
		if (grafted > 0)
		{
			...
			LP_LOGD(GOSSIPSUB_MODULE,
				"opportunistic grafted %zu peers on topic %s (median=%.3f threshold=%.3f)", grafted,
				topic->name ? topic->name : "<unnamed>", median, threshold);
		}
...
void gossipsub_heartbeat_tick(libp2p_gossipsub_t *gs, uint64_t now_ms)
{
	...
	for (gossipsub_topic_state_t *topic = gs->topics; topic; topic = topic->next)
	{
		LP_LOGD(GOSSIPSUB_MODULE, "heartbeat topic=%s mesh_size=%zu d_lo=%zu d=%zu d_hi=%zu",
			topic && topic->name ? topic->name : "(null)", topic ? topic->mesh_size : 0,
			(size_t)((gs->cfg.d_lo > 0) ? gs->cfg.d_lo : 0), (size_t)((gs->cfg.d > 0) ? gs->cfg.d : 0),
			(size_t)((gs->cfg.d_hi > 0) ? gs->cfg.d_hi : 0));
...
void gossipsub_heartbeat_run(libp2p_gossipsub_t *gs)
{
	...
	uint64_t now_ms = gossipsub_now_ms();
	LP_LOGD(GOSSIPSUB_MODULE, "heartbeat run start now_ms=%" PRIu64 " topics=%p", now_ms, (void *)gs->topics);
