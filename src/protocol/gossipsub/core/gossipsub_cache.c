...
static void gossipsub_message_cache_clear_window(gossipsub_message_cache_t *cache, gossipsub_cache_entry_t *head)
{
	while (head)
	{
...
static void gossipsub_seen_cache_gc_entry(gossipsub_seen_cache_t *cache, gossipsub_seen_entry_t *entry, uint64_t now_ms)
{
...
{
	return entry &&
	       (entry->state == GOSSIPSUB_CACHE_ENTRY_PENDING || entry->state == GOSSIPSUB_CACHE_ENTRY_FINALIZING);
}
