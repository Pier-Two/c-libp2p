...
static double gossipsub_topic_publish_threshold(const libp2p_gossipsub_t *gs, const gossipsub_topic_state_t *topic)
{
...
static libp2p_err_t gossipsub_px_parse_peer_record_payload(const peer_id_t *expected_peer, const uint8_t *payload,
							   size_t payload_len, multiaddr_t ***out_addrs,
							   size_t *out_count)
{
	...
	*out_count = 0;
	...
	while (!noise_protobuf_read_at_end_element(&pb, end_posn))
	{
		...
		case 1: {
			...
			{
				peer_id_t *tmp = NULL;
				if (peer_id_new_from_multihash((const uint8_t *)peer_buf, peer_sz, &tmp) == PEER_ID_OK)
				{
		...
		case 3: {
			...
			while (!noise_protobuf_read_at_end_element(&pb, addr_end))
			{
				...
				if (addr_tag == 1)
				{
					...
					{
						int ma_err = 0;
						multiaddr_t *ma = multiaddr_new_from_bytes((const uint8_t *)addr_buf,
											   addr_len, &ma_err);
						...
						if (addr_count == addr_cap)
						{
							...
							multiaddr_t **new_list = (multiaddr_t **)realloc(
								addr_list, new_cap * sizeof(*new_list));
							if (!new_list)
							{
								multiaddr_free(ma);
								continue;
...
static libp2p_err_t gossipsub_px_build_unsigned(const uint8_t *payload_type, size_t payload_type_len,
						...
						size_t *out_len)
{
	...
	static const uint8_t *fields[] = {(const uint8_t *)GOSSIPSUB_PX_ENVELOPE_DOMAIN, NULL, NULL};
	...
	{
		size_t len_sz = 0;
		if (unsigned_varint_encode((uint64_t)lengths[i], varint_buf, sizeof(varint_buf), &len_sz) !=
		    UNSIGNED_VARINT_OK)
			return LIBP2P_ERR_INTERNAL;
...
static libp2p_err_t gossipsub_px_verify_signed_record(const peer_id_t *expected_peer, const uint8_t *public_key_pb,
						      size_t public_key_pb_len, const uint8_t *payload_type,
						      size_t payload_type_len, const uint8_t *payload,
						      size_t payload_len, const uint8_t *signature,
						      size_t signature_len)
{
	if (!public_key_pb || public_key_pb_len == 0 || !payload_type || payload_type_len == 0 || !payload ||
	    payload_len == 0 || !signature || signature_len == 0)
		return LIBP2P_ERR_NULL_PTR;
	...
	if (key_type != PEER_ID_KEY_ED25519 || raw_key_len != 32)
		return LIBP2P_ERR_UNSUPPORTED;

	if (expected_peer)
	{
		peer_id_t *derived = NULL;
		peer_id_error_t pid_rc = peer_id_new_from_public_key_pb(public_key_pb, public_key_pb_len, &derived);
		if (pid_rc != PEER_ID_OK)
			return LIBP2P_ERR_UNSUPPORTED;
	...
	libp2p_err_t build_rc = gossipsub_px_build_unsigned(payload_type, payload_type_len, payload, payload_len,
...
static libp2p_err_t gossipsub_px_parse_signed_peer_record(const peer_id_t *expected_peer, const uint8_t *signed_record,
							  size_t signed_record_len, multiaddr_t ***out_addrs,
							  size_t *out_count)
{
	...
	*out_count = 0;
	...
	if (!payload || payload_len == 0 || !payload_type || payload_type_len == 0 || !public_key ||
	    public_key_len == 0 || !signature || signature_len == 0)
	{
		if (payload_type)
			noise_protobuf_free_memory(payload_type, payload_type_len);
		if (payload)
	...
	libp2p_err_t verify_rc =
		gossipsub_px_verify_signed_record(expected_peer, public_key, public_key_len, payload_type,
						  payload_type_len, payload, payload_len, signature, signature_len);
	...
	multiaddr_t **addr_list = NULL;
	size_t addr_count = 0;
	libp2p_err_t rc = gossipsub_px_parse_peer_record_payload(expected_peer, (const uint8_t *)payload, payload_len,
...
static void gossipsub_propagation_ingest_px_record(libp2p_gossipsub_t *gs, const peer_id_t *peer, const uint8_t *record,
						   size_t record_len)
{
...
static void gossipsub_px_schedule_attempt(libp2p_gossipsub_t *gs, gossipsub_px_dial_ctx_t *ctx)
{
	...
	LP_LOGT(GOSSIPSUB_MODULE, "PX dial schedule start peer=%s index=%zu count=%zu", peer_repr, ctx->protocol_index,
		ctx->protocol_count);
	...
	while (ctx->protocol_index < ctx->protocol_count)
	{
		...
		LP_LOGT(GOSSIPSUB_MODULE, "PX dial attempting peer=%s protocol=%s", peer_repr, protocol);

		int rc = libp2p_host_open_stream_async(gs->host, ctx->peer, protocol, gossipsub_px_dial_cb, ctx);
		if (rc == LIBP2P_ERR_OK)
			return;

		LP_LOGT(GOSSIPSUB_MODULE, "PX dial immediate failure peer=%s protocol=%s rc=%d", peer_repr, protocol,
			rc);
	...
	LP_LOGT(GOSSIPSUB_MODULE, "PX dial exhausted protocols peer=%s attempts=%zu", peer_repr, ctx->protocol_count);
...
static void gossipsub_px_dial_cb(libp2p_stream_t *s, void *user_data, int err)
{
	...
	{
		LP_LOGT(GOSSIPSUB_MODULE, "PX dial succeeded peer=%s protocol=%s", peer_repr, protocol);
		gossipsub_on_stream_open(s, gs);
		gossipsub_px_ctx_destroy(ctx);
		return;
	}

	LP_LOGT(GOSSIPSUB_MODULE, "PX dial failed peer=%s protocol=%s err=%d", peer_repr, protocol, err);
...
void gossipsub_propagation_propagate_frame(libp2p_gossipsub_t *gs, gossipsub_topic_state_t *topic,
					   const peer_id_t *exclude_peer, const uint8_t *frame, size_t frame_len)
{
	...
	LP_LOGI(GOSSIPSUB_MODULE, "propagate_frame START topic=%s frame_len=%zu mesh_count=%zu flood=%d threshold=%.2f",
		topic && topic->name ? topic->name : "(null)", frame_len, mesh_count, flood_enabled, publish_threshold);
	...
	if (topic)
	{
		for (gossipsub_mesh_member_t *member = topic->mesh; member; member = member->next)
		{
			...
			if (member->peer)
			{
				peer_str = gossipsub_peer_to_string(member->peer, peer_buf, sizeof(peer_buf));
			}
			...
			if (!peer_entry || !peer_entry->connected)
			{
				...
				LP_LOGD(GOSSIPSUB_MODULE,
					"propagate_frame SKIP mesh peer=%s reason=disconnected peer_entry=%p "
					"connected=%d",
			...
			if (exclude_peer && gossipsub_peer_equals(member->peer, exclude_peer))
			{
				...
				LP_LOGD(GOSSIPSUB_MODULE, "propagate_frame SKIP mesh peer=%s reason=excluded",
					peer_str);
				continue;
			}
			if (flood_enabled && has_topic && peer_entry->score < publish_threshold)
			{
				mesh_skipped_score++;
				LP_LOGD(GOSSIPSUB_MODULE,
					"propagate_frame SKIP mesh peer=%s reason=low_score score=%.2f threshold=%.2f",
					peer_str, peer_entry->score, publish_threshold);
				continue;
			}

			LP_LOGT(GOSSIPSUB_MODULE, "propagate_frame ENQUEUE mesh peer=%s frame_len=%zu stream=%p",
			...
			{
				LP_LOGD(GOSSIPSUB_MODULE, "propagate_frame ENQUEUE_FAILED mesh peer=%s rc=%d", peer_str,
					enq_rc);
			}
			mesh_sent++;
		}
	...
	LP_LOGT(GOSSIPSUB_MODULE,
		"propagate_frame MESH_DONE sent=%zu skipped_disconn=%zu skipped_explicit=%zu skipped_excluded=%zu "
		"skipped_score=%zu",
	...
	LP_LOGT(GOSSIPSUB_MODULE, "propagate_frame COMPLETE total_mesh_sent=%zu", mesh_sent);
	pthread_mutex_unlock(&gs->lock);
}

void gossipsub_propagation_emit_gossip_locked(libp2p_gossipsub_t *gs, gossipsub_topic_state_t *topic,
					      uint64_t gossip_round)
{
	...
	size_t *lengths = NULL;
	size_t count = 0;
	libp2p_err_t cache_rc = gossipsub_message_cache_collect_ids(&gs->message_cache, topic->name, &ids, &lengths,
								    &count, gossip_round);
	...
	libp2p_err_t enc_rc = gossipsub_rpc_encode_control_ihave(topic->name, ids, lengths, count, &frame);
	...
	if (enc_rc == LIBP2P_ERR_OK && frame.frame && frame.frame_len)
	{
		...
		for (gossipsub_peer_entry_t *entry = gs->peers; entry; entry = entry->next)
		{
			...
			if (eligible_count == eligible_capacity)
			{
				...
				gossipsub_peer_entry_t **new_list =
					(gossipsub_peer_entry_t **)realloc(eligible, new_cap * sizeof(*new_list));
				if (!new_list)
				{
					free(eligible);
					eligible = NULL;
		...
		{
			size_t gossip_percent =
				(gs->cfg.gossip_factor_percent > 0) ? (size_t)gs->cfg.gossip_factor_percent : 0;
			...
			for (size_t i = 0, sent = 0; i < limit && sent < limit; ++i)
			{
				...
				if (!entry->explicit_peering && entry->score < gs->cfg.gossip_threshold)
					continue;
				if (gossipsub_peer_enqueue_frame_locked(gs, entry, frame.frame, frame.frame_len) ==
				    LIBP2P_ERR_OK)
					sent++;
			}
		}
...
libp2p_err_t gossipsub_propagation_handle_inbound_publish(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry,
							  libp2p_gossipsub_Message *proto_msg, const uint8_t *frame,
							  size_t frame_len)
{
	...
	if (!topic_raw || topic_len == 0)
	{
		...
		LP_LOGW(GOSSIPSUB_MODULE, "inbound publish has no topic peer=%s has_topic=%d topic_ids_count=%zu",
			peer_repr, libp2p_gossipsub_Message_has_topic(proto_msg),
	...
	if (topic_str)
	{
		...
		LP_LOGD(GOSSIPSUB_MODULE, "inbound publish topic=%s peer=%s data_len=%zu", topic_str, peer_repr,
			msg.data_len);
	}
...
libp2p_err_t gossipsub_propagation_handle_subscriptions(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry,
							gossipsub_rpc_subscription_t *subs, size_t count)
{
	char peer_buf[128];
	const char *peer_repr = gossipsub_peer_to_string(entry ? entry->peer : NULL, peer_buf, sizeof(peer_buf));
	LP_LOGT(GOSSIPSUB_MODULE, "handle_subscriptions peer=%s count=%zu", peer_repr, count);
	for (size_t i = 0; i < count; ++i)
	{
		gossipsub_rpc_subscription_t *sub = &subs[i];
		if (!sub || !sub->topic)
			continue;
		LP_LOGD(GOSSIPSUB_MODULE, "subscription_inbound peer=%s topic=%s subscribe=%d", peer_repr,
			sub->topic ? sub->topic : "(null)", sub->subscribe ? 1 : 0);
	}
	if (!gs || !entry)
		return LIBP2P_ERR_NULL_PTR;
	if (!subs || count == 0)
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		if (sub->subscribe)
		{
			...
			const char *peer_repr =
				gossipsub_peer_to_string(entry ? entry->peer : NULL, peer_buf, sizeof(peer_buf));
			LP_LOGT(GOSSIPSUB_MODULE, "subscription peer=%s topic=%s topic_id=%s subscribe=1", peer_repr,
				sub->topic ? sub->topic : "(null)", sub->topic_id ? sub->topic_id : "(null)");
		...
		else
		{
			...
			const char *peer_repr =
				gossipsub_peer_to_string(entry ? entry->peer : NULL, peer_buf, sizeof(peer_buf));
			LP_LOGT(GOSSIPSUB_MODULE, "subscription peer=%s topic=%s topic_id=%s subscribe=0", peer_repr,
				sub->topic ? sub->topic : "(null)", sub->topic_id ? sub->topic_id : "(null)");
...
libp2p_err_t gossipsub_propagation_handle_control_iwant(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry,
							const gossipsub_rpc_control_iwant_t *iwants, size_t count)
{
	if (!gs || !entry)
		return LIBP2P_ERR_NULL_PTR;
	if (!iwants || count == 0)
...
libp2p_err_t gossipsub_propagation_handle_control_ihave(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry,
							const gossipsub_rpc_control_ihave_t *ihaves, size_t count)
{
	if (!gs || !entry)
		return LIBP2P_ERR_NULL_PTR;
	if (!ihaves || count == 0)
...
budget_exhausted:
	if (want_count > 0)
	{
		...
		if (send_count > 0)
		{
			uint64_t expire_ms = gossipsub_propagation_compute_backoff_expiry(
				gossipsub_now_ms(), (uint64_t)gs->cfg.iwant_followup_time_ms);
			gossipsub_promises_track(&gs->promises, entry->peer, (const uint8_t *const *)want_ids,
						 want_lens, send_count, expire_ms);
		}
		pthread_mutex_unlock(&gs->lock);
		...
		{
			pthread_mutex_lock(&gs->lock);
			libp2p_err_t send_rc =
				gossipsub_peer_enqueue_frame_locked(gs, entry, frame.frame, frame.frame_len);
			pthread_mutex_unlock(&gs->lock);
			if (send_rc != LIBP2P_ERR_OK && result == LIBP2P_ERR_OK)
				result = send_rc;
		}
...
libp2p_err_t gossipsub_propagation_handle_control_graft(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry,
							const gossipsub_rpc_control_graft_t *grafts, size_t count)
{
	if (!gs || !entry)
		return LIBP2P_ERR_NULL_PTR;
	if (!grafts || count == 0)
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		if (!store_for_prune)
		{
			LP_LOGD(GOSSIPSUB_MODULE, "graft_accept peer=%s topic=%s score=%.3f decision=%s outbound=%d",
				peer_repr, topic_name, peer_score, decision, entry && entry->outbound_stream ? 1 : 0);
			if (px_peers)
		...
		if (prune_count == prune_cap)
		{
			...
			gossipsub_prune_target_t **new_array =
				(gossipsub_prune_target_t **)realloc(prune_targets, new_cap * sizeof(*new_array));
			if (!new_array)
			{
				gossipsub_prune_target_free(target);
				if (result == LIBP2P_ERR_OK)
		...
		LP_LOGD(GOSSIPSUB_MODULE,
			...
			peer_repr, topic_name, peer_score, decision, extend_backoff, px_len);
	}
	...
	if (prune_count > 0)
	{
		...
		gossipsub_rpc_out_init(&frame);
		libp2p_err_t enc_rc =
			gossipsub_rpc_encode_control_prune((const gossipsub_prune_target_t *const *)prune_targets,
							   prune_count, config_backoff_seconds, &frame);
		if (enc_rc == LIBP2P_ERR_OK && frame.frame && frame.frame_len)
		{
			pthread_mutex_lock(&gs->lock);
			libp2p_err_t send_rc =
...
libp2p_err_t gossipsub_propagation_handle_control_prune(libp2p_gossipsub_t *gs, gossipsub_peer_entry_t *entry,
							const gossipsub_rpc_control_prune_t *prunes, size_t count)
{
	if (!gs || !entry)
		return LIBP2P_ERR_NULL_PTR;
	if (!prunes || count == 0)
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		if (topic)
		{
			...
			if (!gossipsub_backoff_add(topic, entry->peer, expire_ms) && result == LIBP2P_ERR_OK)
				result = LIBP2P_ERR_INTERNAL;
			LP_LOGD(GOSSIPSUB_MODULE,
				"prune_recv peer=%s topic=%s backoff_ms=%" PRIu64 " px_count=%zu score=%.3f", peer_repr,
				prune->topic, backoff_ms, prune->px_count, entry ? entry->score : 0.0);
		}
		...
		if (allow_px && prune->px && prune->px_count > 0)
		{
			for (size_t j = 0; j < prune->px_count; ++j)
			{
				...
				if (dial_count == dial_cap)
				{
					...
					gossipsub_peer_entry_t **new_list = (gossipsub_peer_entry_t **)realloc(
						dial_peers, new_cap * sizeof(*new_list));
					if (!new_list)
						continue;
					dial_peers = new_list;
					dial_cap = new_cap;
				...
				if (px->signed_peer_record && px->signed_peer_record_len > 0)
				{
					if (ingest_count == ingest_cap)
					{
						...
						const gossipsub_rpc_px_record_t **new_list =
							(const gossipsub_rpc_px_record_t **)realloc(
								ingest_records, new_cap * sizeof(*new_list));
						if (new_list)
						{
		...
		for (size_t j = 0; j < ingest_count; ++j)
		{
			...
			if (px && px->peer && px->signed_peer_record && px->signed_peer_record_len > 0)
				gossipsub_propagation_ingest_px_record(gs, px->peer, px->signed_peer_record,
								       px->signed_peer_record_len);
		}
