...
static libp2p_multiselect_err_t recv_msg_io(libp2p_io_t *io, char **out, uint64_t deadline_ms)
{
    ...
    if (!pl_len || pl_len > MS_MAX_MESSAGE_SIZE)
    {
        LP_LOGD(
            "MULTISELECT",
            "proto_mal bad_len io=%p pl_len=%" PRIu64 " var_len=%zu",
            (void *)io,
            pl_len,
            used);
...
libp2p_multiselect_err_t libp2p_multiselect_dial_io(libp2p_io_t *io, const char *const proposals[], uint64_t timeout_ms, const char **accepted_out)
{
    ...
    if (rc)
    {
        LP_LOGD(
            "MULTISELECT",
            "dial send header rc=%d timeout_ms=%" PRIu64 " io=%p",
            (int)rc,
            timeout_ms,
            (void *)io);
        goto done;
    ...
    if (rc)
    {
        LP_LOGD(
            "MULTISELECT",
            "dial recv header rc=%d timeout_ms=%" PRIu64 " io=%p",
            (int)rc,
            timeout_ms,
            (void *)io);
        goto done;
    ...
    if (rc)
    {
        LP_LOGD(
            "MULTISELECT",
            "dial send proposal idx=%zu proto=%s rc=%d timeout_ms=%" PRIu64 " io=%p",
            idx,
    ...
    while (proposals[idx])
    {
        ...
        if (rc)
        {
            LP_LOGD(
                "MULTISELECT",
                "dial recv response idx=%zu rc=%d timeout_ms=%" PRIu64 " io=%p",
                idx,
                (int)rc,
                timeout_ms,
                (void *)io);
                                                      ...
                                                      const char **accepted_out)
{
    ...
    for (;;)
    {
        ...
        if (rc)
        {
            LP_LOGD(
                "MULTISELECT",
                "listen recv msg header_received=%d rc=%d timeout_ms=%" PRIu64 " io=%p",
                header_received ? 1 : 0,
                (int)rc,
        ...
        if (!header_received)
        {
            ...
            if (!header_sent)
            {
                ...
                if (rc)
                {
                    LP_LOGD(
                        "MULTISELECT",
                        "listen send header rc=%d timeout_ms=%" PRIu64 " io=%p",
                        (int)rc,
                        cfg.handshake_timeout_ms,
                        (void *)io);
                    goto fail;
        ...
        if (!strcmp(msg, LIBP2P_MULTISELECT_PROTO_ID))
        {
            ...
            if (!header_sent)
            {
                ...
                if (rc)
                {
                    LP_LOGD(
                        "MULTISELECT",
                        "listen re-send header rc=%d timeout_ms=%" PRIu64 " io=%p",
                        (int)rc,
                        cfg.handshake_timeout_ms,
                        (void *)io);
                    goto fail;
        ...
        if (!strcmp(msg, LIBP2P_MULTISELECT_LS))
        {
            ...
            if (rc)
            {
                LP_LOGD(
                    "MULTISELECT",
                    "listen send ls/na rc=%d timeout_ms=%" PRIu64 " io=%p",
                    (int)rc,
                    cfg.handshake_timeout_ms,
                    (void *)io);
                goto fail;
        ...
        if (str_in_list(msg, supported))
        {
            ...
            if (rc)
            {
                LP_LOGD(
                    "MULTISELECT",
                    "listen send accept rc=%d timeout_ms=%" PRIu64 " io=%p",
                    (int)rc,
                    cfg.handshake_timeout_ms,
                    (void *)io);
                free(msg);
        ...
        if (rc)
        {
            LP_LOGD(
                "MULTISELECT",
                "listen send na rc=%d timeout_ms=%" PRIu64 " io=%p",
                (int)rc,
                cfg.handshake_timeout_ms,
                (void *)io);
            goto fail;
