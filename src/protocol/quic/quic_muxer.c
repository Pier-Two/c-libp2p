...
static int quic_session_callback(picoquic_cnx_t *cnx, uint64_t stream_id, uint8_t *bytes, size_t length,
		...
		for (protocol_match_entry_t *m = host->matchers; m && !found; m = m->next)
		{
			...
			case LIBP2P_PROTO_MATCH_SEMVER: {
				...
				if (extract_version_from_id(incoming_id, base, &vin) == 0)
				{
					...
					if (parse_semver_range(m->matcher.pattern, &range) == 0 &&
					    semver_in_range(&vin, &range))
					{
						chosen = m->def;
						found = 1;
					}
...
static int quic_run_inbound_handshake(quic_muxer_ctx_t *mx, quic_stream_ctx_t *st)
{
	...
	{
		LP_LOGD("QUIC", "run_inbound_handshake multiselect failed ms=%d stream_id=%llu", (int)ms,
			(unsigned long long)st->stream_id);
		return 0;
	}
	LP_LOGI("QUIC", "run_inbound_handshake multiselect OK accepted=%s stream_id=%llu", accepted,
		(unsigned long long)st->stream_id);
	...
	LP_LOGI("QUIC", "run_inbound_handshake protocol_lookup proto=%s found=%d has_on_open=%d", proto_id, found,
		chosen.on_open ? 1 : 0);
	if (!found)
	{
...
#ifndef _WIN32
static int quic_multiaddr_to_sockaddr(const multiaddr_t *addr, struct sockaddr_storage *ss, socklen_t *ss_len)
{
	if (!addr || !ss || !ss_len)
		return -1;
	...
	if (code0 == MULTICODEC_IP6)
	{
		...
		if (code == MULTICODEC_IP6ZONE)
		{
			...
			if (multiaddr_get_address_bytes(addr, idx, (uint8_t *)zonebuf, &zl) != MULTIADDR_SUCCESS ||
			    zl == 0)
				return -1;
			zonebuf[zl] = '\0';
#ifdef __APPLE__
...
static int quic_packet_loop_cb(picoquic_quic_t *quic, picoquic_packet_loop_cb_enum cb_mode, void *callback_ctx,
			       void *callback_arg)
{
	(void)quic;
	(void)callback_arg;
...
int libp2p__quic_session_start_loop(libp2p_quic_session_t *session, const multiaddr_t *local_addr,
				    const multiaddr_t *remote_addr)
{
	...
	int expected = 0;
	if (!atomic_compare_exchange_strong_explicit(&session->loop_started, &expected, 1, memory_order_acq_rel,
	...
	int loop_ret = 0;
	picoquic_network_thread_ctx_t *loop_ctx = picoquic_start_network_thread(
		session->quic, &session->loop_param, quic_packet_loop_cb, session, &loop_ret);
...
void libp2p__quic_session_attach_thread(libp2p_quic_session_t *session, picoquic_network_thread_ctx_t *thread_ctx)
{
	if (!session)
		return;
	pthread_mutex_lock(&session->lock);
...
void libp2p__quic_session_get_io_stats(libp2p_quic_session_t *session, uint64_t *last_rx_ms, uint64_t *last_tx_ms,
				       uint64_t *rx_count, uint64_t *tx_count)
{
	if (!session)
		return;
	if (last_rx_ms)
...
static quic_session_event_t *quic_session_event_new(uint64_t stream_id, const uint8_t *bytes, size_t len,
						    picoquic_call_back_event_t event)
{
...
static void quic_session_pending_append_locked(libp2p_quic_session_t *session, quic_session_event_t *ev)
{
	if (!session || !ev)
		return;
	ev->next = NULL;
...
static void quic_stream_start_handshake(quic_muxer_ctx_t *mx, quic_stream_ctx_t *st,
					libp2p_stream_t *stream_for_handshake)
{
	if (!st)
		return;
...
static int quic_session_dispatch(libp2p_quic_session_t *session, quic_muxer_ctx_t *mx, picoquic_cnx_t *cnx,
				 uint64_t stream_id, uint8_t *bytes, size_t length, picoquic_call_back_event_t event,
				 void *stream_ctx)
{
	(void)session;
	(void)cnx;
	...
	LP_LOGI("QUIC", "muxer_stream_event stream_id=%llu event=%d length=%zu", (unsigned long long)stream_id,
		(int)event, length);
	...
	case picoquic_callback_stop_sending:
		...
		else
		{
			...
			LP_LOGW("QUIC", "stop_sending event for unknown stream %" PRIu64 ", queueing for later",
				stream_id);
			pthread_mutex_lock(&mx->lock);
...
static void quic_session_replay_events(libp2p_quic_session_t *session, quic_muxer_ctx_t *mx, picoquic_cnx_t *cnx,
				       quic_session_event_t *events)
{
	...
	{
		quic_session_event_t *next = cur->next;
		quic_session_dispatch(session, mx, cnx, cur->stream_id, cur->data, cur->len, cur->event, NULL);
		quic_session_event_free(cur);
		cur = next;
	}
}
...
static void quic_stream_flush_outgoing_locked(libp2p_quic_session_t *session, picoquic_cnx_t *cnx,
					      quic_stream_ctx_t *st)
{
	(void)session;
	if (!st || !cnx)
	...
	if (st->reset_pending)
	{
		...
		{
			LP_LOGW("QUIC", "stream_reset pending ctx=%p stream_id=%" PRIu64 " rc=%d", (void *)st,
				st->stream_id, rc);
		}
		return;
	}
				 ...
				 picoquic_call_back_event_t event, void *callback_ctx, void *stream_ctx)
{
	...
	int closing_event = (event == picoquic_callback_close || event == picoquic_callback_application_close ||
			     event == picoquic_callback_stateless_reset);

	if (closing_event)
		libp2p__quic_session_disable_cnx(session);

	int bufferable = (event == picoquic_callback_stream_data || event == picoquic_callback_stream_fin ||
			  event == picoquic_callback_stream_reset || event == picoquic_callback_stop_sending);
...
static void quic_log_drop_unknown(picoquic_cnx_t *cnx, const char *kind, uint64_t stream_id)
{
	...
	if (n % log_every == 1)
	{
		...
		LP_LOGW("QUIC",
			...
			kind ? kind : "data", (unsigned long long)stream_id, n, role, initiator, dir, local_initiated,
			(uint64_t)(stream_id >> 2), libp2p__quic_state_name(st), (int)st, cid_hex[0] ? cid_hex : "-",
			have_recent, age_ms, have_recent && recent.proto[0] ? recent.proto : "-",
			have_recent ? recent.fin_local : 0, have_recent ? recent.fin_remote : 0,
			have_recent ? recent.reset_remote : 0, have_recent ? recent.stop_sending : 0,
...
static ssize_t quic_stream_backend_write(void *io_ctx, const void *buf, size_t len)
{
	...
	LP_LOGD("QUIC", "stream_backend_write queued ctx=%p cnx=%p stream_id=%" PRIu64 " len=%zu", (void *)st,
		(void *)cnx, st->stream_id, len);
...
static int quic_stream_backend_is_writable(void *io_ctx)
{
	...
	pthread_mutex_lock(&st->lock);
	int blocked =
		st->out_blocked || st->stop_sending_received || st->reset_remote || st->fin_local || st->fin_pending;
	pthread_mutex_unlock(&st->lock);
	return blocked ? 0 : 1;
}
...
static libp2p_muxer_err_t quic_muxer_open_stream(libp2p_muxer_t *mx, const uint8_t *name, size_t name_len,
						 libp2p_stream_t **out)
{
	(void)name;
	(void)name_len;
