...
static void quic_stream_handshake_exec(void *arg)
{
    ...
    if (!ok)
    {
        if (mx && mx->session)
        {
            picoquic_cnx_t *cnx = atomic_load_explicit(&mx->session->cnx, memory_order_acquire);
            if (cnx)
...
static ssize_t quic_stream_backend_write(void *io_ctx, const void *buf, size_t len)
{
    ...
    if (len == 0)
        return 0;
    picoquic_cnx_t *cnx = atomic_load_explicit(&st->mx->session->cnx, memory_order_acquire);
    if (!cnx)
    {
        LP_LOGW("QUIC",
...
static libp2p_muxer_err_t quic_muxer_open_stream(libp2p_muxer_t *mx, const uint8_t *name, size_t name_len, libp2p_stream_t **out)
{
    ...
    picoquic_cnx_t *cnx = atomic_load_explicit(&ctx->session->cnx, memory_order_acquire);
