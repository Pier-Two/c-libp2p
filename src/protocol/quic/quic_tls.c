...
static int verify_signed_key(uint64_t key_type, const uint8_t *key_data, size_t key_len, const uint8_t *sig,
			     size_t sig_len, const uint8_t *msg, size_t msg_len)
{
	if (!key_data || !sig || !msg)
	{
		LP_LOGE("quic-tls", "verify_signed_key received null input (key=%p sig=%p msg=%p)", (void *)key_data,
			(void *)sig, (void *)msg);
		return LIBP2P_ERR_NULL_PTR;
	}
	...
	if (key_type == PEER_ID_KEY_SECP256K1)
	{
		SHA256_HASH hash;
	...
	if (key_type == PEER_ID_KEY_RSA)
	{
		...
		int stat = 0;
		int rc = rsa_verify_hash_ex(sig, (unsigned long)sig_len, hash.bytes, sizeof(hash.bytes),
					    LTC_PKCS_1_V1_5, sha_idx, 0, &stat, &rsa);
		rsa_free(&rsa);
		if (rc != CRYPT_OK || stat == 0)
		{
			LP_LOGW("quic-tls", "verify_signed_key RSA signature verification failed (rc=%d stat=%d)", rc,
				stat);
			return LIBP2P_ERR_INTERNAL;
		}
		return LIBP2P_ERR_OK;
...
static int parse_signed_key(const ASN1_OCTET_STRING *raw, LIBP2P_TLS_SIGNED_KEY **out)
{
	...
	if (tmp != ptr + len)
	{
		...
		LP_LOGW("quic-tls", "parse_signed_key trailing bytes (consumed=%td len=%ld)", (ptrdiff_t)(tmp - ptr),
			len);
		return LIBP2P_ERR_INTERNAL;
	}
	*out = sk;
...
static int build_public_key_protobuf(uint64_t key_type, const uint8_t *identity_key, size_t identity_key_len,
				     uint8_t **out_pb, size_t *out_pb_len)
{
	...
	*out_pb_len = 0;
	...
	if (key_type == PEER_ID_KEY_RSA)
	{
		...
		peer_id_error_t perr = peer_id_build_public_key_protobuf(PEER_ID_KEY_RSA, der_buf, (size_t)der_len,
									 out_pb, out_pb_len);
		secure_zero(der_buf, (size_t)der_len);
		free(der_buf);
		return (perr == PEER_ID_OK) ? LIBP2P_ERR_OK : LIBP2P_ERR_INTERNAL;
...
static int sign_identity_key(uint64_t key_type, const uint8_t *identity_key, size_t identity_key_len,
			     const uint8_t *msg, size_t msg_len, uint8_t **out_sig, size_t *out_sig_len)
{
	...
	*out_sig_len = 0;
	...
	if (key_type == PEER_ID_KEY_RSA)
	{
		...
		if (rsa_sign_hash_ex(hash.bytes, sizeof(hash.bytes), sig, &sig_len, LTC_PKCS_1_V1_5, NULL, 0, sha_idx,
				     0, &rsa) != CRYPT_OK)
		{
			rsa_free(&rsa);
			free(sig);
...
int libp2p_quic_tls_generate_certificate(const libp2p_quic_tls_cert_options_t *opts, libp2p_quic_tls_certificate_t *out)
{
	...
	int ret = build_public_key_protobuf(opts->identity_key_type, opts->identity_key, opts->identity_key_len,
					    &public_key_pb, &public_key_pb_len);
	if (ret != LIBP2P_ERR_OK)
	{
		LP_LOGE("quic-tls", "build_public_key_protobuf failed (type=%" PRIu64 ", rc=%d)",
			opts->identity_key_type, ret);
		goto cleanup;
	}
	...
	if (sign_identity_key(opts->identity_key_type, opts->identity_key, opts->identity_key_len, msg, msg_len,
			      &signature, &signature_len) != LIBP2P_ERR_OK)
	{
	...
	if (parsed_type != opts->identity_key_type)
		goto cleanup;
	if (verify_signed_key(parsed_type, raw_key, raw_key_len, signature, signature_len, msg, msg_len) !=
	    LIBP2P_ERR_OK)
		goto cleanup;
...
int libp2p_quic_tls_identity_from_certificate(const uint8_t *cert_der, size_t cert_len, libp2p_quic_tls_identity_t *out)
{
	...
	{
		LP_LOGW("quic-tls",
			"verify_signed_key failed during certificate check (type=%" PRIu64
			" key_len=%zu sig_len=%zu rc=%d)",
			key_type, raw_key_len, signature_len, verify_rc);
		LIBP2P_TLS_SIGNED_KEY_free(signed_key);
		X509_free(cert);
		return verify_rc;
	}
	...
	peer_id_t *pid = NULL;
	peer_id_error_t perr = peer_id_new_from_public_key_pb(public_key_pb, public_key_len, &pid);
	if (perr != PEER_ID_OK)
	{
