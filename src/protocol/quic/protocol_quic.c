...
static void quic_log_handshake_diag(const char *reason, picoquic_cnx_t *cnx, picoquic_state_enum st, uint64_t local_err,
				    uint64_t remote_err)
{
	...
	LP_LOGE("QUIC",
		...
		reason ? reason : "handshake diagnostic", state_name ? state_name : "unknown", st, local_err,
		picoquic_error_name(local_err), remote_err, picoquic_error_name(remote_err),
		initial_cid_buf[0] ? initial_cid_buf : "-", local_cid_buf[0] ? local_cid_buf : "-",
		remote_cid_buf[0] ? remote_cid_buf : "-", local_addr ? local_addr : "-",
...
int libp2p__quic_transport_copy_identity(quic_transport_ctx_t *ctx, uint8_t **out_key, size_t *out_len,
					 uint64_t *out_type)
{
...
static void quic_transport_verify_ctx_free(ptls_verify_certificate_t *self)
{
	if (!self)
		return;
	quic_transport_verify_ctx_t *ctx =
		(quic_transport_verify_ctx_t *)((uint8_t *)self - offsetof(quic_transport_verify_ctx_t, super));
	free(ctx);
}
...
static int quic_transport_verify_cb(struct st_ptls_verify_certificate_t *self, ptls_t *tls, const char *server_name,
...
multiaddr_t *libp2p__quic_multiaddr_from_sockaddr(const struct sockaddr *sa, socklen_t len)
{
	...
	defined(_WIN32)
			if (if_indextoname(v6->sin6_scope_id, zonebuf))
				snprintf(ma_str, sizeof(ma_str), "/ip6/%s/ip6zone/%s/udp/%u", buf, zonebuf,
...
static libp2p_transport_err_t quic_wait_for_ready(libp2p_quic_session_t *session, picoquic_cnx_t *cnx,
						  uint64_t timeout_ms)
{
	...
	for (;;)
	{
		...
		if (st != last_state)
		{
			...
			LP_LOGT("QUIC",
				"wait state=%s(%d) rx_count=%" PRIu64 " tx_count=%" PRIu64
				" rx_seen=%d rx_age_ms=%" PRIu64 " tx_seen=%d tx_age_ms=%" PRIu64,
				libp2p__quic_state_name(st), st, rx_count, tx_count, rx_seen, rx_age, tx_seen, tx_age);
			last_state = st;
		}
		...
		if (st == picoquic_state_disconnected || st == picoquic_state_handshake_failure ||
		    ...
		    st == picoquic_state_closing || st == picoquic_state_closing_received ||
...
static void quic_transport_session_close(libp2p_quic_session_t *session)
{
	...
	if (cnx)
	{
		...
		LP_LOGD("QUIC",
			"session_close begin session=%p cnx=%p state=%s(%d) client=%d local_err=%" PRIu64
			" (%s) remote_err=%" PRIu64 " (%s) app_err=%" PRIu64,
			(void *)session, (void *)cnx, libp2p__quic_state_name(state_before), (int)state_before,
			picoquic_is_client(cnx), local_err, picoquic_error_name(local_err), remote_err,
			picoquic_error_name(remote_err), app_err);
		...
		LP_LOGD("QUIC",
			"session_close end session=%p cnx=%p state=%s(%d) elapsed_us=%" PRIu64 " local_err=%" PRIu64
			" (%s) remote_err=%" PRIu64 " (%s) app_err=%" PRIu64,
			(void *)session, (void *)cnx, libp2p__quic_state_name(st), (int)st, elapsed, final_local,
			picoquic_error_name(final_local), final_remote, picoquic_error_name(final_remote), final_app);
	}
	libp2p__quic_session_wake(session);
}
...
#define QUIC_DIAL_RETURN(_rc)                                                                                          \
	...
	picoquic_quic_t *quic = picoquic_create(1, NULL, NULL, NULL, ctx->cfg.alpn, NULL, NULL, NULL, NULL, NULL,
	...
	LP_LOGD("QUIC",
		"client transport params configured loss_bit=%d min_ack_delay=%" PRIu64 " max_idle_timeout=%" PRIu64
		"ms",
		client_tp.enable_loss_bit, client_tp.min_ack_delay, client_tp.max_idle_timeout);
	...
	picoquic_cnx_t *cnx = picoquic_create_cnx(quic, picoquic_null_connection_id, picoquic_null_connection_id,
						  (struct sockaddr *)&remote_ss, picoquic_current_time(), 0,
						  (sni[0] != '\0') ? sni : NULL, ctx->cfg.alpn, 1);
	if (!cnx)
	{
	...
	libp2p_conn_t *conn = libp2p_quic_conn_new(NULL, addr, session, quic_transport_session_close,
						   quic_transport_session_free, NULL);
	if (!conn)
	{
		quic_transport_verify_ctx_free(&verify_ctx->super);
