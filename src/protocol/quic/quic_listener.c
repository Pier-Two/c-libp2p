...
static void quic_listener_schedule_cert_rotation(quic_listener_ctx_t *ctx, uint64_t now_ms)
{
	if (!ctx)
		return;
	uint32_t lifetime_s =
		ctx->cert_lifetime_s ? ctx->cert_lifetime_s : libp2p_quic_tls_cert_options_default().not_after_lifetime;
	if (lifetime_s == 0)
		lifetime_s = 1;
	uint64_t lifetime_ms = (uint64_t)lifetime_s * 1000ULL;
...
static int quic_listener_rotate_certificate_locked(quic_listener_ctx_t *ctx, uint64_t now_ms)
{
	...
	LP_LOGI("QUIC", "listener TLS certificate rotated successfully; next rotation in %" PRIu64 " ms",
...
static void *quic_listener_cert_rotation_thread_main(void *arg)
{
	...
	while (!atomic_load_explicit(&ctx->closing, memory_order_acquire))
	{
		...
		if (rotate_at_ms > 0 && now_ms >= rotate_at_ms)
		{
			...
			if (ctx->quic && !atomic_load_explicit(&ctx->closing, memory_order_acquire) &&
			    ctx->cert_rotate_at_ms > 0 && now_ms >= ctx->cert_rotate_at_ms)
			{
				...
				{
					uint64_t retry_ms =
						quic_listener_rotation_retry_delay_ms(ctx->cert_rotation_failures);
...
static int quic_listener_make_conn(quic_listener_ctx_t *ctx, picoquic_cnx_t *cnx, libp2p_conn_t **out_conn)
{
	if (!ctx || !cnx || !out_conn)
		return -1;
	...
	multiaddr_t *remote_base =
		peer_len ? libp2p__quic_multiaddr_from_sockaddr((struct sockaddr *)&peer_copy, peer_len) : NULL;
	if (!remote_base)
		return -1;
	...
	multiaddr_t *local_base =
		local_len ? libp2p__quic_multiaddr_from_sockaddr((struct sockaddr *)&local_copy, local_len) : NULL;
	if (!local_base)
	{
		multiaddr_free(remote);
		return -1;
	...
	libp2p_conn_t *conn = libp2p_quic_conn_new(local, remote, session, quic_listener_session_close,
						   quic_listener_session_free, peer);
...
static int quic_listener_conn_cb(picoquic_cnx_t *cnx, uint64_t stream_id, uint8_t *bytes, size_t length,
...
static void quic_listener_flush_outgoing(quic_listener_ctx_t *ctx)
{
	...
	while (cnx)
	{
		libp2p_quic_session_t *session = (libp2p_quic_session_t *)picoquic_get_callback_context(cnx);
		if (session)
...
static int quic_listener_loop_cb(picoquic_quic_t *quic, picoquic_packet_loop_cb_enum cb_mode, void *callback_ctx,
				 void *callback_arg)
{
	(void)quic;
	quic_listener_ctx_t *ctx = (quic_listener_ctx_t *)callback_ctx;
...
int quic_listener_start(quic_listener_ctx_t *ctx)
{
	...
	int loop_ret = 0;
	picoquic_network_thread_ctx_t *net_thread =
		picoquic_start_network_thread(ctx->quic, &ctx->loop_param, quic_listener_loop_cb, ctx, &loop_ret);
...
libp2p_transport_err_t quic_listener_create(libp2p_transport_t *transport, quic_transport_ctx_t *transport_ctx,
					    const multiaddr_t *addr, libp2p_listener_t **out)
{
	...
	picoquic_quic_t *quic = picoquic_create(max_connections, NULL, NULL, NULL,
						cfg.alpn ? cfg.alpn : LIBP2P_QUIC_TLS_ALPN, quic_listener_conn_cb, ctx,
	...
	LP_LOGD("QUIC",
		"listener transport params configured loss_bit=%d min_ack_delay=%" PRIu64 " max_idle_timeout=%" PRIu64
		"ms",
		listener_tp.enable_loss_bit, listener_tp.min_ack_delay, listener_tp.max_idle_timeout);
