...
static void identify_srv_handle(identify_srv_ctx_t *ctx)
{
	...
	{
		size_t peer_len = 0U;
		if (peer_id_text_write(remote, PEER_ID_TEXT_LEGACY_BASE58, peer_buf, sizeof(peer_buf), &peer_len) !=
		    PEER_ID_OK)
			peer_buf[0] = '\0';
	}
	const char *proto_id = libp2p_stream_protocol_id(s);
	LIBP2P_TRACE("identify", "serving identify stream=%p peer=%s proto=%s", (void *)s, peer_buf,
		     proto_id ? proto_id : "(unknown)");
	...
	if (libp2p_identify_encode_local(host, s, 1, &buf, &blen) == 0 && buf && blen > 0)
	{
		...
		if (written < 0)
		{
			LIBP2P_TRACE("identify", "lp_send failed stream=%p rc=%zd proto=%s", (void *)s, written,
				     proto_id ? proto_id : "(unknown)");
		}
		else
		{
			LIBP2P_TRACE("identify", "lp_send ok stream=%p bytes=%zd peer=%s", (void *)s, written,
				     peer_buf);
		}
	}
	else
...
static void identify_push_process_full(libp2p_stream_t *s, identify_push_state_t *st)
{
	...
	if (remote && (peer_id_clone(remote, &remote_copy) != PEER_ID_OK))
		remote_copy = NULL;

	const peer_id_t *target = remote_copy ? remote_copy : remote;
	peer_id_t *derived = NULL;
	if (id->public_key && id->public_key_len &&
	    peer_id_new_from_public_key_pb(id->public_key, id->public_key_len, &derived) == PEER_ID_OK)
	{
		if (!target || peer_id_equal(target, derived) != 1)
			target = derived;
	}
	...
	if (st->host->peerstore && target)
	{
		char pid_buf[128];
		size_t pid_len = 0U;
		if (peer_id_text_write(target, PEER_ID_TEXT_LEGACY_BASE58, pid_buf, sizeof(pid_buf), &pid_len) !=
		    PEER_ID_OK)
			snprintf(pid_buf, sizeof(pid_buf), "<unknown>");
		LIBP2P_TRACE("identify_push", "apply target=%s remote=%p protocols=%zu", pid_buf, (void *)remote,
			     id->num_protocols);
		if (id->num_protocols && id->protocols)
		{
			if (libp2p_peerstore_set_protocols(st->host->peerstore, target,
							   (const char *const *)id->protocols, id->num_protocols) == 0)
			{
				libp2p__notify_peer_protocols_updated(
					st->host, target, (const char *const *)id->protocols, id->num_protocols);
				LIBP2P_TRACE("identify_push", "protocols updated count=%zu", id->num_protocols);
			}
		}

		if (id->public_key && id->public_key_len)
			(void)libp2p_peerstore_set_public_key(st->host->peerstore, target, id->public_key,
							      id->public_key_len);
...
static void *identify_push_thread(void *arg)
{
	...
	while (1)
	{
		...
		if (n > 0)
		{
			...
			LIBP2P_TRACE("identify_push", "read bytes=%zd header_done=%d need=%llu got=%zu stream=%p", n,
				     st->header_done, (unsigned long long)st->need, st->got, (void *)s);
			if (st->done)
				break;
			continue;
		}
...
int libp2p_identify_encode_local(struct libp2p_host *host, libp2p_stream_t *s, int include_observed, uint8_t **out_buf,
				 size_t *out_len)
{
	...
	if (host->have_identity && host->identity_key && host->identity_key_len > 0)
	{
		...
		if (key_valid)
		{
			peer_id_error_t perr = peer_id_public_key_pb_from_private_raw(
				key_type, host->identity_key, host->identity_key_len, &pubkey_pb, &pubkey_pb_len);
			if (perr == PEER_ID_OK && pubkey_pb)
			{
		...
		if (ln->lst && libp2p_listener_local_addr(ln->lst, &ma) == LIBP2P_LISTENER_OK && ma)
		{
			...
			if (wrote == MULTIADDR_ERR_BUFFER_TOO_SMALL)
			{
				...
				if (heap)
				{
					...
					if (w2 > 0)
					{
						uint8_t **new_addrs = (uint8_t **)realloc(msg.listen_addrs,
											  (msg.num_listen_addrs + 1) *
												  sizeof(uint8_t *));
						size_t *new_lens =
							(size_t *)realloc(msg.listen_addrs_lens,
									  (msg.num_listen_addrs + 1) * sizeof(size_t));
						if (new_addrs && new_lens)
						{
							msg.listen_addrs = new_addrs;
							msg.listen_addrs_lens = new_lens;
							msg.listen_addrs[msg.num_listen_addrs] =
								(uint8_t *)malloc((size_t)w2);
							if (msg.listen_addrs[msg.num_listen_addrs])
							{
								memcpy(msg.listen_addrs[msg.num_listen_addrs], heap,
								       (size_t)w2);
								msg.listen_addrs_lens[msg.num_listen_addrs] =
									(size_t)w2;
			...
			{
				uint8_t **new_addrs = (uint8_t **)realloc(msg.listen_addrs, (msg.num_listen_addrs + 1) *
												    sizeof(uint8_t *));
				size_t *new_lens = (size_t *)realloc(msg.listen_addrs_lens,
								     (msg.num_listen_addrs + 1) * sizeof(size_t));
				if (new_addrs && new_lens)
				{
					msg.listen_addrs = new_addrs;
