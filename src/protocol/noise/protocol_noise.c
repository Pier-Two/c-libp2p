...
static int build_handshake_payload(struct libp2p_noise_ctx *ctx, uint8_t **out, size_t *out_len)
{
	...
	peer_id_error_t perr = PEER_ID_ERR_INVALID_INPUT;
	peer_id_key_type_t key_type = PEER_ID_KEY_RSA;
	uint8_t ed25519_pub[32];
	int have_ed25519_pub = 0;

	if (ctx->identity_type == PEER_ID_KEY_ED25519)
	{
		key_type = PEER_ID_KEY_ED25519;
		ed25519_genpub(ed25519_pub, ctx->identity_key);
		have_ed25519_pub = 1;
	}
	else if (ctx->identity_type == PEER_ID_KEY_SECP256K1)
	{
		key_type = PEER_ID_KEY_SECP256K1;
	}
	else if (ctx->identity_type == PEER_ID_KEY_RSA)
	{
		key_type = PEER_ID_KEY_RSA;
	}
	else if (ctx->identity_type == PEER_ID_KEY_ECDSA)
	{
		key_type = PEER_ID_KEY_ECDSA;
	}
	else
	{
		return -1;
	...
	perr = peer_id_public_key_pb_from_private_raw(key_type, ctx->identity_key, ctx->identity_key_len, &pubkey_pb,
						      &pubkey_pb_len);
	if (perr != PEER_ID_OK)
		return -1;
	...
	if (ctx->identity_type == PEER_ID_KEY_ED25519)
	{
		...
		eddsa_sign(signature, ctx->identity_key, ed25519_pub, to_sign, sizeof(to_sign));
		sig_len = 64;
		pthread_mutex_unlock(&sign_mutex);
	}
	else if (ctx->identity_type == PEER_ID_KEY_SECP256K1)
	...
	else if (ctx->identity_type == PEER_ID_KEY_RSA)
	{
		...
		if (rsa_sign_hash_ex(hash.bytes, sizeof(hash.bytes), signature, &temp_sig_len, LTC_PKCS_1_V1_5, NULL, 0,
				     sha_idx, 0, &rsa) != CRYPT_OK)
		{
			rsa_free(&rsa);
			free(signature);
			free(pubkey_pb);
	...
	else if (ctx->identity_type == PEER_ID_KEY_ECDSA)
	{
		...
		if (ecc_sign_hash(hash.bytes, sizeof(hash.bytes), signature, &temp_sig_len, &prng, prng_idx, &ecdsa) !=
		    CRYPT_OK)
		{
			ecc_free(&ecdsa);
			free(signature);
...
static int verify_handshake_payload(NoiseHandshakeState *hs, const uint8_t *payload, size_t payload_len,
				    peer_id_t **out_peer, uint8_t **out_ed, size_t *out_ed_len, uint8_t **out_ext,
				    size_t *out_ext_len)
{
	...
	if (unsigned_varint_decode(payload + offset, payload_len - offset, &hdr, &len_sz) != UNSIGNED_VARINT_OK ||
	    hdr != 0x0A || !varint_is_minimal(hdr, len_sz))
	{
		return -1;
	}
	offset += len_sz;
	...
	if (unsigned_varint_decode(payload + offset, payload_len - offset, &hdr, &len_sz) != UNSIGNED_VARINT_OK ||
	    hdr != 0x12 || !varint_is_minimal(hdr, len_sz))
	{
		return -1;
	}
	offset += len_sz;
	...
	if (unsigned_varint_decode(payload + offset, payload_len - offset, &fld_len, &len_sz) != UNSIGNED_VARINT_OK ||
	    fld_len == 0 || fld_len > payload_len - offset - len_sz || !varint_is_minimal(fld_len, len_sz))
	{
		return -1;
	}
	offset += len_sz;
	...
	if (key_type == PEER_ID_KEY_ED25519)
	{
	...
	else if (key_type == PEER_ID_KEY_SECP256K1)
	{
		SHA256_HASH hash;
		Sha256Calculate(to_sign, sizeof(to_sign), &hash);
	...
	else if (key_type == PEER_ID_KEY_RSA)
	{
		SHA256_HASH hash;
		Sha256Calculate(to_sign, sizeof(to_sign), &hash);
		rsa_key rsa;
		...
		int sha_idx = find_hash("sha256");
		int stat = 0;
		int rc = rsa_verify_hash_ex(sig, (unsigned long)fld_len, hash.bytes, sizeof(hash.bytes),
					    LTC_PKCS_1_V1_5, sha_idx, 0, &stat, &rsa);
		rsa_free(&rsa);
		if (rc != CRYPT_OK || stat == 0)
			return -1;
	}
	else if (key_type == PEER_ID_KEY_ECDSA)
	{
		SHA256_HASH hash;
		Sha256Calculate(to_sign, sizeof(to_sign), &hash);
		ecc_key ecdsa;
	...
	if (offset < payload_len &&
	    unsigned_varint_decode(payload + offset, payload_len - offset, &hdr, &len_sz) == UNSIGNED_VARINT_OK &&
	    hdr == 0x22 && varint_is_minimal(hdr, len_sz))
	{
		offset += len_sz;
		if (unsigned_varint_decode(payload + offset, payload_len - offset, &fld_len, &len_sz) !=
			    UNSIGNED_VARINT_OK ||
		    fld_len > payload_len - offset - len_sz || !varint_is_minimal(fld_len, len_sz))
			return -1;
		offset += len_sz;
	...
	{
		peer_id_t *pid = NULL;
		if (peer_id_new_from_public_key_pb(id_key, id_key_len, &pid) != PEER_ID_OK)
			return -1;
		*out_peer = pid;
	}
...
static libp2p_security_err_t noise_secure_outbound(libp2p_security_t *self, libp2p_conn_t *raw,
						   const peer_id_t *remote_hint, uint64_t timeout_ms,
						   libp2p_conn_t **out, peer_id_t **remote_peer)
{
	...
	for (;;)
	{
		...
		else if (action == NOISE_ACTION_READ_MESSAGE)
		{
			...
			{
				err = verify_handshake_payload(hs, pbuf.data, pbuf.size, remote_peer, &remote_ed,
							       &remote_ed_len, &remote_ext, &remote_ext_len);
				if (err == 0 && remote_ext_len > 0)
				{
					if (parse_noise_extensions(remote_ext, remote_ext_len, &parsed_ext) != 0)
						err = -1;
				}
				if (err == 0 && remote_hint && remote_peer && *remote_peer &&
				    peer_id_equal(remote_hint, *remote_peer) != 1)
				{
					hint_mismatch = 1;
				}
			}
	...
	free(payload);
	if (hint_mismatch ||
	    (remote_hint && remote_peer && *remote_peer && peer_id_equal(remote_hint, *remote_peer) != 1))
	{
	...
	libp2p_conn_t *secure = make_noise_conn(raw, send_cs, recv_cs, ctx->max_plaintext, remote_ed, remote_ed_len,
						remote_ext, remote_ext_len, parsed_ext);
	if (!secure)
	{
		noise_cipherstate_free(send_cs);
		noise_cipherstate_free(recv_cs);
...
static libp2p_security_err_t noise_secure_inbound(libp2p_security_t *self, libp2p_conn_t *raw, uint64_t timeout_ms,
						  libp2p_conn_t **out, peer_id_t **remote_peer)
{
	if (!self || !raw || !out)
		return LIBP2P_SECURITY_ERR_NULL_PTR;
	...
	for (;;)
	{
		...
		else if (action == NOISE_ACTION_READ_MESSAGE)
		{
			...
			if (err == NOISE_ERROR_NONE)
			{
				...
				{
					err = verify_handshake_payload(hs, pbuf.data, pbuf.size, remote_peer,
								       &remote_ed, &remote_ed_len, &remote_ext,
								       &remote_ext_len);
					if (err == 0 && remote_ext_len > 0)
					{
						if (parse_noise_extensions(remote_ext, remote_ext_len, &parsed_ext) !=
						    0)
							err = -1;
					}
				}
