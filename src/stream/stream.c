...
static stream_stub_t *S(libp2p_stream_t *s)
{
	return (stream_stub_t *)s;
}
static const stream_stub_t *SC(const libp2p_stream_t *s)
{
	return (const stream_stub_t *)s;
}
...
ssize_t libp2p_stream_write(libp2p_stream_t *s, const void *buf, size_t len)
{
	...
	if (st->has_ops && st->ops.write)
	{
		...
		if (n > 0 && st->host && st->host->metrics)
		{
			...
			libp2p_metrics_observe_histogram(st->host->metrics, "libp2p_stream_write_bytes", labels,
							 (double)n);
		}
		return n;
	}
...
ssize_t libp2p_stream_writev(libp2p_stream_t *s, const struct iovec *iov, int iovcnt)
{
	...
	for (int i = 0; i < iovcnt; i++)
	{
		...
		while (remain > 0)
		{
			ssize_t n = st->has_ops && st->ops.write
					    ? st->ops.write(st->io_ctx, base, remain)
...
void libp2p_stream_free(libp2p_stream_t *s)
{
	...
	int expected = 0;
	if (!atomic_compare_exchange_strong_explicit(&st->destroy_state, &expected, 1, memory_order_acq_rel,
...
ssize_t libp2p_stream_read(libp2p_stream_t *s, void *buf, size_t len)
{
	...
	if (st->has_ops && st->ops.read)
	{
		...
		if (n > 0 && st->host && st->host->metrics)
		{
			...
			libp2p_metrics_observe_histogram(st->host->metrics, "libp2p_stream_read_bytes", labels,
							 (double)n);
		}
		return n;
	}
...
libp2p_stream_t *libp2p_stream_from_conn(struct libp2p_host *host, libp2p_conn_t *c, const char *protocol_id,
...
void libp2p_stream_set_parent(libp2p_stream_t *s, libp2p_conn_t *parent_conn, struct libp2p_muxer *mx,
			      int take_ownership)
{
	stream_stub_t *st = S(s);
	if (!st)
...
void libp2p__stream_mark_deferred(libp2p_stream_t *s)
{
	...
	if (!st)
		return;
	int expected = 0;
	while (!atomic_compare_exchange_weak_explicit(&st->defer_destroy, &expected, 1, memory_order_release,
						      memory_order_acquire))
	{
		if (expected != 0)
			return;
...
int libp2p__stream_retain_async(libp2p_stream_t *s)
{
	...
	if (state != 0)
	{
		...
		{
			int expected = 1;
			if (atomic_compare_exchange_strong_explicit(&st->destroy_state, &expected, 2,
...
int libp2p__stream_release_async(libp2p_stream_t *s)
{
	...
	{
		int expected = 1;
		if (atomic_compare_exchange_strong_explicit(&st->destroy_state, &expected, 2, memory_order_acq_rel,
							    memory_order_acquire))
		{
			libp2p__stream_destroy(s);
			return 1;
...
libp2p_stream_t *libp2p_stream_from_ops(struct libp2p_host *host, void *io_ctx, const libp2p_stream_backend_ops_t *ops,
