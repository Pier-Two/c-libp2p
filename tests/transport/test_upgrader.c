...
static int peer_id_from_ed25519_priv(const uint8_t *sk, peer_id_t **pid)
{
	uint8_t *pubpb = NULL;
	size_t pubpb_len = 0;
	if (peer_id_public_key_pb_from_private_raw(PEER_ID_KEY_ED25519, sk, 32, &pubpb, &pubpb_len) != PEER_ID_OK)
		return -1;
...
static void test_upgrade_handshake(void)
{
	...
	peer_id_t *pid_cli = NULL;
	peer_id_t *pid_srv = NULL;
	...
	TEST_OK("upgrade return codes", cli_args.rc == LIBP2P_UPGRADER_OK && srv_args.rc == LIBP2P_UPGRADER_OK,
	...
	TEST_OK("remote peer verified",
		cli_args.out && cli_args.out->remote_peer && peer_id_equal(cli_args.out->remote_peer, pid_srv) == 1,
		"match=%d",
		cli_args.out && cli_args.out->remote_peer ? peer_id_equal(cli_args.out->remote_peer, pid_srv) : -1);
	TEST_OK("server saw client",
		srv_args.out && srv_args.out->remote_peer && peer_id_equal(srv_args.out->remote_peer, pid_cli) == 1,
		"match=%d",
		srv_args.out && srv_args.out->remote_peer ? peer_id_equal(srv_args.out->remote_peer, pid_cli) : -1);
	...
	if (ctx_c && ctx_s)
	{
		...
		recv_state_t rs = {0};
		libp2p_mplex_event_callbacks_t cbs = {
			.on_stream_event = on_stream_event_cb, .on_error = NULL, .user_data = &rs};
		...
		TEST_OK("stream open",
			libp2p_mplex_stream_open(ctx_c, (const uint8_t *)"s", 1, &stream) == LIBP2P_MPLEX_OK, "open");
		TEST_OK("stream send", libp2p_mplex_stream_write_async(stream, (const uint8_t *)"ping", 4) >= 0,
			"send");
		...
		TEST_OK("data after upgrade", rs.got && rs.n == 4 && memcmp(rs.buf, "ping", 4) == 0, "got=%d n=%zu",
			rs.got, rs.n);
	...
	if (cli_args.out)
	{
		...
		{
			peer_id_free(cli_args.out->remote_peer);
		}
		free(cli_args.out);
	}
	if (srv_args.out)
	{
		...
		{
			peer_id_free(srv_args.out->remote_peer);
		}
		free(srv_args.out);
	}
	...
	peer_id_free(pid_cli);
	peer_id_free(pid_srv);
}
