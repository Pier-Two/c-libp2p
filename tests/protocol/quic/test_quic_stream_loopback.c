...
static const uint16_t VERIFY_ALGOS[] = {PTLS_SIGNATURE_ED25519, PTLS_SIGNATURE_RSA_PSS_RSAE_SHA256,
...
static int quic_verify_cb(struct st_ptls_verify_certificate_t *self, ptls_t *tls, const char *server_name,
...
static int quic_server_loop_cb(picoquic_quic_t *quic, picoquic_packet_loop_cb_enum cb_mode, void *callback_ctx,
			       void *callback_arg)
{
	(void)quic;
...
static int quic_server_stream_cb(picoquic_cnx_t *cnx, uint64_t stream_id, uint8_t *bytes, size_t length,
...
static peer_id_t *derive_peer_id_from_ed25519(const uint8_t *sk, size_t len)
{
	uint8_t *pub_pb = NULL;
	size_t pub_len = 0;
	if (peer_id_public_key_pb_from_private_raw(PEER_ID_KEY_ED25519, sk, len, &pub_pb, &pub_len) != PEER_ID_OK)
		return NULL;
...
static int wait_for_ready(libp2p_quic_session_t *session, quic_server_ctx_t *server_ctx, picoquic_cnx_t *cnx,
			  uint64_t timeout_ms)
{
...
static const uint8_t SERVER_ID_KEY[32] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb,
					  0xcc, 0xdd, 0xee, 0xff, 0x10, 0x21, 0x32, 0x43, 0x54, 0x65, 0x76,
					  0x87, 0x98, 0xa9, 0xba, 0xcb, 0xdc, 0xed, 0xfe, 0x0f, 0x1f};

static const uint8_t CLIENT_ID_KEY[32] = {0xfe, 0xed, 0xdc, 0xcb, 0xba, 0xa9, 0x98, 0x87, 0x76, 0x65, 0x54,
					  0x43, 0x32, 0x21, 0x10, 0x00, 0x0f, 0x1e, 0x2d, 0x3c, 0x4b, 0x5a,
...
int main(void)
{
	...
	server_opts.identity_key_type = PEER_ID_KEY_ED25519;
	...
	client_opts.identity_key_type = PEER_ID_KEY_ED25519;
	...
	CHECK(libp2p_quic_tls_identity_from_certificate(server_cert.cert_der, server_cert.cert_len, &server_ident) == 0,
	      "server cert parse");
	...
	CHECK(libp2p_quic_tls_identity_from_certificate(client_cert.cert_der, client_cert.cert_len, &client_ident) == 0,
	      "client cert parse");
	...
	CHECK(write_der_cert_to_pem(server_cert.cert_der, server_cert.cert_len, server_cert_path,
				    sizeof(server_cert_path)) == 0,
	      "write server cert");
	server_cert_created = 1;
	CHECK(write_der_key_to_pem(server_cert.key_der, server_cert.key_len, server_key_path,
				   sizeof(server_key_path)) == 0,
	      "write server key");
	server_key_created = 1;
	CHECK(write_der_cert_to_pem(client_cert.cert_der, client_cert.cert_len, client_cert_path,
				    sizeof(client_cert_path)) == 0,
	      "write client cert");
	client_cert_created = 1;
	CHECK(write_der_key_to_pem(client_cert.key_der, client_cert.key_len, client_key_path,
				   sizeof(client_key_path)) == 0,
	      "write client key");
	client_key_created = 1;

	picoquic_quic_t *server_q =
		picoquic_create(8, server_cert_path, server_key_path, NULL, LIBP2P_QUIC_TLS_ALPN, quic_server_stream_cb,
				&server_ctx, NULL, NULL, NULL, picoquic_current_time(), NULL, NULL, NULL, 0);
	...
	int loop_ret = 0;
	picoquic_network_thread_ctx_t *thread_ctx = picoquic_start_network_thread(
		server_q, &server_ctx.loop_param, quic_server_loop_cb, &server_ctx, &loop_ret);
	...
	if (probe_fd >= 0)
	{
		...
		(void)sendto(probe_fd, probe_msg, sizeof(probe_msg), 0, (struct sockaddr *)&server_addr,
			     sizeof(server_addr));
		close(probe_fd);
	}
#endif
	...
	picoquic_quic_t *client_q =
		picoquic_create(8, client_cert_path, client_key_path, NULL, LIBP2P_QUIC_TLS_ALPN, NULL, NULL, NULL,
	...
	picoquic_cnx_t *client_cnx = picoquic_create_cnx(
		client_q, picoquic_null_connection_id, picoquic_null_connection_id, (struct sockaddr *)&server_addr,
		picoquic_current_time(), 0, "localhost", LIBP2P_QUIC_TLS_ALPN, 1);
	...
	libp2p_conn_t *conn =
		libp2p_quic_conn_new(local_ma, remote_ma, session, test_session_close, test_session_free, NULL);
	...
	const uint8_t *actual_bytes = NULL;
	size_t actual_mh_len = 0;
	(void)peer_id_multihash_view(client_verify.peer, &actual_bytes, &actual_mh_len);
	fprintf(stderr, "actual bytes: ");
	for (size_t i = 0; i < actual_mh_len; i++)
		fprintf(stderr, "%02x", actual_bytes[i]);
	fprintf(stderr, "\n");
	int peer_cmp = peer_id_equal(client_verify.peer, expected_server);
	char expected_str[256] = {0};
	char actual_str[256] = {0};
	size_t expected_len = 0U;
	if (peer_id_text_write(expected_server, PEER_ID_TEXT_LEGACY_BASE58, expected_str, sizeof(expected_str),
			       &expected_len) != PEER_ID_OK)
		strcpy(expected_str, "<err>");
	size_t actual_len = 0U;
	if (peer_id_text_write(client_verify.peer, PEER_ID_TEXT_LEGACY_BASE58, actual_str, sizeof(actual_str),
			       &actual_len) != PEER_ID_OK)
		strcpy(actual_str, "<err>");
	fprintf(stderr, "expected peer=%s\n", expected_str);
	fprintf(stderr, "actual peer=%s\n", actual_str);
	const uint8_t *expected_bytes = NULL;
	size_t expected_len_bytes = 0;
	(void)peer_id_multihash_view(expected_server, &expected_bytes, &expected_len_bytes);
	fprintf(stderr, "expected bytes: ");
	for (size_t i = 0; i < expected_len_bytes; i++)
		fprintf(stderr, "%02x", expected_bytes[i]);
	fprintf(stderr, "\n");
	...
	CHECK(libp2p_stream_write(stream1, (const uint8_t *)RESET_TAG, sizeof(RESET_TAG) - 1) ==
		      (ssize_t)(sizeof(RESET_TAG) - 1),
	      "write reset");
...
cleanup:
	...
	if (expected_client)
	{
		peer_id_free(expected_client);
	}
	if (expected_server)
	{
		peer_id_free(expected_server);
	}
