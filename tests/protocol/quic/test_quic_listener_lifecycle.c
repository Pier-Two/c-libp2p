...
int main(void)
{
	...
	TEST_CHECK("Server identity setup", libp2p_quic_transport_set_identity(server, &server_opts) == 0,
		   "set identity failed");
	...
	TEST_CHECK("Client identity setup", libp2p_quic_transport_set_identity(client, &client_opts) == 0,
		   "set identity failed");
	...
	if (listener)
	{
		...
		TEST_CHECK("Retrieve bound address", laddr_rc == LIBP2P_LISTENER_OK && bound_addr != NULL,
			   "local addr rc=%d", laddr_rc);
		if (bound_addr)
		{
			int s_err = 0;
	...
	TEST_CHECK("Client dial after certificate rotation", dial_rc == LIBP2P_TRANSPORT_OK && client_conn != NULL,
		   "dial rc=%d", dial_rc);

	acc_rc = accept_with_backoff(listener, &server_conn);
	TEST_CHECK("Server accept after certificate rotation", acc_rc == LIBP2P_LISTENER_OK && server_conn != NULL,
		   "accept rc=%d", acc_rc);
	...
	{
		int prc = libp2p_quic_conn_copy_verified_peer(client_conn, &client_peer);
		TEST_CHECK("Client verified peer after rotation", prc == LIBP2P_ERR_OK && client_peer != NULL, "rc=%d",
			   prc);
	}
	if (client_peer)
	{
	...
	{
		int prc = libp2p_quic_conn_copy_verified_peer(server_conn, &server_peer);
		TEST_CHECK("Server verified peer after rotation", prc == LIBP2P_ERR_OK && server_peer != NULL, "rc=%d",
			   prc);
	}
	if (server_peer)
	{
