...
static int test_roundtrip(void)
{
	...
	const uint8_t from_bytes[] = {0x00, 0x02, 0x12, 0x34};
	peer_id_t *from = NULL;
...
static int test_decoder_handles_chunked_frames(void)
{
	...
	const uint8_t from_bytes[] = {0x00, 0x03, 0x99, 0x88, 0x77};
	peer_id_t *from = NULL;
	if (peer_id_new_from_multihash(from_bytes, sizeof(from_bytes), &from) != PEER_ID_OK)
		return 0;
	...
	for (size_t i = 0; i < slices_count && pos < total; ++i)
	{
		...
		if (take == 0)
			break;
		libp2p_err_t rc =
			libp2p_gossipsub_rpc_decoder_feed(&decoder, wire + pos, take, decoder_capture_cb, &ctx);
		if (rc != LIBP2P_ERR_OK)
		{
			ctx.ok = 0;
			break;
...
static int test_control_idontwant_roundtrip(void)
{
	...
	libp2p_gossipsub_ControlIDontWant *decoded_idw =
		libp2p_gossipsub_ControlMessage_get_at_idontwant(decoded_ctrl, 0);
	if (!decoded_idw)
		goto cleanup;
