...
static void test_message_cache_put_and_find(void)
{
	...
	assert(gossipsub_message_cache_put(&cache, id1, sizeof(id1), "topic-a", frame1, sizeof(frame1)) ==
	       LIBP2P_ERR_OK);
...
static void test_message_cache_shift_eviction(void)
{
	...
	assert(gossipsub_message_cache_put(&cache, id1, sizeof(id1), "topic-b", frame1, sizeof(frame1)) ==
	       LIBP2P_ERR_OK);
	assert(gossipsub_message_cache_find(&cache, id1, sizeof(id1)) != NULL);
...
static void test_message_cache_collect_ids_filter(void)
{
	...
	assert(gossipsub_message_cache_put(&cache, id_a1, sizeof(id_a1), "topic-a", frame, sizeof(frame)) ==
	       LIBP2P_ERR_OK);
	assert(gossipsub_message_cache_put(&cache, id_a2, sizeof(id_a2), "topic-a", frame, sizeof(frame)) ==
	       LIBP2P_ERR_OK);
	assert(gossipsub_message_cache_put(&cache, id_b1, sizeof(id_b1), "topic-b", frame, sizeof(frame)) ==
	       LIBP2P_ERR_OK);
...
static void test_message_cache_collect_ids_round_gate(void)
{
	...
	assert(gossipsub_message_cache_put(&cache, msg_id, sizeof(msg_id), "topic-x", frame, sizeof(frame)) ==
	       LIBP2P_ERR_OK);
...
static void test_message_cache_pending_entries_are_not_gossiped(void)
{
	...
	assert(gossipsub_message_cache_put_pending(&cache, msg_id, sizeof(msg_id), "topic-p", frame, sizeof(frame),
						   NULL) == LIBP2P_ERR_OK);
