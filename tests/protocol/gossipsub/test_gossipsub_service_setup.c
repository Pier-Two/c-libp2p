...
int gossipsub_service_run_setup(gossipsub_service_test_env_t *env)
{
	...
	int overlay_defaults_ok = default_ok && default_cfg.d == 6 && default_cfg.d_lo == 5 && default_cfg.d_hi == 12 &&
				  default_cfg.d_lazy == 6 && default_cfg.d_out == 2 && default_cfg.d_score == 4 &&
	...
	if (!overlay_defaults_ok)
	{
		...
		if (default_ok)
		{
			printf("expected overlay defaults d=6 d_lo=5 d_hi=12 d_lazy=6 d_out=2 d_score=4 "
			       "gossip_factor=25,"
			       " got d=%d d_lo=%d d_hi=%d d_lazy=%d d_out=%d d_score=%d gossip_factor=%d\n",
			       default_cfg.d, default_cfg.d_lo, default_cfg.d_hi, default_cfg.d_lazy, default_cfg.d_out,
			       default_cfg.d_score, default_cfg.gossip_factor_percent);
		}
	}
	...
	int ihave_defaults_ok =
		default_ok && default_cfg.max_ihave_messages == 10 && default_cfg.max_ihave_length == 5000;
	...
	if (!ihave_defaults_ok)
	{
		...
		if (default_ok)
		{
			printf("expected max_ihave_messages=10 max_ihave_length=5000, got %zu and %zu\n",
			       default_cfg.max_ihave_messages, default_cfg.max_ihave_length);
		}
	}
	...
	libp2p_gossipsub_topic_config_t topic_cfg = {
		...
		.descriptor = {.struct_size = sizeof(topic_cfg.descriptor), .topic = "test/topic"},
		.score_params = NULL};
	...
	if (ok)
	{
		...
		libp2p_gossipsub_message_t msg = {.topic = {.struct_size = sizeof(msg.topic), .topic = "test/topic"},
						  .data = payload,
