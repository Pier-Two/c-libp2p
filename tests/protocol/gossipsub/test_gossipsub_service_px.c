...
int gossipsub_service_run_px_and_opportunistic_tests(gossipsub_service_test_env_t *env)
{
...

	{
		...
		libp2p_gossipsub_topic_config_t px_cfg = {
			...
			.descriptor = {.struct_size = sizeof(px_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
		...
		const char *peer_strs[] = {"12D3KooWExgVnyL9F9ktsHTTXV9cZ6rxWPPPwJ9V6u3yyFvEfYst",
		...
		if (subscribe_ok && peers_created)
		{
			...
			if (setup_ok)
			{
				...
				int target_in_mesh = libp2p_gossipsub__topic_mesh_contains(
					gs, topic_name, peers[target_index], &target_outbound, NULL);
				...
				if (target_pruned)
				{
					...
					while (1)
					{
						...
						libp2p_err_t pop_rc = libp2p_gossipsub__peer_pop_sendq(
							gs, peers[target_index], &frame, &frame_len);
						...
						if (!prune_found)
							prune_found = decode_prune_px_count(frame, frame_len,
											    topic_name, &px_count);
						free(frame);
						if (prune_found)
							break;
...

	{
		...
		const char *peer_strs[] = {"12D3KooWNNK9n7fh5R7sFjstqA5H1vcs1PEqnbPGZL4sD7dJgxX8",
...

	{
		...
		libp2p_gossipsub_topic_config_t px_ingest_cfg = {
			...
			.descriptor = {.struct_size = sizeof(px_ingest_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
		...
		peer_id_error_t px_pb_rc = peer_id_build_public_key_protobuf(
			PEER_ID_KEY_ED25519, px_public, sizeof(px_public), &px_pub_pb, &px_pub_pb_len);
		...
		if (sender_ok && px_peer_ok && addr_ok)
		{
			...
			size_t record_len = 0;
			libp2p_err_t rec_rc =
				encode_signed_peer_record(px_peer, addr_list, 1, kTestPxSecretKey,
							  sizeof(kTestPxSecretKey), &record_buf, &record_len);
			...
			if (record_ok)
			{
				...
				size_t prune_len = 0;
				libp2p_err_t prune_rc = encode_prune_px_rpc(topic_name, px_peer, record_buf, record_len,
									    &prune_frame, &prune_len);
				...
				{
					libp2p_err_t inj_rc =
						libp2p_gossipsub__inject_frame(gs, sender_peer, prune_frame, prune_len);
					...
					if (inject_ok && host && host->peerstore)
					{
						...
						size_t stored_len = 0;
						int ps_rc = libp2p_peerstore_get_addrs(host->peerstore, px_peer,
										       &stored, &stored_len);
						...
						else
						{
							...
							for (size_t i = 0; i < stored_len; ++i)
							{
								...
								if (addr_str && str_err == MULTIADDR_SUCCESS &&
								    strcmp(addr_str, px_addr_str) == 0)
									match_found = 1;
								if (addr_str)
									free(addr_str);
								if (match_found)
...

	{
		...
		libp2p_gossipsub_topic_config_t gossip_cfg = {
			...
			.descriptor = {.struct_size = sizeof(gossip_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
		...
		const char *mesh_peer_strs[] = {"12D3KooWHZjVdysJ8V5Y2Tyshzw31wY1M2fjTw83YVHC6rU1ttzv",
						"12D3KooWQX1pP6uPQ7RZicMv6z4dGYBHc9B7iKLB9gowgCJFzQEw",
						"12D3KooWDbSkFwsij4BjjHfZxQqJ1zuvBABFqQ5uwSX6ZiUvUv9d",
						"12D3KooWN9oSkqZSS7Y7gsnAmfmNgmcByKYEzGyv1mCXN8vQiyTe"};
		...
		if (subscribe_ok && mesh_peers_ok && gossip_peer_ok)
		{
			...
			for (size_t i = 0; i < mesh_count && setup_ok; ++i)
			{
				...
				libp2p_err_t mesh_rc =
					libp2p_gossipsub__topic_mesh_add_peer(gs, topic_name, mesh_peers[i], 1);
			...
			if (setup_ok)
			{
				...
				libp2p_gossipsub_message_t msg = {
					.topic = {.struct_size = sizeof(msg.topic), .topic = topic_name},
					.data = payload,
				...
				if (publish_ok)
				{
					...
					for (int attempt = 0; attempt < 100 && !mesh_queues_ready; ++attempt)
					{
						...
						{
							size_t qlen =
								libp2p_gossipsub__peer_sendq_len(gs, mesh_peers[i]);
							if (qlen == 0)
							{
								mesh_queues_ready = 0;
								break;
					...
					int gossip_in_mesh = libp2p_gossipsub__topic_mesh_contains(
						gs, topic_name, gossip_peer, NULL, NULL);
					...
					if (pop_rc == LIBP2P_ERR_OK && frame && frame_len)
					{
						...
						if (libp2p_gossipsub_rpc_decode_frame(frame, frame_len, &rpc) ==
							    LIBP2P_ERR_OK &&
						    rpc)
						{
							if (libp2p_gossipsub_RPC_has_control(rpc))
							{
								libp2p_gossipsub_ControlMessage *control =
									libp2p_gossipsub_RPC_get_control(rpc);
								if (control &&
								    libp2p_gossipsub_ControlMessage_has_ihave(
									    control) &&
								    libp2p_gossipsub_ControlMessage_count_ihave(
									    control) > 0)
								{
									libp2p_gossipsub_ControlIHave *ihave =
										libp2p_gossipsub_ControlMessage_get_at_ihave(
											control, 0);
									if (ihave &&
									    libp2p_gossipsub_ControlIHave_has_topic(
										    ihave) &&
									    libp2p_gossipsub_ControlIHave_count_message_ids(
										    ihave) > 0)
									{
										size_t len =
											libp2p_gossipsub_ControlIHave_get_size_topic(
												ihave);
										const char *raw =
											libp2p_gossipsub_ControlIHave_get_topic(
												ihave);
										if (raw && len == strlen(topic_name) &&
										    memcmp(raw, topic_name, len) == 0)
											ihave_seen = 1;
									}
								}
							}
...

	{
		...
		libp2p_gossipsub_topic_config_t topic_cfg = {
			...
			.descriptor = {.struct_size = sizeof(topic_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
		...
		static const char *const gossip_ids_lazy[] = {"12D3KooWQezYB7Lr9n7xw3v63ogpDkC6BasuCCJJksDkD6ZrB8Sx",
		...
		static const char *const gossip_ids_extra[] = {"12D3KooWExgVnyL9F9ktsHTTXV9cZ6rxWPPPwJ9V6u3yyFvEfYst",
							       "12D3KooWFjecZx2YM5mAZ1bn46vCeWkQS9KpVwhM2r36EJt2vqCr",
							       "12D3KooWDH8u1o1YiA1HXSwSReT7PwtZDs7JhdYKbnvSYnUrWhp2",
		...
		enum
		{
			LAZY_COUNT = sizeof(gossip_ids_lazy) / sizeof(gossip_ids_lazy[0])
		};
		enum
		{
			EXTRA_COUNT = sizeof(gossip_ids_extra) / sizeof(gossip_ids_extra[0])
		};
		enum
		{
			TOTAL_COUNT = LAZY_COUNT + EXTRA_COUNT
		};
		...
		if (gossip_setup_ok)
		{
			...
			size_t expected_lazy =
				compute_expected_gossip_targets(lazy_count, config_gossip_percent, config_d_lazy);
			const uint8_t payload_lazy[] = {0xA1, 0xB2, 0xC3, 0xD4};
			size_t lazy_selected = 0;
			size_t lazy_limit = 0;
			(void)run_gossip_factor_scenario(local_gs, topic_name, gossip_peers, lazy_count, payload_lazy,
							 sizeof(payload_lazy), expected_lazy, &lazy_selected,
							 &lazy_limit);
			size_t actual_expected_lazy = compute_expected_gossip_targets(
				gossipsub_debug_last_eligible, config_gossip_percent, config_d_lazy);
			...
			if (!lazy_scenario_ok)
			{
				...
				printf("DETAIL: gossip_factor_lazy limit=%zu eligible=%zu recomputed=%zu\n", lazy_limit,
				       gossipsub_debug_last_eligible, actual_expected_lazy);
				for (size_t i = 0; i < lazy_count; ++i)
				{
					...
					if (qlen)
					{
						...
						size_t frame_len = 0;
						if (libp2p_gossipsub__peer_pop_sendq(local_gs, gossip_peers[i],
										     &frame_buf,
										     &frame_len) == LIBP2P_ERR_OK)
						{
							libp2p_gossipsub_RPC *rpc = NULL;
							if (libp2p_gossipsub_rpc_decode_frame(frame_buf, frame_len,
											      &rpc) == LIBP2P_ERR_OK &&
							    rpc)
							{
		...
		if (gossip_setup_ok && lazy_scenario_ok && extra_setup_ok)
		{
			int config_d_lazy = local_cfg.d_lazy;
			int config_gossip_percent = local_cfg.gossip_factor_percent;
			size_t expected_factor =
				compute_expected_gossip_targets(total_count, config_gossip_percent, config_d_lazy);
			const uint8_t payload_percent[] = {0x11, 0x22, 0x33, 0x44};
			size_t factor_selected = 0;
			size_t factor_limit = 0;
			(void)run_gossip_factor_scenario(local_gs, topic_name, gossip_peers, total_count,
							 payload_percent, sizeof(payload_percent), expected_factor,
							 &factor_selected, &factor_limit);
			size_t actual_expected_factor = compute_expected_gossip_targets(
				gossipsub_debug_last_eligible, config_gossip_percent, config_d_lazy);
			...
			if (!factor_scenario_ok)
			{
				...
				printf("DETAIL: gossip_factor_percent limit=%zu eligible=%zu recomputed=%zu\n",
				       factor_limit, gossipsub_debug_last_eligible, actual_expected_factor);
			...
			if (mesh_add_ok)
			{
				...
				if (connected_ok)
				{
					...
					libp2p_gossipsub_message_t pub_msg = {
						.topic = {.struct_size = sizeof(pub_msg.topic), .topic = topic_name},
						.data = payload,
						.data_len = sizeof(payload),
						.from = NULL,
						.seqno = NULL,
					...
					if (publish_ok)
					{
						...
						if (qlen_first_ok)
						{
							...
							print_result("gossipsub_publish_propagation_again",
								     publish_again_ok);
							if (!publish_again_ok)
								failures++;
							...
							if (publish_again_ok)
							{
								usleep(10000);
								size_t qlen_second =
									libp2p_gossipsub__peer_sendq_len(gs, mesh_peer);
								int qlen_second_ok = (qlen_second >= 1);
								print_result("gossipsub_propagation_queue_after_second",
									     qlen_second_ok);
								if (!qlen_second_ok)
									failures++;
							}
...

	{
		...
		if (subscribe_ok && source_ok && target_ok)
		{
			...
			libp2p_gossipsub_message_t inbound_msg = {
				.topic = {.struct_size = sizeof(inbound_msg.topic), .topic = topic_name},
				.data = payload,
				.data_len = sizeof(payload),
				.from = source_peer,
				.seqno = seqno,
			...
			if (encode_ok)
			{
				...
				if (inject_ok)
				{
					...
					if (qlen_first_ok)
					{
						inj_rc = libp2p_gossipsub__inject_frame(gs, source_peer, frame,
											frame_len);
						...
						if (inject_second_ok)
						{
							usleep(10000);
							size_t qlen_second =
								libp2p_gossipsub__peer_sendq_len(gs, target_peer);
...

	{
		...
		if (subscribe_ok && source_ok && target_ok)
		{
			...
			libp2p_gossipsub_message_t second_msg = {
				.topic = {.struct_size = sizeof(second_msg.topic), .topic = topic_name},
				.data = payload_second,
			...
			libp2p_err_t enc_first_rc =
				libp2p_gossipsub_rpc_encode_publish(&first_msg, &frame_first, &frame_first_len);
			...
			libp2p_err_t enc_second_rc =
				libp2p_gossipsub_rpc_encode_publish(&second_msg, &frame_second, &frame_second_len);
			...
			{
				libp2p_err_t inj_rc =
					libp2p_gossipsub__inject_frame(gs, source_peer, frame_first, frame_first_len);
				...
				if (inject_first_ok)
				{
					...
					if (qlen_first_ok)
					{
						inj_rc = libp2p_gossipsub__inject_frame(gs, source_peer, frame_second,
											frame_second_len);
						int inject_second_ok = (inj_rc == LIBP2P_ERR_OK);
...

	{
		...
		peer_id_t *source_peer = NULL;
		peer_id_t *ihave_peer = NULL;
		peer_id_t *request_peer = NULL;
		int source_ok = (peer_id_new_from_text(source_peer_str, &source_peer) == PEER_ID_OK);
		...
		const uint8_t *source_peer_bytes = NULL;
		size_t source_peer_len = 0;
		size_t message_id_len = 0;
		uint8_t *message_id = NULL;
		int message_id_ok =
			(source_ok &&
			 peer_id_multihash_view(source_peer, &source_peer_bytes, &source_peer_len) == PEER_ID_OK &&
			 source_peer_bytes != NULL && source_peer_len > 0);
		if (message_id_ok)
		{
			message_id_len = source_peer_len + sizeof(seqno_bytes);
			message_id = (uint8_t *)malloc(message_id_len);
			message_id_ok = (message_id != NULL);
			if (message_id_ok)
			{
				memcpy(message_id, source_peer_bytes, source_peer_len);
				memcpy(message_id + source_peer_len, seqno_bytes, sizeof(seqno_bytes));
			}
		}
		...
		if (subscribe_ok && ihave_ok && message_id_ok)
		{
			...
			libp2p_err_t ihave_enc = encode_control_ihave_rpc(topic_name, message_id, message_id_len,
									  &ihave_frame, &ihave_frame_len);
			...
			if (ihave_enc_ok)
			{
				libp2p_err_t inject_rc =
					libp2p_gossipsub__inject_frame(gs, ihave_peer, ihave_frame, ihave_frame_len);
		...
		if (subscribe_ok && source_ok && message_id_ok)
		{
			...
			libp2p_err_t pub_enc =
				libp2p_gossipsub_rpc_encode_publish(&pub_msg, &pub_frame, &pub_frame_len);
			...
			{
				libp2p_err_t inject_rc =
					libp2p_gossipsub__inject_frame(gs, source_peer, pub_frame, pub_frame_len);
				int inject_ok = (inject_rc == LIBP2P_ERR_OK);
		...
		if (subscribe_ok && request_ok && message_id_ok)
		{
			...
			libp2p_err_t iwant_enc =
				encode_control_iwant_rpc(message_id, message_id_len, &iwant_frame, &iwant_frame_len);
			...
			{
				libp2p_err_t inject_rc =
					libp2p_gossipsub__inject_frame(gs, request_peer, iwant_frame, iwant_frame_len);
				int inject_ok = (inject_rc == LIBP2P_ERR_OK);
...

	{
		...
		if (peer_ok)
		{
			...
			{
				uint8_t message_id[4] = {(uint8_t)(0x40 | attempt), 0xAA, 0x55,
							 (uint8_t)(0x10 + attempt)};
				uint8_t *ihave_frame = NULL;
				size_t ihave_frame_len = 0;
				libp2p_err_t enc_rc = encode_control_ihave_rpc(
					topic_name, message_id, sizeof(message_id), &ihave_frame, &ihave_frame_len);
				...
				libp2p_err_t inj_rc =
					libp2p_gossipsub__inject_frame(gs, throttle_peer, ihave_frame, ihave_frame_len);
			...
			if (hb_ok)
			{
				...
				else
				{
					libp2p_err_t inj_rc = libp2p_gossipsub__inject_frame(
						gs, throttle_peer, ihave_frame, ihave_frame_len);
					int inject_ok = (inj_rc == LIBP2P_ERR_OK);
...

	{
		...
		if (subscribe_ok)
		{
			...
			peer_id_t *high_peer = NULL;
			peer_id_t *low_peer = NULL;
			...
			if (high_peer_ok)
			{
				...
				libp2p_err_t enc_rc = encode_control_ihave_rpc(
					topic_name, message_id, sizeof(message_id), &high_frame, &high_frame_len);
				...
				else
				{
					...
					libp2p_err_t inj_rc = libp2p_gossipsub__inject_frame(gs, high_peer, high_frame,
											     high_frame_len);
			...
			if (low_peer_ok)
			{
				...
				libp2p_err_t enc_rc = encode_control_ihave_rpc(
					topic_name, message_id, sizeof(message_id), &low_frame, &low_frame_len);
				...
				else
				{
					...
					libp2p_err_t inj_rc =
						libp2p_gossipsub__inject_frame(gs, low_peer, low_frame, low_frame_len);
					int inj_ok = (inj_rc == LIBP2P_ERR_OK);
...

	{
		...
		if (subscribe_ok && peer_ok)
		{
			...
			if (graft_enc_ok)
			{
				libp2p_err_t graft_inj =
					libp2p_gossipsub__inject_frame(gs, remote_peer, graft_frame, graft_frame_len);
...

	{
		...
		if (subscribe_ok && peer_ok)
		{
			...
			if (prune_enc_ok)
			{
				libp2p_err_t prune_inj =
					libp2p_gossipsub__inject_frame(gs, remote_peer, prune_frame, prune_frame_len);
			...
			if (second_enc_ok)
			{
				libp2p_err_t second_inj =
					libp2p_gossipsub__inject_frame(gs, remote_peer, second_graft, second_graft_len);
			...
			size_t queued_len = 0;
			libp2p_err_t pop_rc =
				libp2p_gossipsub__peer_pop_sendq(gs, remote_peer, &queued_frame, &queued_len);
			...
			else
			{
				...
				libp2p_err_t dec_rc =
					libp2p_gossipsub_rpc_decode_frame(queued_frame, queued_len, &rpc_msg);
				...
				{
					libp2p_gossipsub_ControlMessage *control =
						libp2p_gossipsub_RPC_get_control(rpc_msg);
					...
					if (has_control)
					{
						...
						{
							size_t count =
								libp2p_gossipsub_ControlMessage_count_prune(control);
							...
							{
								libp2p_gossipsub_ControlPrune *prune_entry =
									libp2p_gossipsub_ControlMessage_get_at_prune(
										control, 0);
								int topic_match = 0;
								if (prune_entry &&
								    libp2p_gossipsub_ControlPrune_has_topic(
									    prune_entry))
								{
									size_t len =
										libp2p_gossipsub_ControlPrune_get_size_topic(
											prune_entry);
									const char *raw =
										libp2p_gossipsub_ControlPrune_get_topic(
											prune_entry);
									if (raw && len == strlen(topic_name) &&
									    strncmp(raw, topic_name, len) == 0)
										topic_match = 1;
								}
								print_result(
									"gossipsub_graft_backoff_prune_topic_match",
									topic_match);
								if (!topic_match)
									failures++;

								uint64_t backoff_val =
									prune_entry && libp2p_gossipsub_ControlPrune_has_backoff(
											       prune_entry)
										? libp2p_gossipsub_ControlPrune_get_backoff(
											  prune_entry)
										: 0;
								int backoff_ok = (backoff_val == 60);
								print_result(
									"gossipsub_graft_backoff_prune_backoff_seconds",
									backoff_ok);
								if (!backoff_ok)
									failures++;
							}
...

	{
		...
		libp2p_gossipsub_topic_config_t flood_topic_cfg = {
			...
			.descriptor = {.struct_size = sizeof(flood_topic_cfg.descriptor), .topic = flood_topic_name},
			.score_params = NULL,
		...
		if (flood_sub_ok)
		{
			...
			peer_id_t *flood_high = NULL;
			peer_id_t *flood_low = NULL;
			...
			if (high_peer_ok && low_peer_ok)
			{
				...
				libp2p_gossipsub_message_t flood_msg1 = {
					.topic = {.struct_size = sizeof(flood_msg1.topic), .topic = flood_topic_name},
					.data = flood_payload1,
				...
				if (flood_pub1_ok)
				{
					int high_received_initial =
						gossipsub_wait_for_peer_frame(gs, flood_high, 1000, NULL);
					int low_received_initial =
						gossipsub_wait_for_peer_frame(gs, flood_low, 1000, NULL);
				...
				libp2p_gossipsub_message_t flood_msg2 = {
					.topic = {.struct_size = sizeof(flood_msg2.topic), .topic = flood_topic_name},
					.data = flood_payload2,
				...
				if (flood_pub2_ok)
				{
					...
					size_t low_queue = 0;
					int low_stayed_empty =
						gossipsub_wait_for_peer_idle(gs, flood_low, 200, &low_queue);
					int flood_threshold_enforced =
						(high_received && low_stayed_empty && low_queue == 0);
...

	{
		...
		libp2p_gossipsub_topic_config_t opp_cfg = {
			...
			.descriptor = {.struct_size = sizeof(opp_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
		...
		const char *mesh_ids[] = {"12D3KooWQbYFJd8hVZCqxx1Zt9KJ9kzPWxtLs5FoX4iVQQPPrw3X",
		...
		const char *candidate_ids[] = {"12D3KooWJwKkHy2W2Ck3RyAEJgVhtSGcVwqDAVBBusZT6F4LmSpa",
		...
		if (mesh_setup_ok)
		{
			for (size_t i = 0; i < mesh_count; ++i)
			{
				...
				if (libp2p_gossipsub__peer_set_score(gs, mesh_peers[i], mesh_scores[i]) !=
				    LIBP2P_ERR_OK)
				{
					mesh_setup_ok = 0;
					break;
				}
				if (libp2p_gossipsub__topic_mesh_add_peer(gs, topic_name, mesh_peers[i],
									  mesh_outbound[i]) != LIBP2P_ERR_OK)
				{
					mesh_setup_ok = 0;
					break;
				}
		...
		if (candidate_setup_ok)
		{
			for (size_t i = 0; i < candidate_count; ++i)
			{
				...
				if (libp2p_gossipsub__peer_set_score(gs, candidate_peers[i], candidate_scores[i]) !=
				    LIBP2P_ERR_OK)
				{
					candidate_setup_ok = 0;
					break;
		...
		if (candidate_setup_ok && candidate_count > 0)
		{
			...
			{
				libp2p_err_t prune_inj =
					libp2p_gossipsub__inject_frame(gs, candidate_peers[0], prune_buf, prune_len);
			...
			if (prune_inj_ok)
				backoff_seeded =
					libp2p_gossipsub__topic_backoff_contains(gs, topic_name, candidate_peers[0]);
		}
		...
		if (opp_ok)
		{
			...
			size_t expected_mesh = mesh_count + ((candidate_count < (size_t)cfg.opportunistic_graft_peers)
								     ? candidate_count
								     : (size_t)cfg.opportunistic_graft_peers);
			if (mesh_after >= expected_mesh)
				mesh_growth_ok = 1;

			for (size_t i = 0; i < candidate_count; ++i)
			{
				candidate_mesh_ok[i] = libp2p_gossipsub__topic_mesh_contains(
					gs, topic_name, candidate_peers[i], NULL, NULL);
				size_t sendq_len = libp2p_gossipsub__peer_sendq_len(gs, candidate_peers[i]);
				candidate_sendq_ok[i] = (sendq_len > 0);
			}
			backoff_cleared =
				(libp2p_gossipsub__topic_backoff_contains(gs, topic_name, candidate_peers[0]) == 0);
		}
		...
		for (size_t i = 0; i < candidate_count; ++i)
		{
			...
			if (!candidate_mesh_ok[i])
				failures++;
			char queue_label[64];
			snprintf(queue_label, sizeof(queue_label), "gossipsub_opportunistic_candidate%zu_graft_queue",
				 i);
