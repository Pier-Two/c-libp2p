...
int gossipsub_service_run_heartbeat_and_gossip_tests(gossipsub_service_test_env_t *env)
{
...

	{
		...
		peer_id_t *peer = NULL;
		int peer_ok = (peer_id_new_from_text(peer_str, &peer) == PEER_ID_OK);
		...
		if (peer_ok)
		{
			...
			int remote_ok = (peer_id_new_from_text(peer_str, &remote) == PEER_ID_OK);
			print_result("gossipsub_stream_write_closed_remote_peer_alloc", remote_ok);
			if (!remote_ok)
				failures++;

			gossipsub_test_closed_stream_ctx_t *ctx =
				(gossipsub_test_closed_stream_ctx_t *)calloc(1, sizeof(*ctx));
			int ctx_ok = (ctx != NULL);
			...
			if (remote_ok && ctx_ok)
			{
				...
				s = libp2p_stream_from_ops(env->host, ctx, &ops, "/meshsub/1.0.0", 1 /* initiator */,
							   remote);
...

	{
		...
		if (peer_ok)
		{
			...
			gossipsub_test_again_stream_ctx_t *ctx =
				(gossipsub_test_again_stream_ctx_t *)calloc(1, sizeof(*ctx));
			int ctx_ok = (ctx != NULL);
			...
			if (remote_ok && ctx_ok)
			{
				...
				if (stream_ok)
				{
					...
					/* Wait for publish propagation to enqueue the frame. */
					size_t qlen = 0;
					int queued_ok = 0;
...

	{
		...
		libp2p_gossipsub_topic_config_t hb_topic_cfg = {
			...
			.descriptor = {.struct_size = sizeof(hb_topic_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
		...
		if (subscribe_ok && peers_created)
		{
			...
			for (size_t i = 0; i < peer_count; ++i)
			{
				...
				(void)libp2p_gossipsub__peer_set_connected(gs, peers[i], 1);
			}
			...
			if (subscriptions_ok)
			{
				...
				for (size_t i = 0; i < peer_count; ++i)
				{
					int contains = libp2p_gossipsub__topic_mesh_contains(gs, topic_name, peers[i],
											     NULL, NULL);
					...
					if (pop_rc == LIBP2P_ERR_OK && frame && frame_len)
					{
						    ...
						    rpc)
						{
							if (libp2p_gossipsub_RPC_has_control(rpc))
							{
								...
								if (control &&
								    libp2p_gossipsub_ControlMessage_has_graft(
									    control) &&
								    libp2p_gossipsub_ControlMessage_count_graft(
									    control) > 0)
								{
									libp2p_gossipsub_ControlGraft *graft =
										libp2p_gossipsub_ControlMessage_get_at_graft(
											control, 0);
									if (graft &&
									    libp2p_gossipsub_ControlGraft_has_topic(
										    graft))
									{
										size_t len =
											libp2p_gossipsub_ControlGraft_get_size_topic(
												graft);
										const char *raw =
											libp2p_gossipsub_ControlGraft_get_topic(
												graft);
...

	{
		...
		libp2p_gossipsub_topic_config_t prune_cfg = {
			...
			.descriptor = {.struct_size = sizeof(prune_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
		...
		if (subscribe_ok && peers_created)
		{
			...
			for (size_t i = 0; i < peer_count; ++i)
			{
				...
				libp2p_err_t mesh_rc =
					libp2p_gossipsub__topic_mesh_add_peer(gs, topic_name, peers[i], (int)(i % 2));
			...
			if (setup_ok)
			{
				...
				for (size_t i = 0; i < peer_count; ++i)
				{
					int in_mesh = libp2p_gossipsub__topic_mesh_contains(gs, topic_name, peers[i],
											    NULL, NULL);
					if (!in_mesh)
					{
						pruned_count++;
							    ...
							    rpc)
							{
								if (libp2p_gossipsub_RPC_has_control(rpc))
								{
									libp2p_gossipsub_ControlMessage *control =
										libp2p_gossipsub_RPC_get_control(rpc);
									if (control &&
									    libp2p_gossipsub_ControlMessage_has_prune(
										    control) &&
									    libp2p_gossipsub_ControlMessage_count_prune(
										    control) > 0)
									{
...

	{
		...
		libp2p_gossipsub_topic_config_t quota_cfg = {
			...
			.descriptor = {.struct_size = sizeof(quota_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
