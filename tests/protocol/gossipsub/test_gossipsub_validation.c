...
static void counting_async_validator(const libp2p_gossipsub_message_t *msg, libp2p_gossipsub_validator_done_fn done,
				     void *user_data, void *done_user_data)
{
	(void)msg;
	atomic_fetch_add_explicit((atomic_int *)user_data, 1, memory_order_relaxed);
	if (done)
...
static void counting_message_delivery(libp2p_gossipsub_t *gs, const libp2p_gossipsub_message_t *msg,
				      const uint8_t *message_id, size_t message_id_len,
				      const peer_id_t *propagation_source, void *user_data)
{
	(void)gs;
	(void)msg;
	(void)user_data;
	...
	{
		size_t written = 0;
		if (peer_id_text_write(propagation_source, PEER_ID_TEXT_LEGACY_BASE58, g_last_source_text,
				       sizeof(g_last_source_text), &written) != PEER_ID_OK)
...
static int wait_for_peer_frame(libp2p_gossipsub_t *gs, const peer_id_t *peer, uint64_t timeout_ms,
...
int main(void)
{
	...
	libp2p_gossipsub_validator_def_t sync_def = {.struct_size = sizeof(sync_def),
	...
	libp2p_gossipsub_validator_def_t async_def = {.struct_size = sizeof(async_def),
	...
	libp2p_gossipsub_message_t msg = {.topic = {.struct_size = sizeof(msg.topic), .topic = topic_name},
					  .data = payload,
	...
	int recipient_ok = setup_subscribed_peer(gs, topic_name, "12D3KooWPy9khA5QLm5m8wKtPp1sHwoMvV7bgidhraNEnNh4tW7x",
						 &recipient_peer);
	...
	int source_ok = (peer_id_new_from_text("12D3KooWMmV2UmyHk4G2MijvSEezwEWj1vDLteA94ppWKqhzyzk5", &source_peer) ==
			 PEER_ID_OK);
	...
	if (recipient_ok && source_ok)
	{
		...
		if (ok)
		{
			for (int i = 0; i < 200 && atomic_load_explicit(&g_delivery_called, memory_order_relaxed) == 0;
			     ++i)
				usleep(1000);
			ok = (atomic_load_explicit(&g_delivery_called, memory_order_relaxed) == 1 &&
			      g_last_message_id_len > 0 &&
			      strcmp(g_last_source_text, "12D3KooWMmV2UmyHk4G2MijvSEezwEWj1vDLteA94ppWKqhzyzk5") == 0);
		...
		if (g_last_message_id_len > 0)
		{
			rc = libp2p_gossipsub_report_message_validation_result(
				gs, g_last_message_id, g_last_message_id_len, LIBP2P_GOSSIPSUB_VALIDATION_ACCEPT);
			ok = (rc == LIBP2P_ERR_OK);
		...
		libp2p_gossipsub_message_t reject_msg = {
			.topic = {.struct_size = sizeof(reject_msg.topic), .topic = topic_name},
			.data = reject_payload,
		...
		{
			rc = libp2p_gossipsub_report_message_validation_result(
				gs, g_last_message_id, g_last_message_id_len, LIBP2P_GOSSIPSUB_VALIDATION_REJECT);
			ok = (rc == LIBP2P_ERR_OK);
		...
		libp2p_gossipsub_message_t expire_msg = {
			.topic = {.struct_size = sizeof(expire_msg.topic), .topic = topic_name},
			.data = expire_payload,
