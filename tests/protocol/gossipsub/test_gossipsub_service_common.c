...
const uint8_t kTestPxSecretKey[32] = {0x9d, 0x61, 0xb1, 0x9d, 0xef, 0xfd, 0x5a, 0x60, 0xba, 0x84, 0x4a,
...
static int peer_id_bytes_view(const peer_id_t *peer, const uint8_t **out_bytes, size_t *out_len)
{
	if (!peer || !out_bytes || !out_len)
		return 0;
	*out_bytes = NULL;
	*out_len = 0;
	return peer_id_multihash_view(peer, out_bytes, out_len) == PEER_ID_OK && *out_bytes != NULL && *out_len > 0;
}
...
static libp2p_err_t build_peer_record_unsigned(const uint8_t *payload_type, size_t payload_type_len,
					       ...
					       size_t *out_len)
{
	...
	const uint8_t *fields[] = {(const uint8_t *)"libp2p-peer-record", payload_type, payload};
	size_t lengths[] = {strlen("libp2p-peer-record"), payload_type_len, payload_len};
				    ...
				    size_t addr_count)
{
	...
	size_t peer_len = 0;
	if (!pbuf || !peer || !peer_id_bytes_view(peer, &peer_bytes, &peer_len))
		return 0;
	if (noise_protobuf_write_bytes(pbuf, 1, peer_bytes, peer_len) != NOISE_ERROR_NONE)
		return 0;
				       ...
				       size_t *out_len)
{
	...
	*out_len = 0;
	...
	peer_id_error_t pb_rc =
		peer_id_build_public_key_protobuf(PEER_ID_KEY_ED25519, pk, sizeof(pk), &pubkey_pb, &pubkey_pb_len);
	...
	rc = build_peer_record_unsigned(kPeerRecordPayloadType, sizeof(kPeerRecordPayloadType), record_buf, record_len,
	...
	if (noise_protobuf_write_bytes(&measure, 1, pubkey_pb, pubkey_pb_len) != NOISE_ERROR_NONE ||
	    noise_protobuf_write_bytes(&measure, 2, kPeerRecordPayloadType, sizeof(kPeerRecordPayloadType)) !=
		    NOISE_ERROR_NONE ||
	...
	if (noise_protobuf_write_bytes(&out_pb, 1, pubkey_pb, pubkey_pb_len) != NOISE_ERROR_NONE ||
	    noise_protobuf_write_bytes(&out_pb, 2, kPeerRecordPayloadType, sizeof(kPeerRecordPayloadType)) !=
		    NOISE_ERROR_NONE ||
				 ...
				 size_t signed_record_len, uint8_t **out_buf, size_t *out_len)
{
	const uint8_t *peer_bytes = NULL;
	size_t peer_len = 0;
	if (!topic || !px_peer || !peer_id_bytes_view(px_peer, &peer_bytes, &peer_len) || !out_buf || !out_len)
		return LIBP2P_ERR_NULL_PTR;
	*out_buf = NULL;
	*out_len = 0;
	...
	noise_rc = libp2p_gossipsub_PeerInfo_set_peer_id(info, peer_bytes, peer_len);
	if (noise_rc != NOISE_ERROR_NONE)
		goto cleanup;
	...
	if (include_px)
	{
		...
		if (peer_id_new_from_text(px_peer_id, &px_peer) == PEER_ID_OK)
		{
			...
			if (noise_rc == NOISE_ERROR_NONE && px_info)
			{
				const uint8_t *peer_bytes = NULL;
				size_t peer_len = 0;
				if (!peer_id_bytes_view(px_peer, &peer_bytes, &peer_len))
					noise_rc = NOISE_ERROR_INVALID_PARAM;
				else
					noise_rc = libp2p_gossipsub_PeerInfo_set_peer_id(px_info, peer_bytes, peer_len);
			}
