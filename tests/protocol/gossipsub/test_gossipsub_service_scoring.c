...
static int run_ip_colocation_test(gossipsub_service_test_env_t *env)
{
	...
	libp2p_gossipsub_topic_config_t coloc_cfg = {
		...
		.descriptor = {.struct_size = sizeof(coloc_cfg.descriptor), .topic = topic_name},
		.score_params = NULL};
...
int gossipsub_service_run_scoring_tests(gossipsub_service_test_env_t *env)
{
	...
	int failures = 0;
	{
		...
		libp2p_gossipsub_topic_score_params_t score_params = {.struct_size = sizeof(score_params),
		...
		libp2p_gossipsub_topic_config_t score_cfg = {
			...
			.descriptor = {.struct_size = sizeof(score_cfg.descriptor), .topic = topic_name},
...

	{
		...
		libp2p_gossipsub_topic_config_t op_cfg = {
			...
			.descriptor = {.struct_size = sizeof(op_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
...

	{
		...
		libp2p_gossipsub_topic_config_t first_cfg = {
			...
			.descriptor = {.struct_size = sizeof(first_cfg.descriptor), .topic = topic_name},
			.score_params = &score_params};
		...
		if (subscribe_ok && peer_ok)
		{
			...
			if (mesh_ok)
			{
				...
				size_t frame_len = 0;
				libp2p_err_t enc_rc =
					libp2p_gossipsub_rpc_encode_publish(&first_msg, &frame, &frame_len);
...

	{
		...
		libp2p_gossipsub_topic_config_t mesh_cfg = {
			...
			.descriptor = {.struct_size = sizeof(mesh_cfg.descriptor), .topic = topic_name},
			.score_params = &score_params};
...

	{
		...
		libp2p_gossipsub_topic_config_t failure_cfg = {
			...
			.descriptor = {.struct_size = sizeof(failure_cfg.descriptor), .topic = topic_name},
			.score_params = &score_params};
...

	{
		...
		libp2p_gossipsub_topic_config_t invalid_cfg = {
			...
			.descriptor = {.struct_size = sizeof(invalid_cfg.descriptor), .topic = topic_name},
			.score_params = &score_params};
		...
		if (subscribe_ok)
		{
			libp2p_gossipsub_validator_def_t def = {.struct_size = sizeof(def),
...

	{
		...
		libp2p_gossipsub_topic_config_t missing_cfg = {
			...
			.descriptor = {.struct_size = sizeof(libp2p_gossipsub_topic_descriptor_t),
				       .topic = runtime_topic},
			.score_params = NULL};
		...
		libp2p_gossipsub_topic_score_params_t runtime_score_params = {.struct_size =
										      sizeof(runtime_score_params),
									      .topic_weight = 1.0,
...

	{
		...
		libp2p_gossipsub_topic_config_t spam_cfg = {
			...
			.descriptor = {.struct_size = sizeof(spam_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
		...
		if (subscribe_ok && peer_ok)
		{
			...
			for (size_t i = 0; i < 3; ++i)
			{
				...
				size_t frame_len = 0;
				libp2p_err_t enc_rc =
					encode_control_ihave_rpc(topic_name, msgs[i], msg_lens[i], &frame, &frame_len);
				...
				else
				{
					...
					double expected_score =
						(expected_penalty > 0.0 && penalty_weight != 0.0)
...

	{
		...
		libp2p_gossipsub_topic_config_t promise_cfg = {
			...
			.descriptor = {.struct_size = sizeof(promise_cfg.descriptor), .topic = topic_name},
			.score_params = NULL};
		...
		if (subscribe_ok && peer_ok)
		{
			...
			libp2p_err_t enc_rc =
				encode_control_ihave_rpc(topic_name, msg_id, sizeof(msg_id), &ihave_frame, &ihave_len);
