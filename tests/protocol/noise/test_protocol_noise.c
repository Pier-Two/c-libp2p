...
{
	uint8_t key[32] = {0};
	libp2p_noise_config_t cfg = {.identity_private_key = key,
				     .identity_private_key_len = sizeof(key),
				     .identity_key_type = PEER_ID_KEY_ED25519};
	libp2p_security_t *sec = libp2p_noise_security_new(&cfg);
...
static int peer_id_from_rsa_priv(const uint8_t *key, size_t key_len, peer_id_t **pid)
{
	...
	uint8_t *pubpb = NULL;
	size_t pubpb_len = 0;
	int rc = peer_id_public_key_pb_from_private_raw(PEER_ID_KEY_RSA, key, key_len, &pubpb, &pubpb_len);
...
static int peer_id_from_ecdsa_priv(const uint8_t *key, size_t key_len, peer_id_t **pid)
{
	uint8_t *pubpb = NULL;
	size_t pubpb_len = 0;
	int rc = peer_id_public_key_pb_from_private_raw(PEER_ID_KEY_ECDSA, key, key_len, &pubpb, &pubpb_len);
...
static void test_handshake_success(void)
{
	...
	TEST_OK("handshake return codes", cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK,
...
static void test_identity_handshake_success(void)
{
	uint8_t key_cli[32];
	uint8_t key_srv[32];
	uint8_t id_cli[32] = {0x9d, 0x61, 0xb1, 0x9d, 0xef, 0xfd, 0x5a, 0x60, 0xba, 0x84, 0x4a,
			      0xf4, 0x92, 0xec, 0x2c, 0xc4, 0x44, 0x49, 0xc5, 0x69, 0x7b, 0x32,
			      0x69, 0x19, 0x70, 0x3b, 0xac, 0x03, 0x1c, 0xae, 0x7f, 0x60};
	uint8_t id_srv[32] = {0x4c, 0xcd, 0x08, 0x9b, 0x28, 0xff, 0x96, 0xda, 0x9d, 0xb6, 0xc3,
	...
	peer_id_t *expected_srv = NULL;
	TEST_OK("derive server peer id", peer_id_from_ed25519_priv(id_srv, &expected_srv) == 0,
		"srv peer id derivation failed");
	...
	peer_id_t *expected_cli = NULL;
	TEST_OK("derive client peer id", peer_id_from_ed25519_priv(id_cli, &expected_cli) == 0,
		"cli peer id derivation failed");

	libp2p_security_t *sec_cli = libp2p_noise_security_new(&cfg_cli);
	libp2p_security_t *sec_srv = libp2p_noise_security_new(&cfg_srv);
	TEST_OK("security objects allocated (id)", sec_cli && sec_srv, "cli=%p srv=%p", (void *)sec_cli,
		(void *)sec_srv);
	if (!sec_cli || !sec_srv)
		return;
	...
	TEST_OK("handshake return codes (id)", cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);
	TEST_OK("remote peer verified", cli_args.remote_peer && peer_id_equal(cli_args.remote_peer, expected_srv) == 1,
		"match=%d", cli_args.remote_peer ? peer_id_equal(cli_args.remote_peer, expected_srv) : -1);
	TEST_OK("server saw client", srv_args.remote_peer && peer_id_equal(srv_args.remote_peer, expected_cli) == 1,
		"match=%d", srv_args.remote_peer ? peer_id_equal(srv_args.remote_peer, expected_cli) : -1);
	...
	//     printf(" %02x", (unsigned char)buf[i]);
	// printf("\n");
	TEST_OK("data after handshake (id)", n == sizeof(ping) && memcmp(buf, ping, sizeof(ping)) == 0,
		"read %zd bytes", n);
	...
	free_hs_args(&cli_args);
	free_hs_args(&srv_args);
	peer_id_free(expected_cli);
	peer_id_free(expected_srv);
}
...
static void test_secp256k1_identity_handshake_success(void)
{
	uint8_t key_cli[32];
	uint8_t key_srv[32];
	uint8_t id_cli[32] = {0x85, 0x51, 0x15, 0xf4, 0xe0, 0xe7, 0x8a, 0xd2, 0x81, 0x10, 0x17,
			      0xee, 0x12, 0xa9, 0x6c, 0x65, 0xcf, 0x8c, 0x24, 0x34, 0xae, 0x86,
			      0x5e, 0x89, 0xc9, 0x50, 0x10, 0xd8, 0x70, 0xbd, 0xe2, 0x7c};
	uint8_t id_srv[32] = {0x6f, 0x38, 0x31, 0xe1, 0xe1, 0x34, 0x3b, 0x9d, 0x8f, 0x33, 0x8e,
			      0x61, 0xb8, 0x4f, 0x51, 0xbe, 0xfb, 0x5a, 0xcc, 0x95, 0x7c, 0x91,
			      0x8e, 0x82, 0xc8, 0x5a, 0xc6, 0x43, 0x4d, 0xb7, 0xef, 0x9c};
	...
	libp2p_noise_config_t cfg_cli = {.static_private_key = key_cli,
					 ...
					 .identity_private_key = id_cli,
					 .identity_private_key_len = sizeof(id_cli),
					 .identity_key_type = PEER_ID_KEY_SECP256K1,
					 .max_plaintext = 0};
	...
	libp2p_security_t *sec_cli = libp2p_noise_security_new(&cfg_cli);
	libp2p_security_t *sec_srv = libp2p_noise_security_new(&cfg_srv);
	TEST_OK("security objects allocated (secp256k1)", sec_cli && sec_srv, "cli=%p srv=%p", (void *)sec_cli,
		(void *)sec_srv);
	if (!sec_cli || !sec_srv)
		return;
	...
	TEST_OK("handshake return codes (secp256k1)",
		cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK, "cli=%d srv=%d", cli_args.rc,
		srv_args.rc);
	...
	//     printf(" %02x", (unsigned char)buf[i]);
	// printf("\n");
	TEST_OK("data after handshake (secp256k1)", n == sizeof(ping) && memcmp(buf, ping, sizeof(ping)) == 0,
		"read %zd bytes", n);
...
static void test_rsa_identity_handshake_success(void)
{
	...
	libp2p_noise_config_t cfg_cli = {.static_private_key = key_cli,
					 ...
					 .identity_key_type = PEER_ID_KEY_RSA,
					 .max_plaintext = 0};
	libp2p_noise_config_t cfg_srv = {.static_private_key = key_srv,
					 .static_private_key_len = sizeof(key_srv),
					 .identity_private_key = id_srv,
	...
	libp2p_security_t *sec_cli = libp2p_noise_security_new(&cfg_cli);
	libp2p_security_t *sec_srv = libp2p_noise_security_new(&cfg_srv);
	TEST_OK("security objects allocated (rsa)", sec_cli && sec_srv, "cli=%p srv=%p", (void *)sec_cli,
		(void *)sec_srv);
	if (!sec_cli || !sec_srv)
		return;
	...
	TEST_OK("handshake return codes (rsa)", cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);
	TEST_OK("remote peer verified (rsa)",
		cli_args.remote_peer && peer_id_equal(cli_args.remote_peer, expected_srv) == 1, "match=%d",
		cli_args.remote_peer ? peer_id_equal(cli_args.remote_peer, expected_srv) : -1);
	TEST_OK("server saw client (rsa)",
		srv_args.remote_peer && peer_id_equal(srv_args.remote_peer, expected_cli) == 1, "match=%d",
		srv_args.remote_peer ? peer_id_equal(srv_args.remote_peer, expected_cli) : -1);
...
static void test_ecdsa_identity_handshake_success(void)
{
	...
	peer_id_t *expected_cli = NULL;
	peer_id_t *expected_srv = NULL;
	TEST_OK("derive cli ecdsa peer id", peer_id_from_ecdsa_priv(id_cli, id_cli_len, &expected_cli) == 0,
		"pid fail");
	TEST_OK("derive srv ecdsa peer id", peer_id_from_ecdsa_priv(id_srv, id_srv_len, &expected_srv) == 0,
		"pid fail");
	...
	libp2p_noise_config_t cfg_cli = {.static_private_key = key_cli,
					 ...
					 .identity_private_key = id_cli,
					 .identity_private_key_len = id_cli_len,
					 .identity_key_type = PEER_ID_KEY_ECDSA,
					 .max_plaintext = 0};
	libp2p_noise_config_t cfg_srv = {.static_private_key = key_srv,
					 .static_private_key_len = sizeof(key_srv),
					 .identity_private_key = id_srv,
	...
	libp2p_security_t *sec_cli = libp2p_noise_security_new(&cfg_cli);
	libp2p_security_t *sec_srv = libp2p_noise_security_new(&cfg_srv);
	TEST_OK("security objects allocated (ecdsa)", sec_cli && sec_srv, "cli=%p srv=%p", (void *)sec_cli,
		(void *)sec_srv);
	if (!sec_cli || !sec_srv)
		return;
	...
	TEST_OK("handshake return codes (ecdsa)",
		cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK, "cli=%d srv=%d", cli_args.rc,
		srv_args.rc);
	TEST_OK("remote peer verified (ecdsa)",
		cli_args.remote_peer && peer_id_equal(cli_args.remote_peer, expected_srv) == 1, "match=%d",
		cli_args.remote_peer ? peer_id_equal(cli_args.remote_peer, expected_srv) : -1);
	TEST_OK("server saw client (ecdsa)",
		srv_args.remote_peer && peer_id_equal(srv_args.remote_peer, expected_cli) == 1, "match=%d",
		srv_args.remote_peer ? peer_id_equal(srv_args.remote_peer, expected_cli) : -1);
...
static void test_early_data_extensions(void)
{
	...
	TEST_OK("handshake ok (ed)", cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);

	size_t len = 0;
	const uint8_t *ed_from_srv = noise_conn_get_early_data(cli_args.out, &len);
	TEST_OK("cli got srv early data", len == sizeof(srv_ed) && memcmp(ed_from_srv, srv_ed, len) == 0, "len=%zu",
		len);
	...
	TEST_OK("srv got cli early data", len == sizeof(cli_ed) && memcmp(ed_from_cli, cli_ed, len) == 0, "len=%zu",
		len);
	...
	TEST_OK("cli parsed srv ext",
		pex_srv && pex_srv->num_webtransport_certhashes == 1 && pex_srv->webtransport_certhashes_lens[0] == 2 &&
			memcmp(pex_srv->webtransport_certhashes[0], srv_ext + 2, 2) == 0 &&
			pex_srv->num_stream_muxers == 0,
		"");
...
static void test_missing_static_key(void)
{
	...
	TEST_OK("handshake ok with generated static keys",
		cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK, "cli=%d srv=%d", cli_args.rc,
		srv_args.rc);
...
static libp2p_security_err_t first_msg_payload_handshake(libp2p_security_t *sec, libp2p_conn_t *raw)
{
	...
	for (;;)
	{
		...
		if (action == NOISE_ACTION_WRITE_MESSAGE)
		{
			...
			lenbuf[1] = (uint8_t)l;
			if (libp2p_conn_write(raw, lenbuf, 2) != 2 ||
			    libp2p_conn_write(raw, buf, mbuf.size) != (ssize_t)mbuf.size)
			{
				noise_handshakestate_free(hs);
				return LIBP2P_SECURITY_ERR_HANDSHAKE;
			}
...
static void test_corrupt_payload(void)
{
	...
	TEST_OK("handshake fail on corrupt payload",
		cli_args.rc == LIBP2P_SECURITY_ERR_HANDSHAKE && srv_args.rc == LIBP2P_SECURITY_ERR_HANDSHAKE,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);
...
static void test_oversized_payload(void)
{
	...
	TEST_OK("handshake fail on oversize payload",
		cli_args.rc == LIBP2P_SECURITY_ERR_HANDSHAKE && srv_args.rc == LIBP2P_SECURITY_ERR_HANDSHAKE,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);
...
static void test_initiator_payload_msg1(void)
{
	...
	TEST_OK("handshake fail on msg1 payload",
		cli_args.rc == LIBP2P_SECURITY_ERR_HANDSHAKE && srv_args.rc == LIBP2P_SECURITY_ERR_HANDSHAKE,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);
...
static void test_max_plaintext_limit(void)
{
	...
	TEST_OK("handshake (maxpt)", cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);
...
static void test_message_counter_limit(void)
{
	...
	TEST_OK("handshake (msgcnt)", cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);
...
static void test_unregistered_extension(void)
{
	...
	TEST_OK("handshake fail on unregistered ext",
		cli_args.rc == LIBP2P_SECURITY_ERR_HANDSHAKE && srv_args.rc == LIBP2P_SECURITY_ERR_HANDSHAKE,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);
...
static void test_experimental_extension(void)
{
	...
	TEST_OK("handshake ok with exp ext", cli_args.rc == LIBP2P_SECURITY_OK && srv_args.rc == LIBP2P_SECURITY_OK,
		"cli=%d srv=%d", cli_args.rc, srv_args.rc);
