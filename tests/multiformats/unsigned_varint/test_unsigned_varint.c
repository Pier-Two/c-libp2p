...
static void run_round_trip_vector(const varint_vector_t *vector)
{
	...
	report_result(test_name, expected_size == vector->encoding_len,
		      "unsigned_varint_size mismatch");
}
...
static void test_encode_errors(void)
{
	...
	written = 0;
	err = unsigned_varint_encode(UINT64_C(0x8000000000000000), out,
				     sizeof(out), &written);
	report_result("encode overflow value",
		      err == UNSIGNED_VARINT_ERR_VALUE_OVERFLOW,
		      "expected VALUE_OVERFLOW");

	written = SIZE_MAX;
	err = unsigned_varint_encode(UINT64_C(300), out, 1, &written);
	report_result("encode buffer over",
		      (err == UNSIGNED_VARINT_ERR_BUFFER_OVER) &&
			      (written == 0),
		      "expected BUFFER_OVER with written reset");

	written = 0;
	err = unsigned_varint_encode(UINT64_C(1), NULL, sizeof(out), &written);
	report_result("encode NULL out", err == UNSIGNED_VARINT_ERR_NULL_PTR,
		      "expected NULL_PTR");

	err = unsigned_varint_encode(UINT64_C(1), out, sizeof(out), NULL);
	report_result("encode NULL written",
		      err == UNSIGNED_VARINT_ERR_NULL_PTR, "expected NULL_PTR");
}

static void run_decode_error_case(const char *name, const uint8_t *input,
				  size_t input_len,
...
static void test_decode_errors(void)
{
	...
	run_decode_error_case("decode non-minimal zero", non_minimal_zero,
			      sizeof(non_minimal_zero),
			      UNSIGNED_VARINT_ERR_NOT_MINIMAL);
	run_decode_error_case("decode non-minimal one", non_minimal_one,
			      sizeof(non_minimal_one),
			      UNSIGNED_VARINT_ERR_NOT_MINIMAL);
	run_decode_error_case("decode non-minimal 127", non_minimal_127,
			      sizeof(non_minimal_127),
			      UNSIGNED_VARINT_ERR_NOT_MINIMAL);
	run_decode_error_case("decode truncated 1", truncated_1,
			      sizeof(truncated_1),
			      UNSIGNED_VARINT_ERR_TOO_LONG);
	run_decode_error_case("decode truncated 2", truncated_2,
			      sizeof(truncated_2),
			      UNSIGNED_VARINT_ERR_TOO_LONG);
	run_decode_error_case("decode overlong 10 bytes", ten_byte_overlong,
			      sizeof(ten_byte_overlong),
			      UNSIGNED_VARINT_ERR_TOO_LONG);
	run_decode_error_case("decode 2^63 overflow", value_2_63,
			      sizeof(value_2_63),
			      UNSIGNED_VARINT_ERR_VALUE_OVERFLOW);

	decoded = UINT64_MAX;
	read = SIZE_MAX;
	err = unsigned_varint_decode(truncated_1, sizeof(truncated_1), &decoded,
				     &read);
	report_result("decode error resets outputs",
		      (err == UNSIGNED_VARINT_ERR_TOO_LONG) && (decoded == 0) &&
			      (read == 0),
	...
	report_result("decode NULL input", err == UNSIGNED_VARINT_ERR_NULL_PTR,
		      "expected NULL_PTR");

	err = unsigned_varint_decode(non_minimal_zero, 2, NULL, &read);
	report_result("decode NULL value", err == UNSIGNED_VARINT_ERR_NULL_PTR,
		      "expected NULL_PTR");

	err = unsigned_varint_decode(non_minimal_zero, 2, &decoded, NULL);
	report_result("decode NULL read", err == UNSIGNED_VARINT_ERR_NULL_PTR,
		      "expected NULL_PTR");

	decoded = UINT64_MAX;
	read = SIZE_MAX;
	err = unsigned_varint_decode(non_minimal_zero, 0, &decoded, &read);
	report_result("decode empty input",
		      (err == UNSIGNED_VARINT_ERR_EMPTY_INPUT) &&
			      (decoded == 0) && (read == 0),
...
static void test_decode_with_trailing_bytes(void)
{
	...
	report_result("decode with trailing bytes",
		      (err == UNSIGNED_VARINT_OK) && (decoded == UINT64_C(1)) &&
			      (read == 1),
...
{
	report_result("size(0) == 1", unsigned_varint_size(UINT64_C(0)) == 1,
		      "unexpected size for 0");
	report_result("size(127) == 1",
		      unsigned_varint_size(UINT64_C(127)) == 1,
		      "unexpected size for 127");
	report_result("size(128) == 2",
		      unsigned_varint_size(UINT64_C(128)) == 2,
		      "unexpected size for 128");
	report_result("size(16383) == 2",
		      unsigned_varint_size(UINT64_C(16383)) == 2,
		      "unexpected size for 16383");
	report_result("size(16384) == 3",
		      unsigned_varint_size(UINT64_C(16384)) == 3,
		      "unexpected size for 16384");
	report_result("size(max) == 9",
		      unsigned_varint_size(UNSIGNED_VARINT_MAX_VALUE) == 9,
		      "unexpected size for max");
	report_result("size(2^63) == 0",
		      unsigned_varint_size(UINT64_C(0x8000000000000000)) == 0,
		      "expected overflow size to be 0");
	report_result("size(UINT64_MAX) == 0",
		      unsigned_varint_size(UINT64_MAX) == 0,
		      "expected overflow size to be 0");
}
