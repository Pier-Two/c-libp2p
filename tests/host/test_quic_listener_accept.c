...
static libp2p_muxer_err_t stub_muxer_open_stream(libp2p_muxer_t *mx, const uint8_t *name, size_t name_len,
						 libp2p_stream_t **out)
{
	(void)mx;
	(void)name;
...
static libp2p_muxer_t *test_muxer_factory(libp2p_host_t *host, libp2p_quic_session_t *session, const multiaddr_t *local,
					  const multiaddr_t *remote, libp2p_conn_t *conn)
{
	factory_called++;
	factory_host = host;
...
int main(void)
{
	...
	libp2p_conn_t *conn = libp2p_quic_conn_new(local_addr, remote_addr, session, stub_session_close,
	...
	ok &= check(rc == 0 && uc != NULL, "inbound upgrade success");
	if (!ok || !uc)
	{
	...
	ok &= check(factory_host == host, "factory host matches");
	ok &= check(factory_session == session, "factory session matches");
	ok &= check(factory_local == libp2p_conn_local_addr(conn), "factory local address reused");
	ok &= check(factory_remote == libp2p_conn_remote_addr(conn), "factory remote address reused");
	...
	ok &= check(uc->muxer != NULL, "uconn muxer created");
