...
static int wait_for_listen_addr(libp2p_host_t *h, char **out, int timeout_ms)
{
	...
	while (waited < timeout_ms)
	{
		...
		if (got == 1)
		{
			...
			{
				*out = strdup(evt.u.listen_addr_added.addr);
...
int main(void)
{
	...
	for (int i = 0; i < 32; i++)
	{
		seedA[i] = (uint8_t)(i + 11);
		seedB[i] = (uint8_t)(0xA0 - i);
	}
	...
	if (!wait_for_listen_addr(hb, &addrB, 3000))
		return 1;
	peer_id_t *pidB = NULL;
	(void)libp2p_host_get_peer_id(hb, &pidB);
	if (!pidB)
	{
		free(addrB);
		return 1;
	...
	/* Identify controller */
	libp2p_identify_service_t *ids = NULL;
	libp2p_identify_opts_t opts = {.struct_size = sizeof(opts)};
