...
static int quic_server_stream_cb(picoquic_cnx_t *cnx, uint64_t stream_id, uint8_t *bytes, size_t length,
				 picoquic_call_back_event_t event, void *app_ctx, void *stream_ctx)
{
	(void)app_ctx;
	(void)stream_ctx;
...
int main(void)
{
	...
	int ident_rc =
		libp2p_quic_tls_identity_from_certificate(server_cert.cert_der, server_cert.cert_len, &server_ident);
	...
	server_q = picoquic_create(8, NULL, NULL, NULL, LIBP2P_QUIC_TLS_ALPN, quic_server_stream_cb, NULL, NULL, NULL,
	...
	int loop_ret = 0;
	server_ctx.thread_ctx = picoquic_start_network_thread(server_q, &server_ctx.loop_param, quic_server_loop_cb,
							      &server_ctx, &loop_ret);
	...
	mx = libp2p_quic_muxer_new(host, session, libp2p_conn_local_addr(conn), remote_ma, conn);
	if (!mx)
	{
		ok = 0;
		goto cleanup;
