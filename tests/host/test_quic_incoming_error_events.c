...
static int expect_incoming_error(libp2p_host_t *host, libp2p_err_t expected_code, const char *expected_substr,
				 int timeout_ms)
{
...
int main(void)
{
	...
	libp2p_conn_t *conn =
		libp2p_quic_conn_new(NULL, NULL, session, stub_session_close, stub_session_free, conn_peer);
	if (!conn)
	{
