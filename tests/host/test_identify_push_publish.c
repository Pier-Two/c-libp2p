...
static int wait_for_protocol_event(libp2p_host_t *h, const char *proto_id, int timeout_ms)
{
	...
	while (waited < timeout_ms)
	{
		...
		if (got == 1)
		{
			if (evt.kind == LIBP2P_EVT_PROTOCOL_NEGOTIATED && evt.u.protocol_negotiated.protocol_id &&
			    proto_id && strcmp(evt.u.protocol_negotiated.protocol_id, proto_id) == 0)
			{
				libp2p_event_free(&evt);
				return 1;
			}
...
int main(void)
{
	...
	if (libp2p_host_dial_protocol_blocking(ha, addrB, LIBP2P_PING_PROTO_ID, 3000, &ping_stream) != 0 ||
	    !ping_stream)
	{
		free(addrB);
		return 1;
	...
	if (hb && pidA)
	{
		...
		fprintf(stderr, "[TEST_PUB] peerstore query result rc=%d count=%zu\n", peerstore_rc, peerstore_count);
		if (peerstore_rc == 0)
		{
	...
	fprintf(stderr, "[TEST_PUB] verdict event_has_dummy=%d peerstore_has_dummy=%d\n", event_has_dummy,
		peerstore_has_dummy);
