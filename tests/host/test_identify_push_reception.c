...
static int wait_for_event_addr(libp2p_host_t *h, libp2p_event_kind_t kind, char **out, int timeout_ms,
			       void (*trigger_cb)(void *), void *trigger_ud)
{
...
int main(void)
{
	...
	if (!wait_for_event_addr(hb, LIBP2P_EVT_LISTEN_ADDR_ADDED, &addrB, 3000, start_hosts_cb, &start_ctx) ||
	    start_ctx.rc != 0)
		return 1;
	...
	peer_id_t *pidA = NULL;
	(void)libp2p_host_get_peer_id(ha, &pidA);
